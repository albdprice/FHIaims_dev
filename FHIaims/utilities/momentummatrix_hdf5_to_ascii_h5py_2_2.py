...
dim=int32(asarray(str(f.items()[ind_mom][1]).strip().split('(')[1].split(')')[0].split(',')))
dip=f.require_dataset(list_of_names[ind_mom],tuple(dim),float)
dip=dip[...]
ind_k=list_of_names.index('k_points')
dim_k=int32(asarray(str(f.items()[ind_k][1]).strip().split('(')[1].split(')')[0].split(',')))
k_points=f.require_dataset(list_of_names[ind_k],tuple(dim_k),float)
k_points=k_points[...]
ind_win=list_of_names.index('Energy_window')
dim_win=int32(asarray(str(f.items()[ind_win][1]).strip().split('(')[1].split(')')[0].split(',')))
win=f.require_dataset(list_of_names[ind_win],tuple(dim_win),float)
win=win[...]
f.close()
