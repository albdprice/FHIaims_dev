...
cd "$JOB_HOME"
cd regression_tests
python3.3 ./regressiontools.py full --force --exclude="ScaLAPACK only" --mpiexe "$MPIEXE" --termsize=$TERMSIZE --cpus $N_TASKS_TEST --batch ./references_lastrelease/ "$AIMS_HOME/bin/aims.x"
