...
subroutine power_genmat_lapack(n_size, matrix, power, safe_minimum, threshold, name)
  ...
  real*8, intent(INOUT) :: matrix(n_size, n_size)
  ...
  real*8, allocatable :: eigenvalues(:), transform(:,:)
  real*8 :: ev_sqrt
  ...
  call diagonalize_auxmat_lapack(n_size, matrix, safe_minimum, &
...
!  matrix = 0.d0
  ...
  call dgemm('N', 'T', n_size, n_size, n_nonsingular, &
  &          1.0d0, transform(:,1:n_nonsingular), n_size, &
  &                 transform(:,1:n_nonsingular), n_size, &
  &           0.d0, matrix, n_size)

  deallocate(eigenvalues, transform)

end subroutine power_genmat_lapack
!******
