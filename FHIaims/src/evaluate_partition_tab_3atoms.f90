!****s* FHI-aims/evaluate_partition_tab_3atoms
!  NAME
!   evaluate_partition_tab_3atoms
!  SYNOPSIS
...
!  ARGUMENTS
      ...
      real*8 partition_tab(n_atoms*(n_atoms+1)/2)
      ...
      aux_dens = &
        ...
        ( i_r(i_atom), partition_rho_spl(1,1,species(i_atom)), &
         ...
         select case(partition_type_temp)
          ...
          case(1)
             wt_dens = &
                  aux_dens / (dist_tab(i_atom)**2.0d0) + par_offset
          case(2)
             wt_dens = &
      ...
      do i_atom_1 = 1, n_atoms, 1
        ...
        if (i_atom_1 .ne. i_atom ) then
           ...
           select case(partition_type_temp)
            ...
            case(1)
               aux_partition_norm = &
                   aux_dens / (dist_tab(i_atom_1)**2.0d0) + par_offset
            case(2)
               aux_partition_norm = &
        ...
        do i_atom_2 = 1, i_atom_1, 1
         partition_norm = partition_norm_start
          ...
          if (i_atom_2.ne.i_atom.and.i_atom_2.ne.i_atom_1) then
           ...
           aux_dens =   val_spline &
            ( i_r(i_atom_2), partition_rho_spl(1,1,species(i_atom_2)), &
           ...
           select case(partition_type_temp)
            ...
            case(1)
               aux_partition_norm_2 = &
                   aux_dens / (dist_tab(i_atom_2)**2.0d0) + par_offset
            case(2)
               aux_partition_norm_2 = &
                   aux_dens / abs(dist_tab(i_atom_2))
            case(3)
               aux_partition_norm_2 = &
                    aux_dens
            case(4)
               aux_partition_norm_2 = &
                  1.0/ ( 1+ exp( (dist_tab(i_atom_2)- &
                        hartree_partition_parameters(1))/ &
                        hartree_partition_parameters(2) ) )
            case(6)
               aux_partition_norm_2 = &
...
!      end loop over other atoms, i_atom_2
        enddo

!     end loop over other atoms, i_atom_1
      enddo

      end subroutine evaluate_partition_tab_3atoms
