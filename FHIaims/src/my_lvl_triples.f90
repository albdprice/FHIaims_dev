...
module my_lvl_triples
  ...
  use timing
  use localorb_io
  use pbc_lists, only: coords_center, species_center, center_to_atom, &
       basis_cell_to_all, Cbasis_to_center, Cbasis_to_basis 
...
!!!!$omp threadprivate(max_prods_L, max_LL,  max_n_prods_fnLsp, Lsp2n_prods_fnLsp,   &
!!!!$omp               Lsp2prods_fn, max_n_prodsfn, n_prodsfn, fnLpd_to_Lbs,         &
!!!!$omp               fnLpd_to_Lbb, fnLpd_to_fnLbs, fnLpd_to_fnLbb,                 &
!!!!$omp               bbfn_to_fnLbb, bsfn_to_fnLbs, ffk_bs, ffr_bs, vvr_bb,         &
!!!!$omp               sbt_times, time_init, time_all)
!!!
!!!! allocated in a parallel region must be:
!!!! Lsp2n_prods_fnLsp, Lsp2prods_fn, n_prodsfn, radius_pd, fnLpd_to_Lbs,
!!!! fnLpd_to_Lbb, bbfn_to_fnLbb, bsfn_to_fnLbs, ffk_bs, ffr_bs, vvr_bb
contains
  ...
  subroutine my_initialize_lvl_triples(ovlp_type)
...
!$    myThreadId = omp_get_thread_num() + 1
    ...
    allocate(bbfn_to_fnLbb(n_basbas_fns), stat=istat, errmsg=errorMessage)
    if (istat .ne. 0) then
      print *,"error when allocating bbfn_to_fnLbb: "//errorMessage
    endif
    ...
    allocate(rr(N), ffr_bb(N), vvr_bb(N, n_basbas_fns), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating rr, ffr_bb, vvr_bb: "//errorMessage
    endif
    ...
    allocate(bsfn_to_fnLbs(n_basis_fns), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating bsfn_to_fnLbs: "//errorMessage
    endif
    ...
    allocate(ffr_bs(sbtgrid_N, n_basis_fns), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating ffr_bss: "//errorMessage
    endif

    allocate(ffk_bs(sbtgrid_N, n_basis_fns), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating ffk_bs: "//errorMessage
    endif
    ...
    allocate(n_prodsfn(n_species), Lsp2n_prods_fnLsp(0:max_prods_L, n_species), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating n_prodsfn, Lsp2n_prods_fnLsp: "//errorMessage
    endif
    ! count pds
    Lsp2n_prods_fnLsp = 0
    do i_species = 1, n_species
    ...
    ! enumerate pds
    allocate(Lsp2prods_fn(max_n_prods_fnLsp, 0:max_prods_L, n_species), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating Lsp2prods_fn: "//errorMessage
    endif

    
    allocate(radius_pd(max_n_prodsfn, n_species), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating radius_pd: "//errorMessage
    endif

    allocate(fnLpd_to_Lbs(max_n_prods_fnLsp, 0:max_prods_L, n_species), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating fnLpd_to_Lbs: "//errorMessage
    endif

    allocate(fnLpd_to_fnLbs(max_n_prods_fnLsp, 0:max_prods_L, n_species), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating fnLpd_to_fnLbs: "//errorMessage
    endif


    allocate(fnLpd_to_Lbb(max_n_prods_fnLsp, 0:max_prods_L, n_species), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating fnLpd_to_Lbb: "//errorMessage
    endif

    allocate(fnLpd_to_fnLbb(max_n_prods_fnLsp, 0:max_prods_L, n_species), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating fnLpd_to_fnLbb: "//errorMessage
    endif
    allocate(Lsp2n_prods_fnLsp_uptonow(0:max_prods_L), stat=istat)
    if (istat .ne. 0) then
      print *,"error when allocating Lsp2n_prods_fnLsp_uptonow: "//errorMessage
    endif
    do i_species = 1, n_species
  ...
  end subroutine my_initialize_lvl_triples
  !******
  !----------------------------------------------------------------------------
  ...
  subroutine my_cleanup_lvl_triples()

    !  PURPOSE
    !    Clean up module data
    !  USES

    use sbt_overlap, only: my_ffk1_cleanup, output_sbt_timing
  ...
  end subroutine my_cleanup_lvl_triples
  ...
  subroutine my_calculate_lvl_triples(i_species_1, i_species_2, &
  &                                n_Rvec, Rvec, ovlp_3fn, d_ovlp_3fn, calc_deriv)
    ...
    !
    !  USES
    use sbt_overlap, only : my_sbt_twocenter_triples
    implicit none
    ...
    real*8              :: tr_ovlp(max_n_basis_sp, max_n_basbas_sp, max_n_basis_sp, n_Rvec, 2)
    real*8              :: d_tr_ovlp(max_n_basis_sp, max_n_basbas_sp, max_n_basis_sp, 3, n_Rvec, 2)
 ...
 !   real*8, allocatable :: tr_ovlp(:,:,:,:), d_tr_ovlp(:,:,:,:,:)
    ...
    integer :: myThreadId, omp_get_thread_num
    logical :: omp_in_parallel

!!!!$omp parallel private(info)    &
...
! this could be nested to maybe achieve even better scaling???

!$omp parallel do if((.not.omp_in_parallel()))   &
!$omp default(none)  &
!$omp private(i_side, i_species_pd, i_species_ot, use_Rvec,  &
!$omp         sbt_times, i_basis_pd, i_basis_ot, &
!$omp          i_basis_1, i_basis_2, i_basbas_pd, myThreadId)   &
!$omp shared(i_species_1, i_species_2, Rvec, sbtgrid_N, sbtgrid_lnr0, &
!$omp        sbtgrid_lnk0, sbtgrid_lnrange, n_Rvec, n_prodsfn, radius_pd, &
!$omp        max_prods_L, max_n_prods_fnLsp, Lsp2n_prods_fnLsp,   &
!$omp        Lsp2prods_fn, n_basis_fns, ffk_bs, outer_radius, max_basis_L, &
!$omp         max_n_basis_fnLsp, Lsp2n_basis_fnLsp, Lsp2basis_fn, &
!$omp         max_basbas_L, max_n_basbas_fnLsp, Lsp2n_basbas_fnLsp, &
!$omp         fnLpd_to_Lbs,  fnLpd_to_fnLbs, fnLpd_to_Lbb, &
!$omp         fnLpd_to_fnLbb, ffr_bs, &
!$omp         n_basbas_fns, vvr_bb, Lsp2basbas_fn, Lsp2basis_sp, Lsp2basbas_sp, &
!$omp         calc_deriv, ovlp_3fn, d_ovlp_3fn, sp2n_basbas_sp, sp2n_basis_sp,  &
!$omp         tr_ovlp, d_tr_ovlp, n_species) &
!$omp schedule(static)
    do i_side = 1, 2
       if (i_side == 1) then
...
!$ myThreadId = omp_get_thread_num() + 1
       ...
       & tr_ovlp(:,:,:,:,i_side), d_tr_ovlp(:,:,:,:,:,i_side), calc_deriv, sbt_times,(i_species_pd-1)*n_species+i_species_ot)
   enddo
!$omp end parallel do
       ...
       do i_basis_pd = 1, sp2n_basis_sp(i_species_pd)
          do i_basis_ot = 1, sp2n_basis_sp(i_species_ot)
             ...
             do i_basbas_pd = 1, sp2n_basbas_sp(i_species_pd)
                ...
                & = tr_ovlp(i_basis_pd, i_basbas_pd, i_basis_ot, :, i_side)
                if(calc_deriv) then
                   if (i_side == 1) then
!                      d_ovlp_3fn(i_basis_1, i_basis_2, i_basbas_pd, i_side, :, :) &
!                      & =  d_tr_ovlp(i_basis_pd, i_basbas_pd, i_basis_ot, :, :, i_side)
                   else
!                      d_ovlp_3fn(i_basis_1, i_basis_2, i_basbas_pd, i_side, :, :) &
!                      & = -d_tr_ovlp(i_basis_pd, i_basbas_pd, i_basis_ot, :, :,i_side)
                   endif
...
!    deallocate(tr_ovlp, d_tr_ovlp)
    call perfoff
  end subroutine my_calculate_lvl_triples

!  subroutine my_my_calculate_lvl_triples(max_n_basis_sp_in, max_n_basbas_sp_in, i_species_1, i_species_2, &
!                                      lowerBound, upperBound, n_Rvec, Rvec, ovlp_3fn)
!
!    !  PURPOSE
!    !
!    !    Calcluate the triple-overlap integrals of the basis functions of
...
!    integer, intent(in) :: max_n_basis_sp_in, max_n_basbas_sp_in
!    integer, intent(IN) :: n_Rvec
!    real*8, intent(IN) :: Rvec(3, n_Rvec)
!    real*8, intent(OUT) :: ovlp_3fn(max_n_basis_sp_in, max_n_basis_sp_in, &
!                                    lowerBound:upperBound, 2, n_Rvec)
!
!!    real*8              :: ovlp_3fn_tmp(max_n_basis_sp_in, max_n_basis_sp_in, &
!!                                        max_n_basbas_sp_in, 2, n_Rvec)
!
!    !  INPUTS
!    !    o i_species_1, i_species_2 -- Atomic species numbers
!    !    o n_Rvec -- Number of difference vectors
...
!    !  SOURCE
!
!    logical :: is_onsite, doWork
!    real*8 :: use_Rvec(3, n_Rvec)
!    real*8, allocatable :: tr_ovlp(:,:,:,:)
!!    real*8, allocatable :: tr_ovlp_tmp(:,:,:,:)
!    integer :: i_side
!    integer :: i_species_pd, i_species_ot, i_basis_pd, i_basis_ot
!    integer :: i_basis_1, i_basis_2, i_basbas_pd
!    integer :: info
!    character(*), parameter :: func = 'my_calculate_lvl_triples'
!     integer :: k1
!
!    doWork = .false.
!
!    if (max(1,lowerBound) .le. min(max_n_basbas_sp_in, upperBound)) doWork=.true.
!    if ((.not.(doWork))) then
!
!      return
!    endif
!    allocate(tr_ovlp(max_n_basis_sp_in, lowerBound:upperBound, max_n_basis_sp_in, n_Rvec), &
!    &        stat=info)
!    call check_allocation(info, 'tr_ovlp')
!
!!    allocate(tr_ovlp_tmp(max_n_basis_sp_in, max_n_basbas_sp_in, max_n_basis_sp_in, n_Rvec), &
!!    &        stat=info)
!!    call check_allocation(info, 'tr_ovlp_tmp')
!
!
!    ovlp_3fn = 0.d0
!!    ovlp_3fn_tmp = 0.d0
...
!       & Lsp2basbas_sp(:,:, i_species_pd), &
!       & Lsp2basis_sp(:,:, i_species_ot), &
!       & tr_ovlp, sbt_times, lowerBound, upperBound, max_n_basbas_sp_in,   &
!         max_n_basis_sp_in)
!
!       do i_basis_pd = 1, sp2n_basis_sp(i_species_pd)
!          do i_basis_ot = 1, sp2n_basis_sp(i_species_ot)
...
!             end if
!             do i_basbas_pd = 1, sp2n_basbas_sp(i_species_pd)
!               if (i_basbas_pd .ge. lowerBound .and. i_basbas_pd .le. upperBound) then
!                ovlp_3fn(i_basis_1, i_basis_2, i_basbas_pd, i_side, :) &
...
!
!    ! copy remove later
!!    do k1=1,max_n_basbas_sp_in
!!      if (k1 .ge. lowerBound .and. k1 .le. upperBound) then
!!        ovlp_3fn(:,:,k1,:,:) =  ovlp_3fn_tmp(:,:,k1,:,:)
!!      endif
!!    enddo
!
!    deallocate(tr_ovlp)
!!    deallocate(tr_ovlp_tmp)
!
!  end subroutine my_my_calculate_lvl_triples
  !******
  !----------------------------------------------------------------------------
...
!  subroutine my_calculate_lvl_triples_matrix(i_atom_1, i_atom_2, n_row, n_col, &
...
!
!    call my_calculate_lvl_triples(i_species_1, i_species_2, 1, Rvec, ovlp, dummy, .false.)
!
!    do i_row = 1, n_row
...
!          i_side = 2
!       else
!          call aims_stop(func, 'invalid i_atom_pd')
...
!
!  end subroutine my_calculate_lvl_triples_matrix
!  !******
!  !----------------------------------------------------------------------------
end module my_lvl_triples
!******
