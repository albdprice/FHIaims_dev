...
!     counters
      ...
      complex*16, dimension(n_basis,n_basis) :: aux_eri_kqkpqp, aux_eri_kqpkpq
...
!     begin work
      ...
      if(myid.eq.0) then
        ...
        write(use_unit,'(2X,A)') &
              "Parallel in term of k-grid only  ... "
        write(use_unit,*)" -----------------------------------------------------------------------"
      endif
        ...
        if (cpt2_para(1,i_task_real) .ne. 0) then
            ...
            do a_state = n_low_state, n_homo_k(i_k_point, 1), 1
                do b_state = n_low_state, n_homo_k(i_kp_point,1), 1
                    call zgemm('T', 'N', n_basis, n_basis, &
                                n_basbas, (1.0d0,0.0d0), &
                                lvl_tricoeff_kq(:,:,a_state,1), n_basbas, &
                                lvl_tricoeff_kpqp(:,:,b_state,1), n_basbas, (0.d0,0.d0), &
                                aux_eri_kqkpqp, n_basis &
                               )
                    call zgemm('T', 'N', n_basis, n_basis, &
                                n_basbas, (1.0d0,0.0d0), &
                                lvl_tricoeff_kqp(:,:,a_state,1), n_basbas, &
                                lvl_tricoeff_kpq(:,:,b_state,1), n_basbas, (0.d0,0.d0), &
                                aux_eri_kqpkpq, n_basis &
                               )
                    do n_state = n_lumo_k(i_q_point,1), n_states_k(i_q_point), 1
                        do m_state = n_lumo_k(i_qp_point,1), n_states_k(i_qp_point),1
                            ...
                            !enddo  ! i_prodbas_1
                            E_mp2_a = aux_eri_kqkpqp(n_state,m_state)
                            E_mp2_b = aux_eri_kqpkpq(m_state,n_state)
