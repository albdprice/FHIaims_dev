...
!          endif
      ...
      do i_task = n_k_points + 1, n_tasks, 1
        ...
        if (myid .eq. i_tasks(1)) then
            call mpi_send(n_k_points_task, 1, MPI_INTEGER, i_task, 0, mpi_comm_global, info)
            call send_complex_vector(lvl_tricoeff_recip1, size(lvl_tricoeff_recip1),i_task)
            call send_complex_vector(coulomb_matr_recip, size(coulomb_matr_recip),i_task)
        endif
        if (myid .eq. i_task) then
            call mpi_recv(n_k_points_task, 1, MPI_INTEGER, i_tasks(1), 0, mpi_comm_global, status, info)
            if (.not. allocated(lvl_tricoeff_recip1)) then
                deallocate(lvl_tricoeff_recip1)
                allocate(lvl_tricoeff_recip1(max_n_basbas_sp,n_basis,n_basis,n_k_points_task),stat=info)
            endif
            if (.not. allocated(coulomb_matr_recip)) then
                deallocate(lvl_tricoeff_recip1)
                allocate(lvl_tricoeff_recip1(max_n_basbas_sp,n_basis,n_basis,n_k_points_task),stat=info)
            endif
      ...
      do i_p_index = pt2_finish+1, cpt2_max_per_thread, 1
        ...
        do i_task = n_tasks, 1, -1
            ...
            i_duplicate = (i_task - 1)/n_tasks + 1
            ...
            ! For the tasks in "A1" block, the communication
            ! happens in the last second and "A1" blocks.
            !===============================================================

            i_tasks(1)  = mod(cpt2_para(1,i_task), n_tasks) + i_duplicate * n_k_points
            i_tasks(2)  = mod(cpt2_para(2,i_task), n_tasks) + i_duplicate * n_k_points
            i_tasks(3)  = mod(cpt2_para(3,i_task), n_tasks) + i_duplicate * n_k_points
            i_tasks(4)  = mod(cpt2_para(4,i_task), n_tasks) + i_duplicate * n_k_points
            i_tasks(5)  = mod(cpt2_para(5,i_task), n_tasks) + i_duplicate * n_k_points
            i_tasks(6)  = mod(cpt2_para(6,i_task), n_tasks) + i_duplicate * n_k_points

            if (i_tasks(1) .gt. n_tasks) i_tasks(1) = i_tasks(1) - n_k_points
            if (i_tasks(2) .gt. n_tasks) i_tasks(2) = i_tasks(2) - n_k_points
            if (i_tasks(3) .gt. n_tasks) i_tasks(3) = i_tasks(3) - n_k_points
            if (i_tasks(4) .gt. n_tasks) i_tasks(4) = i_tasks(4) - n_k_points
            if (i_tasks(5) .gt. n_tasks) i_tasks(5) = i_tasks(5) - n_k_points
            if (i_tasks(6) .gt. n_tasks) i_tasks(6) = i_tasks(6) - n_k_points
            !===============================================================
        ...
        if (cpt2_para(1,i_task_real) .ne. 0) then
            ...
            ! multiply the 3-center overlap matrix with the squre root
            ! of the coulomb matrix
            call get_v_multi_ovlp3fn_complex(coulomb_tmp_qpk, lvl_tricoeff_sum_all,1)
            ...
            ! multiply the 3-center overlap matrix with the squre root
            ! of the coulomb matrix
            call get_v_multi_ovlp3fn_complex(coulomb_tmp_qpk, lvl_tricoeff_sum_all,2)
