...
!****s* FHI-aims/power_auxmat_scalapack_real
!  NAME
!   power_auxmat_scalapack_cpt2
!  SYNOPSIS

      subroutine power_auxmat_scalapack_real_cpt2 &
...
!  ARGUMENTS
      ...
      real*8, intent(INOUT) :: auxmat(lbb_row:ubb_row, lbb_col:ubb_col)
...
!  SOURCE
      ...
      real*8, dimension(:,:), allocatable ::  transform, auxmat2
...
!  start to work
      ...
      call mpi_comm_size(comm_blacs_row_cpt2,nprow_blacs,mpierr)
      call mpi_comm_rank(comm_blacs_row_cpt2,myprow_blacs,mpierr)
      call mpi_comm_size(comm_blacs_col_cpt2,npcol_blacs,mpierr)
      call mpi_comm_rank(comm_blacs_col_cpt2,mypcol_blacs,mpierr)
...
!      call mpi_allreduce(checkloc, checkglob,1,MPI_DOUBLE_PRECISION,MPI_SUM,comm_blacs_cpt2,mpierr)
!     XR, 2016.10.14
...
!     if((checkglob.gt.5e-11).and.(myid_bl.eq.0)) write (use_unit,'(2X,2A,ES14.4)') &
      !if((checkglob.gt.5e-9).and.(myid_bl_cpt2.eq.0)) write (use_unit,'(2X,2A,ES14.4)') &
      !     'WARNING: input matrix in power_auxmat_scalapack_real does not seem to be self-adjoint!',&
      !     ' Checksum: ',checkglob
...
! diagonalizing the matrix
      ...
      call solve_evp_real_2stage_2013(n_bb, n_bb, auxmat2, &
           ...
           ubound(transform,1), bb_bl_row, comm_blacs_row_cpt2, &
           comm_blacs_col_cpt2, comm_blacs_cpt2)
...
!  deallocate arrays
     ...
     end subroutine power_auxmat_scalapack_real_cpt2
