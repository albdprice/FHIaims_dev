...
!  PURPOSE
!  Subroutine evaluate_cpt2_energy_kspace evaluates the opposite-spin correlation
!  energy at the PT2 level.
!
...
! USES
      ...
      use gw_para, only: n_full_freq, omega_full_grid, womega_full
...
! o  omega_full_grid(n_freq) -- real array
...
!  local variables
      ...
      ! for lapalace quadrature approximation of OS-PT2
      integer :: i_freq
      ...
      complex*16, allocatable :: lvl_tricoeff_kpqp(:,:,:,:) ! Full LVL triple coefficient, one basis index is transformed to
                                                            ! (occupied) molecular orbital index 
      complex*16, dimension(:,:), allocatable :: polar_kq_freq
      complex*16, dimension(:,:), allocatable :: polar_kpqp_freq
...
!     counters
      ...
      ! for laplace quadrature approximation
      real*8  :: tmp_fac, tmp_omega, tmp_womega
...
!          endif
      ...
      ! For laplace quadrature approximation
      allocate(polar_kq_freq(n_basbas, n_basbas),stat=info)
      call check_allocation(info, 'polar_kq_freq                   ')
      allocate(polar_kpqp_freq(n_basbas, n_basbas),stat=info)
      call check_allocation(info, 'polar_kpqp_freq                 ')
      ...
      do i_p_index = pt2_finish+1, cpt2_max_per_thread, 1
        ...
        if (cpt2_para(1,i_task_real) .ne. 0) then
            ...
            ! Now do OS-PT2 calculation
            pt2_term    = 0.0d0
            pt2_term_os = 0.0d0
            pt2_term_ss = 0.0d0
            !if (i_p_index .eq. 1 .and. myid.eq.0) then
            !    write(use_unit,*) omega_full_grid
            !    write(use_unit,*) womega_full
            !endif
            do i_freq = 1, n_full_freq, 1
                tmp_womega      = womega_full(i_freq)**(0.5d0)
                tmp_omega       = omega_full_grid(i_freq)
                polar_kq_freq   = (0.0d0,0.0d0)
                polar_kpqp_freq = (0.0d0,0.0d0)
                ! prepare \bar{M}(k,q) and \bar{N}(k',q')
                do a_state = n_low_state, n_homo_k(i_k_point, 1), 1
                  do n_state = n_lumo_k(i_q_point,1), n_states_k(i_q_point), 1
                      tmp_fac = tmp_womega * exp( - tmp_omega * &
                          (KS_eigenvalue(n_state,1,i_q_point) &
                          - KS_eigenvalue(a_state,1,i_k_point)) &
                          )
                      if (i_freq .eq. 15 .and. i_p_index.eq.1 .and. myid.eq.0) then
                          write(use_unit,'(6I5,3F16.8)')  a_state, n_state, i_k_point, i_q_point, &
                              n_homo_k(i_k_point,1), n_states_k(i_q_point), &
                              KS_eigenvalue(a_state,1,i_k_point), &
                              KS_eigenvalue(n_state,1,i_q_point), &
                              tmp_fac
                      endif
                      do i_prodbas_1 = 1, n_basbas, 1
                        do i_prodbas_2 = 1, n_basbas, 1
                          polar_kq_freq(i_prodbas_2,i_prodbas_1) = &
                              polar_kq_freq(i_prodbas_2,i_prodbas_1) + tmp_fac * &
                              conjg(lvl_tricoeff_kq(i_prodbas_2, n_state,a_state,1)) * &
                ...
                do b_state = n_low_state, n_homo_k(i_kp_point, 1), 1
                  do m_state = n_lumo_k(i_qp_point,1), n_states_k(i_qp_point), 1
                      ...
                      do i_prodbas_1 = 1, n_basbas, 1
                        do i_prodbas_2 = 1, n_basbas, 1
                          polar_kpqp_freq(i_prodbas_2,i_prodbas_1) = &
                              polar_kpqp_freq(i_prodbas_2,i_prodbas_1) + tmp_fac * &
                              conjg(lvl_tricoeff_kpqp(i_prodbas_2, m_state,b_state,1)) * &
                ...
                ! Now calculate OS-PT2 contribution on i_freq
                do i_prodbas_1 = 1, n_basbas, 1
                  do i_prodbas_2 = 1, n_basbas, 1
                      tmp_fac = abs(&
                          polar_kq_freq(i_prodbas_2,i_prodbas_1) &
                      ...
                      !if (i_prodbas_1 .ne. i_prodbas_2) tmp_fac = 2.0d0 * tmp_fac
                      pt2_term_os = pt2_term_os + tmp_fac
                  enddo
                enddo
                if (i_p_index.eq.1 .and. myid.eq.0) then
                    write(use_unit,'(I5,4F16.8)') i_freq, womega_full(i_freq), tmp_womega, tmp_omega,  pt2_term_os
                endif
            enddo !i_freq
