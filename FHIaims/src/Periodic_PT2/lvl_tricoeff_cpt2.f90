module lvl_tricoeff_cpt2
  ...
  real(kind=8),allocatable, dimension(:,:,:,:) :: lvl_tricoeff_cell, bcell_fac
  real(kind=8), allocatable :: trico_cell(:,:,:,:)
  complex(kind=8), allocatable :: trico_k(:,:,:,:,:)
  ...
  logical:: save_cell
  complex(kind=8), allocatable:: k_phase_save(:,:)    
contains
  ...
  subroutine my_fill_coeff_3fn(i_atom_1, i_atom_2, i_species_1, i_species_2, i_cell_in, &
    ...
    real(kind=8), intent(in)   :: coords(3,n_atoms), lattice_vector(3,sizeLatticeVector)
    real(kind=8)              :: Dvec(3), Cvec(3), Rvec(3), dummy(1,1,1,1,1,1)
    
    integer             :: i_cell, i_cell_1, i_cell_2, i_cell_3
    
    real(kind=8), intent(out) :: coeff_3fn(max_n_basis_sp,max_n_basis_sp,max_n_basbas_sp,2)
    call perfon('myfill')
  ...
  subroutine cpt2_init_lvl_tricoeff_recip(n_cells, n_cells_task, n_k_points_special, n_k_points_special_task, n_ks_points_special_task,&
       k_phase_special)
    ...
    complex(kind=8), intent(IN)  :: k_phase_special(n_cells,n_k_points_special)
    ...
    real(kind=8), allocatable :: coeff_3fn(:,:,:,:,:)
...
!    save_cell=.true.
    ...
    n_c3fn_task=n_c3fn/n_tasks_bl_cpt2
    rest=n_c3fn-n_tasks_bl_cpt2*n_c3fn_task
    if(myid_bl_cpt2.lt.rest) then
       n_c3fn_task=n_c3fn_task+1
       myoffset=myid_bl_cpt2*n_c3fn_task
    else
       myoffset=myid_bl_cpt2*n_c3fn_task+rest
    end if
    ...
    if(i_task.lt.rest) then
       per_task=n_c3fn/n_tasks_bl_cpt2+1
    else
       per_task=n_c3fn/n_tasks_bl_cpt2
    end if
    i_cell=0
    ...
    ap_pos(3,n_atoms*n_atoms-1)=n_tasks_bl_cpt2-1
...
!$  numThreads = omp_get_max_threads()
    ...
    do i_c3fn=0,n_c3fn_task-1
       ...
       find_cell(1,i_cell,i_atom_1,i_atom_2)=myid_bl_cpt2
       ...
       if (i_cell.le.n_cells) then
          ...
          if(save_cell) then          
             ...
             do i_ks_point_local = 1,n_ks_points_special_task 
                i_k_point=(i_ks_point_local-1)*n_tasks_kq_cpt2+myid_kq_cpt2+1
    ...
    call mpi_allreduce(MPI_IN_PLACE,find_cell,size(find_cell),MPI_INTEGER,MPI_SUM,comm_blacs_cpt2,mpierr)
...
!     n_bcell_task=n_bcell/n_tasks_col_cpt2
!     brest=n_bcell-n_bcell_task*n_tasks_col_cpt2
!     if(myid_col_cpt2.lt.brest) then
!        n_bcell_task=n_bcell_task+1
!        bmyoffset=myid_col_cpt2*n_bcell_task
!     else
!        bmyoffset=myid_col_cpt2*n_bcell_task+brest
!     end if
...
!     n_b2=ub_b2-lb_b2+1
       ...
       n_b_task=n_basis/n_tasks_col_cpt2
       brest=n_basis-n_b_task*n_tasks_col_cpt2
       if(myid_col_cpt2.lt.brest) then
          n_b_task=n_b_task+1
          myboffset=myid_col_cpt2*n_b_task
       else
          myboffset=myid_col_cpt2*n_b_task+brest
       end if
       lb_b2=myboffset+1
       ...
       call mpi_allreduce(MPI_IN_PLACE,maxfence,1,MPI_INTEGER,MPI_MAX,comm_blacs_cpt2,mpierr)       
       nbytes=max_n_basis_sp*max_n_basis_sp*max_n_basbas_sp*n_c3fn_task
       call mpi_win_create(trico_cell,nbytes,8,MPI_INFO_NULL,comm_blacs_cpt2,win_tri,mpierr)
       ...
       do i_atom_1=basbas_atom(lbb_row),basbas_atom(ubb_row)    
          ...
          nbytes=max_n_basis_sp*max_n_basis_sp*(ubi-lbi+1)
       ...
       allocate(sizearr(0:n_tasks_col_cpt2-1))
       allocate(disparr(0:n_tasks_col_cpt2-1))

       call mpi_allgather(mycount,1,MPI_INTEGER,&
            sizearr,1,MPI_INTEGER,&
            comm_blacs_col_cpt2,mpierr)
       disparr=0
       do i_task=1,n_tasks_col_cpt2-1
  ...
  end subroutine cpt2_init_lvl_tricoeff_recip

  
  subroutine cpt2_cleanup_lvl_tricoeff_recip
    if(kblacs_member) then
       if (save_cell) then
  ...
  end subroutine cpt2_cleanup_lvl_tricoeff_recip


  subroutine cpt2_get_lvl_tricoeff_recip(n_cells, n_k_points_special, n_k_points_special_task, n_ks_points_special_task,&
    ...
    integer, intent(IN) :: n_k_points_special_task, n_ks_points_special_task
    complex(kind=8), intent(IN)  :: k_phase_special(n_cells,n_k_points_special)
    real(kind=8), dimension(n_basis,n_states,n_spin,n_k_points_special_task), intent(IN) :: KS_eigenvector
    complex(kind=8), dimension(n_basis,n_states,n_spin,n_k_points_special_task), intent(IN) :: KS_eigenvector_complex
    complex(kind=8), intent(OUT) :: lvl_tricoeff_mod_r(lbb_row:,:,:,:,:)
    ...
    complex(kind=8), allocatable, dimension(:,:,:,:) :: lvl_tricoeff_tmp
    complex(kind=8), allocatable, dimension(:,:,:,:) :: trico_tmp
    complex(kind=8), allocatable, dimension(:,:,:) :: trico_tmp2
    ...
    character*150 :: info_str
    character(*), parameter :: func = 'get_lvl_tricoeff_recip'
    complex(kind=8), dimension(n_basis,n_states,n_spin) :: KS_eigenvector_k
    real(kind=8), dimension(:,:,:), allocatable :: KS_eigenvector_r
...
!             call perfon('nlvlaggr')   
             ...
             call mpi_gatherv(trico_tmp2,sizearr(myid_col_cpt2),MPI_DOUBLE_COMPLEX,&
...
!             call perfoff
          ...
          nbytes=max_n_basis_sp*max_n_basis_sp*max_n_basbas_sp*n_ks_points_special_task*n_ap
          call mpi_win_create(trico_k,nbytes,16,MPI_INFO_NULL,comm_blacs_cpt2,win_tri,mpierr)
          call mpi_win_fence(0,win_tri,mpierr)
          ...
          nbytes=max_n_basis_sp*max_n_basis_sp*max_n_basbas_sp*n_ks_points_special_task
          offset=0
          ppos=myid_bl_cpt2
          n_exc=(n_cells+1)/(n_c3fn/n_tasks_bl_cpt2)+1
          i_cell=n_c3fn_task+myoffset-ub_ap*(n_cells+1)

          do i_exc=1,n_exc
             if (i_collect.and.(i_cell.lt.(n_cells+1))) then
                ppos=ppos+1
                i_cell=i_cell+n_c3fn/n_tasks_bl_cpt2
...
!          deallocate(trico_tmp)
    ...
    if (mod(myid+n_tasks-1,n_tasks).lt.n_k_points_special) then
       nbytes=n_basis*n_states*n_spin*n_k_points_special_task*8
    else
       nbytes=0 
    end if
    ...
    if(real_eigenvectors) then
       ...
       if (myid_bl_cpt2.eq.0) allocate(KS_eigenvector_r(n_basis,n_states,n_spin))  
    else
       nbytes=nbytes*2
    ...
    do i_ks_point_local = 1, n_k_points_special/n_tasks_kq_cpt2+1
       i_k_point=(i_ks_point_local-1)*n_tasks_kq_cpt2+myid_kq_cpt2+1
       if ((i_k_point.gt.n_k_points_special).or.(.not.kblacs_member)) then
          call mpi_win_fence(0,win_ev,mpierr)
          cycle
       end if
       ...
       if (myid_bl_cpt2.eq.0) then
          ...
          offset=(i_k_point_local-1)*n_basis*n_states*n_spin
          if(real_eigenvectors) then
       ...
       call mpi_win_fence(0,win_ev,mpierr)
       if (real_eigenvectors.and.(myid_bl_cpt2.eq.0)) KS_eigenvector_k=KS_eigenvector_r

       if (.not.kblacs_member) cycle

       if(myid_col_cpt2.eq.0) then
          call mpi_bcast(KS_eigenvector_k, n_basis*n_states*n_spin, MPI_DOUBLE_COMPLEX, 0, &
               comm_blacs_row_cpt2, mpierr)
!          call perfon('lvlgemm')
          do i_spin = 1 , n_spin
             ! new count
...
!          call perfoff
    ...
    call perfoff
  end subroutine cpt2_get_lvl_tricoeff_recip



 subroutine cpt2_get_single_lvl_tricoeff(n_cells, &
       k_phase_special, KS_eigenvector_complex, lvl_tricoeff_mod_r)

    !  PURPOSE
    !
    !    Compute the (myid_row_cpt2 parallel) LVL triple expansion coefficients in real space (i.e., for a set
    !    of Bravais vectors which separate the two unit cells where the basis pairs
    ...
    !    and broadcasts to all myid_col_cpt2. 
    ...
    integer:: n_cells
    complex(kind=8), intent(IN)  :: k_phase_special(n_cells)
    complex(kind=8), dimension(n_basis,n_states,n_spin), intent(IN) :: KS_eigenvector_complex
    complex(kind=8), dimension(lbb_row:ubb_row,max_n_basis_sp,n_states,n_spin), &
...
!    call perfoff
  ...
  end subroutine cpt2_get_single_lvl_tricoeff

 subroutine cpt2_get_single_lvl_tricoeff_0(n_cells, &
       k_phase_special, KS_eigenvector_complex, lvl_tricoeff_mod_r)
    ...
    !    The result is only stored on myid_col_cpt2=0.
    ...
    ! Local variabales
    complex(kind=8), allocatable, dimension(:,:,:) :: lvl_tricoeff_tmp, trico_tmp
    integer :: mpierr, i_spin, count, i_cell
    integer :: info
    character*150 :: info_str
    character(*), parameter :: func = 'get_lvl_tricoeff_recip'
...
!    call perfoff
    ...
    if(myid_col_cpt2.eq.0) allocate(trico_tmp(lbb_row:ubb_row,max_n_basis_sp,n_basis),stat=info)
       
!    call perfon('nlvlaggr')   
    
    call mpi_gatherv(lvl_tricoeff_tmp,sizearr(myid_col_cpt2),MPI_DOUBLE_COMPLEX,&
...
!       call perfoff
    ...
    call perfoff
  end subroutine cpt2_get_single_lvl_tricoeff_0


  subroutine cpt2_get_single_lvl_tricoeff_noev(n_cells, &
    ...
    complex(kind=8), intent(IN)  :: k_phase_special(n_cells)
    integer:: n_cells
    complex(kind=8), dimension(lbb_row:ubb_row,max_n_basis_sp,n_states), intent(OUT) :: lvl_tricoeff
...
!    call perfon('nlvlaggr')   
    call mpi_allgatherv(MPI_IN_PLACE,sizearr(myid_col_cpt2),MPI_DOUBLE_COMPLEX,&
         lvl_tricoeff, sizearr, disparr,MPI_DOUBLE_COMPLEX,&
         comm_blacs_col_cpt2,mpierr)
!    call perfoff

    call perfoff
  end subroutine cpt2_get_single_lvl_tricoeff_noev


  subroutine get_lvl_col_from_row(lvl_col, lvl_row, n_spin_loc)

    complex(kind=8), dimension(lbb_col:ubb_col,max_n_basis_sp,n_basis,n_spin_loc):: lvl_col
    complex(kind=8), dimension(lbb_row:ubb_row,max_n_basis_sp,n_basis,n_spin_loc):: lvl_row
    integer:: n_spin_loc
...
call perfon('colfromrow')
    ...
    mirror=myid_row_cpt2 + myid_col_cpt2*n_tasks_row_cpt2
    if (myid_bl_cpt2.eq.0) then
       lvl_col=lvl_row
    elseif(myid_col_cpt2.eq.0) then
       call mpi_send(lvl_row, count, MPI_DOUBLE_COMPLEX, mirror, 333, &
            comm_blacs_cpt2, mpierr)
    elseif(myid_row_cpt2.eq.0) then
       count=n_bb_col*max_n_basis_sp*n_basis*n_spin_loc
       call mpi_recv(lvl_col, count, MPI_DOUBLE_COMPLEX, mirror, 333, &
            comm_blacs_cpt2, status, mpierr)
    end if
    count=n_bb_col*max_n_basis_sp*n_basis*n_spin_loc
    call mpi_bcast(lvl_col, count, MPI_DOUBLE_COMPLEX, 0, comm_blacs_row_cpt2, mpierr)
call perfoff
  end subroutine get_lvl_col_from_row
end module lvl_tricoeff_cpt2
