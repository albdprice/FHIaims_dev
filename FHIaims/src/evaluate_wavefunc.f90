!****s* FHI-aims/evaluate_wavefunc
!  NAME
!    evaluate_wavefunc
!  SYNOPSIS

subroutine evaluate_wavefunc(n_points, points, &
...
&                                     wavefunc_real, wavefunc_cmplx)

  !  PURPOSE
  !
  !     Evaluate a wave function given by coefficients for given grid points.
  !
  !  USES
  ...
  real*8, allocatable :: wave(:,:), compcoeff_real(:,:)
  ...
  integer :: info
  real*8 :: dummy(1)
  character(*), parameter :: func = 'evaluate_wavefunc'
  ...
  &                  use_bwave, .false., wave, .false., dummy)
...
end subroutine evaluate_wavefunc
!******
