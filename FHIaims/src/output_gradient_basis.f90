...
subroutine output_gradient_basis( local_potential_parts)!, hartree_partition_tab )
...
!  ARGUMENTS
  ...
  real*8, dimension(n_full_points,3) :: local_potential_parts
...
!            end if
           ...
           if (myid.eq.0) then
              write(50,'(6(F20.10,1X))') &
                   (grid_coord(i_coord), i_coord=1,3,1), &
                   (local_potential_parts(i_point,i_coord), i_coord=1,3,1)
  ...
  return
end subroutine output_gradient_basis
!----------------------------------------------------------------------
!******	
