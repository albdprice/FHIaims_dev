!****s* FHI-aims/integrate_first_order_rho_p1
!  NAME
!   integrate_first_order_rho_p1
!  SYNOPSIS

subroutine integrate_first_order_rho_p1 ( &
      partition_tab_std, &
      basis_l_max, &
...
!  remove i_q_point, shanghui 2014.12.05 
!
!  USES
...
!  ARGUMENTS
  ...
  real*8,  dimension(3, n_centers_in_sc_DFPT, n_hamiltonian_matrix_size), intent(in) ::  & 
                                                               first_order_density_matrix_sparse
  real*8,  dimension(3, n_centers_in_sc_DFPT, n_full_points), intent(out) ::  first_order_rho 
  !shanghui------------------------------------------------------------------------
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
  ...
  !shanghui debug 
  !( this debug can be used together with update_density_densmat.f90's debug_one_point
...
!        call  prune_density_matrix_sparse(first_order_density_matrix_sparse(i_coord,i_center,:), & 
!                     first_order_density_matrix_con(i_coord,i_center,:), &
!                    n_compute_c, i_basis)  ! here only give upper matrix
!        do i=1,n_compute_c
!        do j=1,n_compute_c
!           if(i>j) first_order_density_matrix_con(i_coord,i_center,(j-1)*n_compute_c+i)= & 
!                   first_order_density_matrix_con(i_coord,i_center,(i-1)*n_compute_c+j)
!        enddo 
!        enddo
!     enddo 
!     enddo
        ...
        do i_index = 1, batches_work(i_my_batch)%size, 1
           ...
           if (partition_tab(i_full_points).gt.0.d0) then
              ...
              i_basis_fns_inv(:,atom_index(1:n_compute_atoms)) = 0
               ...
               !------------shanghui debug fixed point for first_order_rho-----------------           
                if(debug_one_point) then 
                if(myid.eq.0.and.  & 
                   dabs(batches_work(i_my_batch) % points(i_index) % coords(1) & 
                       - 6.091954737224636E-002).lt.1.0d-4.and.& 
                   dabs(batches_work(i_my_batch) % points(i_index) % coords(2) &
                       - 18.8972612785044).lt.1.0d-4.and.&
                   dabs(batches_work(i_my_batch) % points(i_index) % coords(3) &
                       - 18.8972612785044).lt.1.0d-4) then 

                write(use_unit,*) 'i,j,i_basis(i),i_basis(j),DM^0,DM^1, (DM*wave*wave)^1'
                do i=1,n_compute_c
                   do j=1,n_compute_c
                      if(i>j) density_matrix_con((j-1)*n_compute_c+i)= density_matrix_con((i-1)*n_compute_c+j)
                      ...
                      write(use_unit,'(4i5,3f25.16)') i,j,i_basis(i),i_basis(j),  & 
                                 density_matrix_con((j-1)*n_compute_c+i), &
                                 first_order_density_matrix_con(1,1,(j-1)*n_compute_c+i),   &
                                 first_order_density_matrix_con(1,1,(j-1)*n_compute_c+i)   &
                                 * wave(i,i_point) * wave(j,i_point) -     & 
                                 2.0d0*density_matrix_con((j-1)*n_compute_c+i)* &
                                 gradient_basis_wave_npoints(1,1,i_point)*wave(1,i_point)
                      else if(i.eq.1.and.j.ne.1) then 
                      ...
                      write(use_unit,'(4i5,3f25.16)') i,j,i_basis(i),i_basis(j), &  
                                 ...
                                 * wave(i,i_point) * wave(j,i_point) -     & 
                                 density_matrix_con((j-1)*n_compute_c+i)*  &
                                 gradient_basis_wave_npoints(1,1,i_point)*wave(j,i_point)

                      else if(i.ne.1.and.j.eq.1) then 

                      write(use_unit,'(4i5,3f25.16)') i,j,i_basis(i),i_basis(j),density_matrix_con((j-1)*n_compute_c+i), &
                                 first_order_density_matrix_con(1,1,(j-1)*n_compute_c+i),   &
                                 first_order_density_matrix_con(1,1,(j-1)*n_compute_c+i)   &
                                 * wave(i,i_point) * wave(j,i_point) -     & 
                                 density_matrix_con((j-1)*n_compute_c+i)*  &
                                 gradient_basis_wave_npoints(1,1,i_point)*wave(i,i_point)
               ...
               !------------shanghui end debug fixed point for first_order_rho-----------------           
...
!----------------------------shanghui begin first_order_rho---------------------------------------             
       ...
       do i_index = 1, batches_work(i_my_batch)%size, 1
          ...
          if (partition_tab(i_full_points_DM_rho).gt.0.d0) then
                 ...
                 do i_center=1,n_centers_in_sc_DFPT
                    do i_coord=1,3
                 first_order_rho(i_coord,i_center,i_full_points_DM_rho)= & 
                 local_first_order_rho(i_coord,i_center,i_point) 
                    enddo
                 enddo
               ...
               !------------shanghui debug fixed point for first_order_rho-----------------           
                 ...
                 write(use_unit,*) batches(i_my_batch) % points(i_index) % index_atom, & 
                            first_order_rho(1,1,i_full_points_DM_rho)
...
end subroutine integrate_first_order_rho_p1
!******
