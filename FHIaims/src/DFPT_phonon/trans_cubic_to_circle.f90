!****s* FHI-aims/trans_cubic_to_circle
!  NAME
!   trans_cubic_to_circle
!  SYNOPSIS

subroutine trans_cubic_to_circle &
     (partition_tab_std, first_order_rho_cubic, first_order_rho_circle) 
...
!  ARGUMENTS
  ...
  real*8, dimension(3, n_centers_in_sc_DFPT,n_full_points),intent(in) :: & 
          first_order_rho_cubic        ! just inside center cubic cell 

  real*8, dimension(3, n_centers_in_sc_DFPT,n_full_points),intent(out) :: & 
          first_order_rho_circle       ! could outside of center cubic cell 

  !shanghui------------------------------------------------------------------------

!  INPUTS
!  o partition_tab_std -- values of partition functions
!  o first_order_rho_cubic
!
!  OUTPUT
!  o first_order_rho_circle 
!
!  AUTHOR
     ...
     do i_index = 1, batches_work(i_my_batch)%size, 1
        ...
        if (partition_tab(i_full_points).gt.0.d0) then
           ...
           do i_center = 1 , n_centers_in_sc_DFPT 
              ...
              i_cell_delta = cell_diff_sc_DFPT(i_cell,i_cell_current)

              i_center_trans = cell_and_atom_to_center_sc_DFPT(i_cell_delta,  &      
                                 center_in_sc_DFPT_to_atom(i_center) )              


              first_order_rho_circle(1:3,i_center,      i_full_points) = &   ! i_full_points = r + R  
              first_order_rho_cubic(1:3, i_center_trans,i_full_points)       ! i_full_points = r
...
end subroutine trans_cubic_to_circle
!******
