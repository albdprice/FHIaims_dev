!****s* FHI-aims/evaluate_first_order_EDM_supercell_p1
!  NAME
!    evaluate_first_order_EDM_supercell_p1
!  SYNOPSIS

subroutine evaluate_first_order_EDM_supercell_p1(  &
...
!  first_order_energy_density_matrix_sparse
 ...
 !--------------initialize----------------------
  first_order_energy_density_matrix_sparse(1:3,1:n_centers_in_sc_DFPT,1:n_hamiltonian_matrix_size) = 0.0d0
 ...
 if(use_scalapack_DFPT_phonon)then
     ...
     do k_atom = 1,n_atoms, 1
        ...
        !-----(4) calcualte first_order_edm (k_coord,k_atom,n_basis_sc_DFPT,n_basis_sc_DFPT)-----------
        call construct_first_order_edm_supercell_scalapack()

        !-----(6) calcualte first_order_density_matrix_sparse(k_coord,k_centers,n_hamiltonian_matrix_size)---
        call get_first_order_edm_sparse_matrix_from_supercell_scalapack &
             (k_coord, k_atom, first_order_energy_density_matrix_sparse)

     enddo !k_coord 
     enddo !k_atom 

       !-----(7) sync ------------------------------   
        call sync_vector(first_order_energy_density_matrix_sparse, 3*n_centers_in_sc_DFPT*n_hamiltonian_matrix_size)
...
!----------------the first term-------------------------------
  ...
  do j_state=1, n_occ_states
   ...
   ( temp_H(i_state,j_state) - & 
     temp_S(i_state,j_state)*(KS_eigenvalue_supercell(i_state)+KS_eigenvalue_supercell(j_state)) )  
  enddo
  enddo
  ...
  call add_matrix_to_sparse(k_atom, temp_first_order, first_order_energy_density_matrix_sparse(k_coord,:,:)) 

  ! temp_1 = Cui * (temp_H - (Ei+Ej)*temp_S)ij * Cvj----->
  ...
  !      2.0d0*KS_eigenvector_supercell(i_basis,i_state) &
  !     *( temp_H(i_state,j_state) - & 
  !        temp_S(i_state,j_state)*(KS_eigenvalue_supercell(i_state)+KS_eigenvalue_supercell(j_state)) )    &
  !     * KS_eigenvector_supercell(j_basis,j_state) 
  ...
  !enddo
  !enddo
  !call add_matrix_to_sparse(temp_1, first_order_energy_density_matrix_sparse(i_coord,i_center,:)) 
...
!----------------in fact, we only neet U(unocc,occ)=U(a,i) like this:----------
  ...
  do i_state=1, n_occ_states
     KS_eigen_times_occnumber(1:n_basis_sc_DFPT,i_state)=   &
     KS_eigenvector_supercell(1:n_basis_sc_DFPT,i_state)*2.0d0*KS_eigenvalue_supercell(i_state)  
  enddo
...
!       * KS_eigenvector_supercell(j_basis,j_state)*KS_eigenvalue_supercell(i_state)   &  
!       + 2.0d0*KS_eigenvector_supercell(i_basis,j_state) &
!       * first_order_U(j_state,i_state )                  &
!       *  KS_eigenvector_supercell(j_basis,i_state)*KS_eigenvalue_supercell(i_state) 
!     enddo
!     enddo 
!
! enddo
! enddo
!
! call add_matrix_to_sparse(temp_first_order, first_order_energy_density_matrix_sparse(i_coord,i_center,:)) 
!-----------------------end the second term----------------------------
...
end subroutine evaluate_first_order_EDM_supercell_p1
!******
