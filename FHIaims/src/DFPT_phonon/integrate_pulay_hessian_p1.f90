!****s* FHI-aims/DFPT_phonon/integrate_pulay_hessian_p1
!  NAME
!   integrate_pulay_hessian_p1
!  SYNOPSIS

subroutine integrate_pulay_hessian_p1 &
     ( hartree_potential_std, &  
       first_order_rho_std, first_order_potential_std, &  
...
!  ARGUMENTS
  ...
  real*8, target, dimension(3, n_centers_in_sc_DFPT, n_full_points) :: first_order_rho_std
...
! VB - renewed index infrastructure starts here
  ...
  integer i_coord,i_coord_1, i_coord_2,j_coord,i_center,j_center
  integer i_division
  ...
  ! Keep an array of indices for unpacking the hessian matrix into (symmetrical) 3x3 form
  index_hessian = transpose(reshape((/1,2,3, 2,4,5, 3,5,6/), (/3,3/)))
  
  ! begin work
  if(use_batch_permutation > 0) then
    write(info_str,'(2X,A)') "Integrating Pulay-Hessian: batch-based integration with load balancing"
  else
    write(info_str,'(2X,A)') "Integrating Pulay-Hessian: batch-based integration."
  endif
...
!-------For pulay_hessian--------------------------------------
  ...
  allocate(first_order_energy_density_matrix_con(3,n_centers_in_sc_DFPT,n_max_compute_dens*n_max_compute_dens),stat=info)
...
!-------End for pulay_hessian--------------------------------------
  ...
  !  do i_coord_1 = 2,3
  !     do i_coord_2 = 1,i_coord_1-1
  !        index_hessian(i_coord_1, i_coord_2) = 0 !init needed to avoid seg fault 
  !     end do
  !  end do
  ...
  do i_my_batch = 1, n_my_batches_work, 1
    ...
    !-----------shanghui begin test prune_matrix here-------------------------
     ...
     call  prune_density_matrix_sparse(energy_density_matrix_sparse, energy_density_matrix_con, &
         ...
         do i=1,n_compute_c
         do j=1,n_compute_c
            if(i>j) energy_density_matrix_con((j-1)*n_compute_c+i)= energy_density_matrix_con((i-1)*n_compute_c+j)
         enddo
         enddo
    ...
    call  prune_first_order_density_matrix_sparse(first_order_energy_density_matrix_sparse, &
     ...
     if (n_compute_c.gt.0) then
        ...
        do i_index = 1, batches_work(i_my_batch)%size, 1
           ...
           if (partition_tab(i_full_points).gt.0.d0) then
              ...
              do i_coord=1,3,1             
              local_first_order_rho(i_coord,1:n_centers_in_sc_DFPT,i_point)=      &
              first_order_rho_std(i_coord,1:n_centers_in_sc_DFPT,i_full_points)
...
!!!                   .false., n_compute_c, n_max_compute_fns_ham )
        ...
        do i_spin = 1, n_spin, 1
           call  evaluate_pulay_hessian_p1( & 
                 partition,  &
                 n_points,n_compute_c, i_basis,  &
                 wave, gradient_basis_wave_npoints, & 
                 index_hessian, hessian_basis_wave, &
                 H_times_psi(1,1,i_spin), &
                 local_first_order_rho,local_first_order_potential,local_dVxc_drho(i_spin,1:n_points), &
                 H_times_gradient_psi(1,1,1,i_spin),  &
    ...
    if(.not. use_local_index) call sync_vector(pulay_hessian,3*n_centers_in_sc_DFPT*3*n_atoms)
...
!  if(.not. use_local_index) call sync_integrate_hamiltonian( hamiltonian )
  ...
  if(allocated( first_order_energy_density_matrix_con )) deallocate( & 
                first_order_energy_density_matrix_con   )
...
end subroutine integrate_pulay_hessian_p1
!******
