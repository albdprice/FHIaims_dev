!****s* FHI-aims/gaussian_lattice_sum
!  NAME
!    gaussian_lattice_sum
!  SYNOPSIS

subroutine gaussian_lattice_sum(grid, pos, power, exponent, max_L, max_n_LM, &
&                               exclude_zero, n_kvec, kvec, lattice_sum)
  ...
  !
  !   But this procedure is not meant to be used in this limit.
  !
  !  USES
  ...
  !    o power -- Additional power N in |pos+R|^N.
  !               Will be 0. for straight GTOs and -2. for Coulomb interaction
  ...
  character(*), parameter :: func = 'gaussian_lattice_sum'

  if (power < 0.d0 .and. .not. exclude_zero .and. all(abs(pos) < 1d-10)) then
     call aims_stop('Cannot evaluate Gamma point for power<0.d0', func)
  end if
...
!                   write(use_unit,*) L, M, val_l, ylms(i_LM), val_lm
                 do i_kvec = 1, n_kvec
                    subgls(i_LM, i_kvec, 2) = subgls(i_LM, i_kvec, 2) &
                    &                         + phase(i_kvec) * val_lm
...
!                   tmp(M+L+1)
     ...
     lattice_sum = lattice_sum + subgls(:,:, 1)
  end do
  deallocate(ylms)

end subroutine gaussian_lattice_sum
