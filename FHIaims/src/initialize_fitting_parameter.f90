...
!  SYNOPSIS
      ...
      subroutine  initialize_fitting_parameter (n, x, y, npar, par)
!
!  PURPOSE
...
!   Solve the complex linear equation 
     ...
     end subroutine initialize_fitting_parameter
!***************
!****s* FHI-aims/initialize_fitting_parameter
!  NAME
!     initialize_fitting_parameter
!  SYNOPSIS
      ...
      subroutine  initialize_fitting_parameter_re (n, x, y, npar, par)
!
!  PURPOSE
!     this subroutine evaluate the initial fitting parameter using Pade
!     approximation P_N(x) = A_N(x)/B_N(x). Here N is the order of the 
...
!   Solve the complex linear equation 
      call dgesv(npar, 1, coeff_matr, npar, ipiv, par, npar, info)

      if(info.ne.0) then
        write(STDERR,'(2X,A,A)') &
        " Warning! Initialization of the fitting", &
        " parameter has a problem:", &
        " error in solving the linear equation."
        write(STDERR,*) "info=", info
      endif

     end subroutine initialize_fitting_parameter_re
!***************
