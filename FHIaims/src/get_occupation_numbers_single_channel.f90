...
!******	
!-------------------------------------------------------------------------------
!****s* force_occupation/get_occupation_numbers_single_channel
!  NAME
!   get_occupation_numbers_single_channel
!  SYNOPSIS

  subroutine get_occupation_numbers_single_channel &
    ( KS_eigenvalue, n_electrons, t_out, occ_numbers, &
      chemical_potential,current_spin )
  
  !  PURPOSE
  !  Based on a given Kohn-Sham eigenvalue spectrum, subroutine
  !  get_occupation_numbers returns the occupation numbers 
  !  according to one of three available schemes:
  ...
  !
  !  This version takes Kohn-Sham eigenvalues for a single spin channel only,
  !  without knowledge of any other spin channels. The KS_eigenvalue array is
  !  therefore implicitly reshaped inside this subroutine, and any loop over
  !  spins can safely happen outside this subroutine.
  !
  !  USES
  ...
  !
  !  o integer current_spin remembers the current spin channel, but is "only" pulled through
  !    - for one output statement below
  !    - for a corner case (experimental feature only) of fixed occupation numbers 
  !      in periodic boundary conditions - development purposes only.
  !
  !  OUTPUT
  ...
  !  SOURCE
    ...
    call check_norm_periodic_v2(chemical_potential_l, KS_eigenvalue, n_electrons, occ_numbers, diff_l,&
      i_counter,current_spin)
    call check_norm_periodic_v2(chemical_potential_r, KS_eigenvalue, n_electrons, occ_numbers, diff_r,&
      i_counter,current_spin)
    ...
    call zeroin_periodic_v2(chemical_potential_l, chemical_potential_r, diff_l, diff_r,&
    ...
    call check_norm_periodic_v2( chemical_potential, KS_eigenvalue, n_electrons, occ_numbers,&
    ...
    if (t_out) then
      write (info_str, '(2X, A,I1,A,F15.10)')&
        "| Chemical potential (Fermi level) for spin channel ",&
        current_spin, " in eV               : ", &
        chemical_potential * hartree
      call localorb_info(info_str,use_unit,'(A)')
      write (info_str, '(2X, A, E10.4)') &
        "| Error in electron count due to remaining E_F inaccuracy: ",&
        diff_electrons
      call localorb_info(info_str,use_unit,'(A)')
      if (occupation_thr.gt.0.d0) then
    ...
    call check_n_states(n_states, 1, n_k_points, occ_numbers, t_out)
  ...
  end subroutine get_occupation_numbers_single_channel
