!****s* FHI-aims/evaluate_sosex_2w_selfenergy
!  NAME
!   evaluate_sosex_2w_selfenergy
!  SYNOPSIS

      subroutine evaluate_sosex_2w_selfenergy &
...
!  local variables
      ...
      real*8, dimension(:,:), allocatable :: ovlpw1
      real*8, dimension(:,:), allocatable :: ovlpw2
...
!      endif
      ...
      if (.not.allocated(ovlpw1)) then
          allocate(ovlpw1(n_loc_prodbas,n_states))
      endif
      if (.not.allocated(ovlpw2)) then
          allocate(ovlpw2(n_loc_prodbas,n_states))
      endif
      if (.not.allocated(screened_coulomb)) then
          allocate(screened_coulomb(n_basbas,n_loc_prodbas))
      endif
      ...
      screened_coulomb(:,:)=dielec_func(:,:,1)


! square root of the inverted dielectric function
      call power_auxmat_lapack(screened_coulomb, 5.d-1, '')
...
!                 i_state .gt. n_lumo(i_spin) .and. i_state_2.gt.n_lumo(i_spin) ) cycle
              ...
              ovlpw1(:,:)=ovlp_3KS(:,:,i_state,i_spin)
              ovlpw2(:,:)=ovlp_3KS(:,:,i_state_2,i_spin)
              call multi_ovlp3KS_sqrtw(screened_coulomb,ovlpw1) 
              call multi_ovlp3KS_sqrtw(screened_coulomb,ovlpw2) 
              ...
              do i_spin_1 = 1, n_spin, 1
               ...
               if(n_loc_prodbas .gt. 0) then
                 call dgemm('T', 'N', n_unocc(i_spin), n_homo(i_spin), &
                         n_loc_prodbas, 1.0d0, &
                         ovlpw2(:,n_lumo(i_spin):n_states), &
                         n_loc_prodbas, &
                         ovlpw1(:,1:n_homo(i_spin)), &
                         n_loc_prodbas,0.d0, &
                         aux_eri(1,1), n_unocc_max)

                  call dgemm('T', 'N', n_unocc(i_spin), n_homo(i_spin), &
                          n_loc_prodbas, 1.0d0, &
                          ovlpw1(:,n_lumo(i_spin):n_states), &
                          n_loc_prodbas, &
                          ovlpw2(:,1:n_homo(i_spin)), &
                          n_loc_prodbas,0.d0, &
...
!      Ending
      ...
      if (allocated(ovlpw1)) then
        deallocate (ovlpw1)
      endif
      if (allocated(ovlpw2)) then
        deallocate (ovlpw2)
      endif
      ...
      end subroutine evaluate_sosex_2w_selfenergy
