!****s* FHI-aims/evaluate_first_order_U_phonon_reduce_memory
!  NAME
!    evaluate_first_order_U_phonon_reduce_memory
!  SYNOPSIS

subroutine evaluate_first_order_U_phonon_reduce_memory( & 
           first_order_H_complex, first_order_S_complex,  &
...
!  ARGUMENTS
  ...
  complex*16, dimension( n_basis, n_basis),intent(IN) :: first_order_S_complex
  complex*16, dimension( n_basis, n_basis), intent(IN) :: first_order_H_complex
  complex*16, dimension( n_basis, n_basis), intent(OUT) :: first_order_U_complex
...
!  first_order_E 
  ...
  ! shanghui need to change this in the future for n_spin
    ...
    do i_basis=1,n_basis
     ...
     first_order_S_complex(i_basis,1:n_basis)

     temp_eigenvector(i_basis,1:n_basis)=          &
     dconjg(KS_eigenvector_complex(i_basis,1:n_basis,1))
    ...
    first_order_H_complex(1:n_basis,1:n_basis)
...
!----------------------we do not need this term, we calculate here just for debug--------
     first_order_U_complex( i_basis,j_basis)=         &
...
!----------------in fact, we only neet U(unocc,occ)=U(a,i) like this:----------
     first_order_U_complex( j_basis,i_basis)= &
...
end subroutine evaluate_first_order_U_phonon_reduce_memory
!******
