...
!****s* FHI-aims/integrate_free_atom_sum_gradient_phonon_reduce_memory
!  NAME
!   integrate_free_atom_sum_gradient_phonon_reduce_memory
!  SYNOPSIS

subroutine integrate_free_atom_sum_gradient_phonon_reduce_memory &
     (partition_tab_std, i_q_point, j_atom, j_coord, rho_free_gradient,v_free_gradient)
...
!  ARGUMENTS
  ...
  !-------------------shanghui add here------------------------------------
  integer, intent(in) ::  i_q_point, j_atom, j_coord
  real*8, target, dimension(n_full_points)        :: partition_tab_std
  complex*16, target, dimension(n_full_points) :: rho_free_gradient
  complex*16, target, dimension(n_full_points) :: v_free_gradient
  !------------------shanghui end add------------------------------------- 
...
!-------------end mpi---------------------------
 ...
 !-----------------------------------------------------------------------------
 ! initialize
   rho_free_gradient(1:n_full_points) = (0.0d0,0.0d0) 
   v_free_gradient(1:n_full_points) = (0.0d0, 0.0d0)
    ...
    do i_center = 1, n_centers_in_hamiltonian, 1
      ...
      if(current_spl_atom.eq.j_atom) then 
      ...
      do i_batch = 1, n_my_batches_work
        ...
        do i_index = 1, batches_work(i_batch)%size, 1
          ...
          if (partition_tab(i_full_points).gt.0.d0) then
           ...
           Gr(1) = ddot(3,recip_lattice_vector(1:3,1),1, coord_current(1:3),1)
           Gr(2) = ddot(3,recip_lattice_vector(1:3,2),1, coord_current(1:3),1)
           Gr(3) = ddot(3,recip_lattice_vector(1:3,3),1, coord_current(1:3),1)
           ...
           rho_free_gradient(i_full_points)= &
           rho_free_gradient(i_full_points)+ &
           rho_free_gradient_temp(j_coord)*  & 
           k_phase(center_to_cell(current_center),i_q_point)* & 
           exp_iqr(1)*exp_iqr(2)*exp_iqr(3) 

           v_free_gradient(i_full_points)= &
           v_free_gradient(i_full_points)+ &
           v_free_gradient_temp(j_coord)*    &
           k_phase(center_to_cell(current_center),i_q_point)* & 
           exp_iqr(1)*exp_iqr(2)*exp_iqr(3) 
...
end subroutine integrate_free_atom_sum_gradient_phonon_reduce_memory
!******
!------------------------------------------------------------------------------------------------------------
