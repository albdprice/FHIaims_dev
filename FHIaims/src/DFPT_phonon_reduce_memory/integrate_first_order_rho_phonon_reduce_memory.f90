!****s* FHI-aims/integrate_first_order_rho_phonon_reduce_memory
!  NAME
!   integrate_first_order_rho_phonon_reduce_memory
!  SYNOPSIS

subroutine integrate_first_order_rho_phonon_reduce_memory &
     (partition_tab_std, & 
      basis_l_max,  &  
...
!  ARGUMENTS
  ...
  complex*16,  dimension(n_full_points), intent(out) ::  first_order_rho 
  !shanghui------------------------------------------------------------------------
...
!---------------------------add for first_order_rho------------------------------------------             
             call  evaluate_first_order_rho_phonon_reduce_memory( &
                    n_points, n_compute_c,i_basis,& 
                    wave(:,1:n_points),gradient_basis_wave_npoints(:,:,1:n_points), &
                    density_matrix_con,first_order_density_matrix_con, coords_npoints(:,1:n_points), & 
                    i_q_point, j_atom, j_coord, &
...
end subroutine integrate_first_order_rho_phonon_reduce_memory
!******
