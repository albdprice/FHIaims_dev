!****s* FHI-aims/integrate_first_order_H_phonon_reduce_memory
!  NAME
!   integrate_first_order_H_phonon_reduce_memory
!  SYNOPSIS

subroutine integrate_first_order_H_phonon_reduce_memory &
...
!
! shanghui 2015.07 : change to phonon_reduce_memory
!
!  USES
...
!  real*8 dir_tab(3,n_centers_integrals, n_max_batch_size)
  ...
  complex*16, dimension( :, :, :), allocatable :: local_first_order_gradient_rho
  real*8, dimension(:),  allocatable :: density_matrix_con 
  complex*16, dimension(:,:),  allocatable :: first_order_density_matrix_con 


!! DSB  atomic zora
  real*8,dimension(:)  ,allocatable:: kinetic_wave_deriv    !!!DSB aux to  grad(T*PHI)
  real*8, dimension(:,:,:), allocatable :: kinetic_gradient_basis_wave  !!!DSB this is grad(T*PHI)
...
! VB - renewed index infrastructure starts here
  ...
  allocate(local_first_order_gradient_rho( 3, n_spin, n_max_batch_size), STAT=info )
...
!shanghui: we always need gradient. 
...
!-------End for hessian used for gga--------------------------------------
  ...
  ! initialize
   ...
   first_order_H_sparse = (0.0d0,0.0d0)
  ...
  do i_my_batch = 1, n_my_batches_work, 1
   !write(use_unit,*) 'shanghui in integrid_first_order_H:',n_my_batches_work
     ...
     !-----------shanghui begin prune_matrix here-------------------------
     ...
     call  prune_density_matrix_sparse_phonon_reduce_memory(first_order_density_matrix_sparse, & 
                     ...
                     n_compute_c, i_basis)  
     !-----------shanghui end prune_matrix here-------------------------
...
!shanghui: we alwayse need gradients
...
!              end if
              ...
              do i_spin = 1, n_spin, 1
              !---------shanghui added in order to get: 
              !---------H_times_psi= (T+Vhartree+V_xc)  in LDA case.  
              !---------H_times_psi= (T+Vhartree)       in GGA case.  
...
!----------shanghui-gga: always calculate gradient---------------------
               ...
               if(.not.use_gga) then ! the LDA case, only need gradient
              ...
              if(flag_rel .eq.  REL_atomic_zora)then   !!!DSB atomic zora
              ...
              enddo
              if ((flag_rel.eq.REL_atomic_zora)) then    !!!DSB atomic zora (evaluate grad[T_z*PHI])
                 call evaluate_radial_functions_deriv_p0 &
                   (spline_array_start, spline_array_end, &
                   n_compute_atoms, n_compute_fns,  &
                   dist_tab(1,i_point), i_r(1), &
              ...
              !---------shanghui add: we will not use H_times_psi in GGA case,but just TVhartee_times_psi,
              !---------because we will treate d[ (Vgga)_uv]/dR wholely in evaluate_H1. 
              !---------H_times_psi= (T+Vhartree+V_xc)  in LDA case.  
              !---------H_times_psi= (T+Vhartree)       in GGA case.  
        ...
        do i_spin = 1, n_spin, 1
           ...
           if(use_gga) then 
           call  evaluate_first_order_gradient_rho_phonon_reduce_memory( &
           ...
           call evaluate_first_order_H_GGA_phonon_reduce_memory  &
            ...
            call evaluate_first_order_H_phonon_reduce_memory  &
               ( n_points, partition(1:n_points), &
                 n_compute_c, i_basis,  &
                 wave(:,1:n_points), gradient_basis_wave_npoints(:,:,1:n_points), &
                 kinetic_gradient_basis_wave(:,:,1:n_points), &
  ...
  if(.not. use_local_index) call sync_vector_complex(first_order_H_sparse, n_hamiltonian_matrix_size)
  ...
  if(allocated( kinetic_wave_deriv    )) deallocate( kinetic_wave_deriv )      !DSB ZORA
  if(allocated( kinetic_gradient_basis_wave          )) deallocate( kinetic_gradient_basis_wave        ) !DSB ZORA 
  ...
  if(allocated( local_first_order_gradient_rho  )) deallocate( local_first_order_gradient_rho     )  
  ...
  if(allocated( hessian_basis_wave   )) deallocate( hessian_basis_wave   ) 
...
end subroutine integrate_first_order_H_phonon_reduce_memory
!******
