!****s* FHI-aims/divide_angular_grid_p0
!  NAME
!    divide_angular_grid_p0
!  SYNOPSIS 
      subroutine divide_angular_grid_p0( &
...
!  USES
      use dimensions, only : n_max_angular, n_max_angular_division, use_angular_division
      implicit none
!  ARGUMENTS
      integer :: n_angular
...
!    o n_angular_points -- number of angular points in the grid
!  OUTPUT
...
!     if we are below the cutoff raduis, do nothing
      ...
      else if (n_angular_points.le.110) then
!     smallest grids: only one division
         n_division = 1
         division_index = 1
         points_in_division(1) = n_angular

      else if (n_angular_points.le.302) then
!     divide into 2 blocks, by z-coordinate
      ...
      else if (n_angular_points.le.770) then
...
!     copy the permuted arrays to the original ones
      ...
      end subroutine divide_angular_grid_p0
!******
