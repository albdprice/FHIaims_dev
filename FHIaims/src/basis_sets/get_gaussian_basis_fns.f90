!****s* FHI-aims/get_gaussian_basis_fns
!  NAME
!   get_gaussian_basis_fns
!  SYNOPSIS

      subroutine get_gaussian_basis_fns( i_species)
...
!  USES
      ...
      use species_data,    only : gaussian_wave, gaussian_kinetic, gaussian_wave_deriv, &
                                  gaussian_outer_radius, gaussian_n, gaussian_alpha, &
                                  gaussian_coeff, gaussian_n_contr, gaussian_l, &
                                  n_gaussian
...
!test - create new outermost radius array after the fact, for each radial function
            ...
            do while (abs(gaussian_wave(i_grid,i_species,i_gaussian)).lt.wave_threshold)
               i_grid = i_grid - 1 
            end do
            gaussian_outer_radius(i_species,i_gaussian) = r_grid(i_grid,i_species)
!end test
...
!     output Gaussian basis data
      if (myid.eq.0) then
        ...
        write(use_unit,'(4X,A,4X,A,4X,A)') &
        "L", "l", "outer radius"
        do i_gaussian = 1, n_gaussian(i_species), 1
          write(use_unit,'(2X,I3,2X,I3,2X,F15.5,A)') &
          gaussian_n(i_species,i_gaussian), &
          gaussian_l(i_species,i_gaussian), &
          gaussian_outer_radius(i_species,i_gaussian)*bohr, " AA"
        enddo
        write(use_unit,*)
      end if
...
!  that's all folks
    ...
    end subroutine get_gaussian_basis_fns
