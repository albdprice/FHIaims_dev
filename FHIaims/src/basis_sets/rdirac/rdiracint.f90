...
! !ROUTINE: rdiracint
! !INTERFACE:
subroutine rdiracint(mode,sol,m,kpa,e,np,nr,r,vr,nn,g0p,f0p,g0,g1,f0,f1,max_nr)
...
ri=1.d0/r(1)
if (mode) then
    g0(1)=(alpha*(2.d0*e0-vr(1))*f0(1)-g1(1))/(rkpa*ri)
...
! compute the derivatives
  if (mode) then
    g1(ir)=alpha*(2.d0*e0-vr(ir))*f0(ir)-rkpa*ri*g0(ir)
