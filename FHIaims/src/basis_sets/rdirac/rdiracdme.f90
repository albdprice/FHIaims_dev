...
! !ROUTINE: rdiracdme
! !INTERFACE:
subroutine rdiracdme(mode,sol,m,kpa,e,np,nr,r,vr,nn,g0,g1,f0,f1,max_nr)
! !INPUT/OUTPUT PARAMETERS:
!   mode: use zora approximation (in, logical)
...
if (m.eq.0) then
  call rdiracint(mode,sol,m,kpa,e,np,nr,r,vr,nn,g0p,f0p,g0,g1,f0,f1,max_nr)
else
  do im=0,m
    call rdiracint(mode,sol,im,kpa,e,np,nr,r,vr,nn,g0p,f0p,g0,g1,f0,f1,max_nr)
    g0p(:)=g0(:)
    f0p(:)=f0(:)
  end do
end if
