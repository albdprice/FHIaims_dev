...
c
c  take care of common /gas/ in a calling subroutine 
c  pure spin singularity numerically removed
c
c original version by J P Perdew
...
c      if(abs(zet) .ge. 1.d0) then
c        if(zet .lt. 0.d0) zet=-1.d0+1.d-15
c        if(zet .gt. 0.d0) zet=1.d0-1.d-15
cmf        if(flag .eqv. .true.) write(*,50)
cmf        flag=.false.
cmf  50    format(/'warning: corrga  -  zet substituted')
c      endif
cmf
c
c   Above code did had problem with spin polarized hydorogen atom
c   So we deal zet = abs(1.0) explicitly to avoid NaN problems.
c    -Matti
      if(zet .eq. -1.d0) then
         gz = -2.d0**thrdm/3.0
      else if (zet .eq. 1.d0) then
         gz = 2.d0**thrdm/3.0
      else
