!****s* FHI-aims/evaluate_qpe_sicrpa_energy
!  NAME
!   evaluate_qpe_sicrpa_energy
!  SYNOPSIS

      subroutine evaluate_qpe_sicrpa_energy &
           (n_electrons, &
...
!  PURPOSE
!  Subroutine evaluate_qpe_sicrpa_energy evaluates the correlation
...
!  local variables
      ...
      real*8  rpa_c_energy_tot
      real*8  rpa_c_energy_sic
...
!   invert the bare Coulomb matrix for calculating the screened Coulomb interaction .
      ...
      rpa_c_energy     = 0.d0 
      rpa_c_energy_tot = 0.d0 
      rpa_c_energy_sic = 0.d0 
...
! how to improve?
       ...
       call evaluate_sicrpa_integrand(polar_freq,rpa_c_integrand_tot, rpa_c_integrand_sic)

!       call sync_real_number(rpa_c_integrand) 

       rpa_c_energy_tot = rpa_c_energy_tot + &
                       rpa_c_integrand_tot * womega_full(i_freq)

       rpa_c_energy_sic = rpa_c_energy_sic + &
                       rpa_c_integrand_sic * womega_full(i_freq)

       rpa_c_energy = rpa_c_energy + &
                       (rpa_c_integrand_tot - rpa_c_integrand_sic) * &
                       womega_full(i_freq)
...
!      close(112)
      ...
      rpa_c_energy_tot=rpa_c_energy_tot/2.d0/pi
      rpa_c_energy_sic=rpa_c_energy_sic/2.d0/pi
...
! out over frequency axis.
      ...
      if(myid.eq.0) then
        ...
        write(use_unit,'(2X,A,2X,f19.8,2X,A,f19.8,2X,A)') &
            "  RPA correlation energy     :", rpa_c_energy_tot, "Ha,", &
             rpa_c_energy_tot*hartree, "eV"
        write(use_unit,'(2X,A,2X,f19.8,2X,A,f19.8,2X,A)') &
            "  SIC-RPA correlation energy :", rpa_c_energy, "Ha,", &
...
!                 "-------------------------"
      ...
      end subroutine evaluate_qpe_sicrpa_energy
!---------------------------------------------------------------------
!******
