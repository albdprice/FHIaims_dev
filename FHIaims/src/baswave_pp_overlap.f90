...
!DEBUG: THIS IS THE INVERSE OF baswave_pp_overlap3()

  subroutine baswave_pp_overlap()
            ...
            if (Rabs_sq < (maxval(outer_radius) + maxval(pp_outer_radius))**2) then
              ...
              if (use_forces) then
                ...
                call sbt_atomic_ovlp(N, lnr0, lnk0, lnrange, Rvec, 1D0, .false., &
                                  max_pp_basis_l, max_L, &
                                 max_n_pp_basis_fnLsp, max_n_basis_fnLsp,  &
                                 n_pp_basis_fns, pseudowave, pp_outer_radius, moment_2, & 
                                 Lsp2n_pp_basis_fnLsp(:,i_pp_species), Lsp2_pp_basis_fn(:,:,i_pp_species),pp_fnL_to_col, &
                                 n_basis_fns, baswave, outer_radius, moment_1, &
                                 Lsp2n_basis_fnLsp(:,i_species), Lsp2basis_fn(:,:,i_species), fnL_to_row,&
                                 pp_basiswave_overlap, times, d_pp_basiswave_overlap)
...
!    do i= 1, n_pp_basis
!      do j = 1, n_basis
!        write(use_unit,*) 'i_pp_basis', i 
!        write(use_unit,*) 'i_basis', j 
!        write(use_unit,*) 'basiswave_pp_overlap(',i,',',j,')', basiswave_pp_overlap(i,j)
!      enddo
!    enddo
...
!        write(use_unit,*) 'i_pp_basis', i 
!        write(use_unit,*) 'i_basis', j 
!        write(use_unit,*) 'd_basiswave_pp_overlap(:,',i,',',j,')', d_basiswave_pp_overlap(:,i,j)
!      enddo
!    enddo


!    open(792,file='d_pp_basiswava_overlap')

!    do j= 1, n_pp_basis
!     do i = 1, n_basis
!        write(792,*) 'i_pp_basis', j 
!        write(792,*) 'i_basis', i 
!        write(792,*) 'd_pp_basiswave_overlap(:,',j,',',i,')', d_pp_basiswave_overlap(:,j,i)
!      enddo
!    enddo
!    close(792)
...
!     if(myid==0)  write(use_unit,*) 'baswave_pp_overlap: minval(basiswave_pp_overlap) =', minval(basiswave_pp_overlap)
  ...
  end subroutine baswave_pp_overlap
