!****s* FHI-aims/integrate_ovlp_prodbas
!  NAME
!   integrate_ovlp_prodbas
!  SYNOPSIS

      subroutine integrate_ovlp_prodbas &
      (basis_l_max, ovlp_prodbas)

!  PURPOSE
!  this subroutine is intended to calculate the overlap matrix
!  of two product basis functions.
!
!  USES
...
!  OUTPUTS
!  o ovlp_prodbas -- real array, the overlap matrix between the auxiliary
!            basis         
!
!  AUTHOR
...
!    v_times_wave:
      ...
      real*8, dimension(:,:), allocatable :: loc_waves
...
!  begin work
      ...
      if(myid.eq.0) then
       ...
       "Integrating the overlap matrix for auxiliary basis functions" &
           , " ..."
       write(use_unit,*)
      endif
...
!     begin with general allocations
      ...
      allocate( loc_waves (n_loc_prodbas, n_max_angular) )
...
!  obtain the product basis at the current processor

                    do i_compute = 1, n_loc_prodbas, 1

                       i_basis_1 = i_prodbas(i_compute)
                       i_basbas = map_prodbas(i_basis_1, i_task)
                       if(i_basbas.gt.0) then
                         loc_waves(i_compute,n_points) =  &
                           wave(i_basbas,n_points)
                       endif
                   enddo
...
!          write(use_unit, '(4X,I5,5X,I5, 5X, 2I5, 5X, 2I5, 5X,f25.10)') &
!                 i_basis_1, i_basis_2, basbas_l(i_basis_1), &
!                 basbas_m(i_basis_1), basbas_l(i_basis_2),basbas_m(i_basis_2), &
!                 ovlp_prodbas(i_basis_2,i_basis_1)
!         enddo
!        enddo
!       endif
!test
      ...
      if (allocated( loc_waves) ) then
        deallocate( loc_waves )
      endif
      ...
      return
      end subroutine integrate_ovlp_prodbas
