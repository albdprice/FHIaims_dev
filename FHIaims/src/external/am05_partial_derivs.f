c Header:
c**********************************************************************
c - MR: Original routine can be found in this folder, called: am05_xscss_web.f
c       I kept here only the "wbs" scheme and modified it slightly to be compatible
c       with FHI-aims structure.
c - the changes were mostly in the output: 
c
c
...
c**********************************************************************
      ...
      subroutine am05_partial_derivs(nup, ndn, gup, gdn,
...
c     ** Dummy parameters (not used)
      ...
      call saferecp(sup,gup,(4.0d0*kFup*nup))
      call saferecp(sdn,gdn,(4.0d0*kFdn*ndn))
...
c     ** Form the output
c     Change one: must divide by the whole density
      fxc = (fx + fc)/(nup+ndn)

      dfxcup = dfxup + dfcup
      dfxcdn = dfxdn + dfcdn
c     Change two: must not multiply by gdn(gup) and there is a factor 1/4
      dfxcdgup = (dfxdgup + dfcdgup)*0.25
      dfxcdgdn = (dfxdgdn + dfcdgdn)*0.25
