...
{ 
   ...
   if(time>=steerplan.nextstage_time||firstTime){
       for(i=0;i<steerplan.totstages;i++){
	   if(  time>=steerplan.actions[i].t && time<steerplan.actions[i+1].t ){steerplan.current_stage=i;
              if((i+1)!=steerplan.totstages){steerplan.nextstage_time=steerplan.actions[i+1].t;}
              else{steerplan.nextstage_time=1.e9;} 
           break;} 
       }
       fprintf(mtd_data.fplog,"|- CURRENT STAGE %d OVER %d NSTAGES ACTUAL TIME %f NEXT CHANGE %f\n",steerplan.current_stage,steerplan.totstages,time,steerplan.nextstage_time);
       // setup the run
