...
#elif defined(DL_POLY) || defined(AMBER) || defined(PLUMED_QESPRESSO) || defined(GAT_LJONES) || defined (FHIAIMS)
...
#ifdef FHIAIMS
void meta_force_calculation__(real *cell, int *istep, real *xxx, real *yyy, real *zzz, real *fxx, real *fyy, real *fzz, real *energy)
#else
...
#if defined (DL_POLY) || defined (GAT_LJONES) || defined (FHIAIMS)
...
// rehack for FHI AIMS
#ifdef FHIAIMS
for(i=0;i<9;i++)mtd_data.cell[i]=0.;
int j;
for(i=0;i<3;i++){
  for(j=0;j<3;j++){
    if(i==j){
    	mtd_data.cell[i]=cell[i*3+j];
    }else{
	if(cell[i*3+j]>1.e-5)plumed_error("PLUMED+FHIAIMS only supports orthorombic cells"); 
    }
  }
}
#endif
...
void meta_force_calculation_(real *cell, int *istep, real *xxx, real *yyy, real *zzz, real *fxx, real *fyy, real *fzz, real *energy)
{
        meta_force_calculation__(cell, istep, xxx, yyy, zzz, fxx, fyy, fzz, energy);
}

void init_metadyn_(int *atoms, real *ddt, real *mass, real *charge, int *imcon, real *eunit, char *metainp, int pp)
{
        init_metadyn__(atoms, ddt, mass, charge, imcon, eunit, metainp, pp);
}
#endif
...
#elif defined (DL_POLY) || defined (AMBER) || defined (PLUMED_QESPRESSO) || defined (GAT_LJONES) || defined (FHIAIMS)
  void init_metadyn__(int *atoms, real *ddt, real *mass, real *charge, int *imcon, real *eunit, char *metainp, int pp) 
#elif defined (PLUMED_CPMD)
...
#endif
  *mod_rij=sqrt(pow(rij[0],2)+pow(rij[1],2)+pow(rij[2],2));
#elif defined (ACEMD) || defined (DRIVER) || defined (AMBER) || defined (STANDALONE) || defined (PLUMED_QESPRESSO) || defined (FHIAIMS) 
  int i;
