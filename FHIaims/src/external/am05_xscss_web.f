...
c   gtot   abs of gradient of total density, |grad(nup+ndn)| [bohr**(-4)]
c   ggup   grad(nup) * grad(|grad(nup)|) [bohr**(-9)]
c   ggdn   grad(ndn) * grad(|grad(ndn)|) [bohr**(-9)]
c   lup    laplacian of density, grad*grad(nup)=grad**2(nup) [bohr**(-5)]
c   ldn    laplacian of density, grad*grad(ndn)=grad**2(ndn) [bohr**(-5)]
c
c output
c   fxc          exchange-correlation energy density [hartree]
c   vxcup        upspin exchange-correlation potential [hartree]
c   vxcdn        downspin exchange-correlation potential [hartree]
c   dfxcdgupogup        d(fxc)/d(|grad(nup)|) / |grad(nup)|, for use 
c                in the stress tensor [hartree * bohr**(5)]
c   dfxcdgdnogdn        d(fxc)/d(|grad(ndn)|) / |grad(ndn)|, for use 
c                in the stress tensor [hartree * bohr**(5)]
c
c Note that dfxcdgtotogtot = 
c                   = d(fxc)/d(|grad(nup+ndn)|) / |grad(nup+ndn)| = 0
c
c**********************************************************************

      subroutine am05trads(nup, ndn, gup, gdn, gtot, ggup, ggdn, lup, ldn,
     *     fxc, vxcup, vxcdn, dfxcdgupogup, dfxcdgdnogdn)

      implicit none

c     ** Input parameters
      real*8 nup, ndn, gup, gdn, ggup, ggdn, gtot, lup, ldn

c     ** Output parameters
      real*8 fxc, vxcup, vxcdn, dfxcdgupogup, dfxcdgdnogdn

c     ** Internal parameters
      real*8 kFup, sup, uup, tup
      real*8 kFdn, sdn, udn, tdn
      real*8 kFtot, stot
      real*8 fx,fc,vxup,vcup,vxdn,vcdn
      real*8 dfxdgup, dfxdgdn
      real*8 dfcdgup, dfcdgdn
...
c     ** Dummy parameters (not used)
      real*8 dfxup,dfxdn,dfcup,dfcdn

      kFup = (3.0d0*pi**2*2.0d0*nup)**(1.0d0/3.0d0)
      kFdn = (3.0d0*pi**2*2.0d0*ndn)**(1.0d0/3.0d0)
      kFtot = (3.0d0*pi**2*(nup+ndn))**(1.0d0/3.0d0)
      call saferecp(sup,gup,(2.0d0*kFup*nup))
      call saferecp(sdn,gdn,(2.0d0*kFdn*ndn))  
      call saferecp(stot,gtot,(2.0d0*kFtot*(nup+ndn)))  
      call saferecp(uup,ggup,((2.0d0*kFup)**3*nup**2))
      call saferecp(udn,ggdn,((2.0d0*kFdn)**3*ndn**2))
      call saferecp(tup,lup,((2.0d0*kFup)**2*nup))
      call saferecp(tdn,ldn,((2.0d0*kFdn)**2*ndn)) 
...
c     ** Form the output
      ...
      vxcup = vxup + vcup
      vxcdn = vxdn + vcdn
      dfxcdgupogup = dfxdgup + dfcdgup 
      dfxcdgdnogdn = dfxdgdn + dfcdgdn 
...
c**********************************************************************
      ...
      subroutine am05wbs(nup, ndn, gup, gdn,
     *     fxc, dfxcup, dfxcdn, dfxcdgup, dfxcdgdn)
...
c     ** Output parameters
      real*8 fxc, dfxcup, dfxcdn, dfxcdgup, dfxcdgdn
...
c     ** Form the output
      ...
      dfxcup = dfxup + dfcup
      dfxcdn = dfxdn + dfcdn
      dfxcdgup = gup*(dfxdgup + dfcdgup)
      dfxcdgdn = gdn*(dfxdgdn + dfcdgdn)
...
c
c input
c   nup     electron spin-up density [bohr**(-3)]
c   ndn     electron spin-down density [bohr**(-3)]
c   gup2    abs squared of gradient of spin-up density, 
c           |grad(nup)|**2 [bohr**(-8)]
c   gdn2    abs squared of gradient of spin-down density, 
c           |grad(ndn)|**2 [bohr**(-8)]
c   gupgdn  gradient of spin-up density * gradient of spin-down density, 
c           grad(nup)*grad(ndn) [bohr**(-8)]
c
c output
c   fxc       exchange-correlation energy density [hartree * bohr**(-3)]
c   dfxcup    d(fxc)/d(nup)  [hartree]
c   dfxcdn    d(fxc)/d(ndn)  [hartree]
c   dfxcdgup2  d(fxc)/d(|grad(nup)|**2)  [hartree * bohr**(5)]
c   dfxcdgdn2  d(fxc)/d(|grad(ndn)|**2)  [hartree * bohr**(5)]
c   dfxcdgupgdn d(fxc)/d(grad(nup)*grad(ndn))   [hartree * bohr**(5)]
c
c Note that 
...
c**********************************************************************
      ...
      subroutine am05pgjs(nup,ndn,gup2,gdn2,gupgdn,
     *     fxc,dfxcup,dfxcdn,dfxcdgup2,dfxcdgdn2,dfxcdgupgdn)

      implicit none

c     ** Input parameters
      real*8 nup,ndn,gup2,gdn2,gupgdn

c     ** Output parameters
      real*8 fxc,dfxcup,dfxcdn,dfxcdgup2,dfxcdgdn2,dfxcdgupgdn
...
c     ** Form the output
      ...
      dfxcup = dfxup + dfcup
      dfxcdn = dfxdn + dfcdn
      dfxcdgup2 = (dfxdgup + dfcdgup)/2.0d0
      dfxcdgdn2 = (dfxdgdn + dfcdgdn)/2.0d0
      dfxcdgupgdn = 0.0d0
      return
      end
...
c**********************************************************************
      ...
      subroutine am05wbnums(nup, ndn, gup, gdn,
     *     fxc, dfxcup, dfxcdn, dfxcdgup, dfxcdgdn)

      implicit none

c     ** Input parameters
      real*8 nup, ndn, gup, gdn
...
c     ** Internal parameters
      ...
      real*8 dfxup,dfxdn,dfcup,dfcdn
      real*8 dfxdgup,dfxdgdn,dfcdgup,dfcdgdn
      real*8 nmdup, npdup, kFnmdup, kFnpdup 
      real*8 snmdup, snpdup, sgnmdup, sgnpdup
      real*8 nmddn, npddn, kFnmddn, kFnpddn 
      real*8 snmddn, snpddn, sgnmddn, sgnpddn

      real*8 fxnmdup, fcnmdup, fxnpdup, fcnpdup 
      real*8 fxgnmdup, fcgnmdup, fxgnpdup, fcgnpdup
      real*8 fxnmddn, fcnmddn, fxnpddn, fcnpddn 
      real*8 fxgnmddn, fcgnmddn, fxgnpddn, fcgnpddn

      real*8 deltaup, deltadn, sdeltaup, sdeltadn
...
c     ** not needed input for pot=0
      ...
      call saferecp(sdn,gdn,(2.0d0*kFdn*ndn))  
      deltaup = nup*0.01d0
      if (deltaup .ge. 1.0e-4) deltaup = 1.0e-4
      deltadn = ndn*0.01d0
      if (deltadn .ge. 1.0e-4) deltadn = 1.0e-4

      nmdup = nup - deltaup
      npdup = nup + deltaup
      nmddn = ndn - deltadn
      npddn = ndn + deltadn
      kFnmdup = (3.0d0*pi**2*2.0d0*nmdup)**(1.0d0/3.0d0)
      kFnpdup = (3.0d0*pi**2*2.0d0*npdup)**(1.0d0/3.0d0)
      kFnmddn = (3.0d0*pi**2*2.0d0*nmddn)**(1.0d0/3.0d0)
      kFnpddn = (3.0d0*pi**2*2.0d0*npddn)**(1.0d0/3.0d0)
      call saferecp(snmdup,gup,(2.0d0*kFnmdup*nmdup))
      call saferecp(snpdup,gup,(2.0d0*kFnpdup*npdup))
      call saferecp(snmddn,gdn,(2.0d0*kFnmddn*nmddn))
      call saferecp(snpddn,gdn,(2.0d0*kFnpddn*npddn))

      sdeltaup = gup*0.01d0
      if (sdeltaup .ge. 1.0e-4) sdeltaup = 1.0e-4
      sdeltadn = gdn*0.01d0
      if (sdeltadn .ge. 1.0e-4) sdeltadn = 1.0e-4
      call saferecp(sgnmdup,(gup - sdeltaup),(2.0d0*kFup*nup))
      call saferecp(sgnpdup,(gup + sdeltaup),(2.0d0*kFup*nup))
      call saferecp(sgnmddn,(gdn - sdeltadn),(2.0d0*kFdn*ndn))
      call saferecp(sgnpddn,(gdn + sdeltadn),(2.0d0*kFdn*ndn))
     ...
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)
      ...
      call am05_xscss(nmdup,ndn,snmdup,sdn,stot,uup,udn,tup,tdn,
     *     fxnmdup,fcnmdup,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

      call am05_xscss(nup,nmddn,sup,snmddn,stot,uup,udn,tup,tdn,
     *     fxnmddn,fcnmddn,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

      call am05_xscss(npdup,ndn,snpdup,sdn,stot,uup,udn,tup,tdn,
     *     fxnpdup,fcnpdup,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

      call am05_xscss(nup,npddn,sup,snpddn,stot,uup,udn,tup,tdn,
     *     fxnpddn,fcnpddn,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

      call am05_xscss(nup,ndn,sgnmdup,sdn,stot,uup,udn,tup,tdn,
     *     fxgnmdup,fcgnmdup,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

      call am05_xscss(nup,ndn,sup,sgnmddn,stot,uup,udn,tup,tdn,
     *     fxgnmddn,fcgnmddn,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

      call am05_xscss(nup,ndn,sgnpdup,sdn,stot,uup,udn,tup,tdn,
     *     fxgnpdup,fcgnpdup,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

      call am05_xscss(nup,ndn,sup,sgnpddn,stot,uup,udn,tup,tdn,
     *     fxgnpddn,fcgnpddn,vxup,vxdn,vcup,vcdn,
     *     dfxup,dfxdgup,dfcup,dfcdgup,
     *     dfxdn,dfxdgdn,dfcdn,dfcdgdn,pot)

c     ** Form the output
      call saferecp(dfxcup,((fxnpdup + fcnpdup) - (fxnmdup + fcnmdup)),
     *     (2.0d0*deltaup))
      call saferecp(dfxcdn,((fxnpddn + fcnpddn) - (fxnmddn + fcnmddn)),
     *     (2.0d0*deltadn))

      call saferecp(dfxcdgup,((fxgnpdup + fcgnpdup)-
     *     (fxgnmdup + fcgnmdup)),(2.0d0*sdeltaup))
      call saferecp(dfxcdgdn,((fxgnpddn + fcgnpddn)-
     *     (fxgnmddn + fcgnmddn)),(2.0d0*sdeltadn))
