c--------------------------------------------------------------------
c
c     original subroutine from Scuseria,
c
c     modified by SG
c
c--------------------------------------------------------------------
c
c     wPBE Enhancement Factor (erfc approx.,analytical, gradients)
c
c--------------------------------------------------------------------

      subroutine HSEFx_partial(rho,s_aux,omega,
     +     Fx_wpbe,d1rfx,d1sfx)
      ...
      Real*8 rho,s,omega,Fx_wpbe,d1sfx,d1rfx,s_aux

      real*8 sreal,strans,smax, sconst,stra1s
...
c     Calculate helper variables (should be moved later on...)
     ...
     1                   Four*B*(DHs2) + Eight*A*(DHs3))
     2                * (One / (Sixteen * DHs72))
     3                 - f34*pi*sqrt(A) * exp(f94*H*s2/A) *
     4                   (One - arch_erf(f32*s*sqrt(H/A)))

        d1sG_a = (One/r32)*srpi *
     1           ((r36*(Two*H + d1sH*s) / (A12*sqrt(H/A)))
     2            + (One/DHs92) *
     3               (-Eight*A*d1sDHs*DHs3 - r105*d1sDHs*E
     4                -r30*C*d1sDHs*DHs*(One+s2*F)
     5                +r12*DHs2*(-B*d1sDHs + C*s*(d1sF*s + Two*F)))
     6            - ((r54*exp(f94*H*s2/A)*srpi*s*(Two*H+d1sH*s)*
     7               arch_erfc(f32*sqrt(H/A)*s))
     8               / A12))
...
c    Calculate the terms needed in any case
     ...
     1            + DHs2 * (-d1sDHs*B + s*C*(d1sF*s + Two*F))
     2            + Two*DHs * (Two*EG*s - d1sDHs*C 
     3            + s2 * (d1sEG - d1sDHs*C*F)))
     4           / (Two*DHs4)

      term3 = - w  * (Four*DHsw2*B + Six*DHsw*C + Fifteen*E
     1                + Six*DHsw*s2*C*F + Fifteen*s2*EG) /
     2               (Eight*DHs*DHsw52)

      d1sterm3 = w * (Two*d1sDHs*DHsw * (Four*DHsw2*B 
     1                   + Six*DHsw*C + Fifteen*E
     2                   + Three*s2*(Five*EG + Two*DHsw*C*F))
     3                + DHs * (r75*d1sDHs*(EG*s2 + E)
     4                   + Four*DHsw2*(d1sDHs*B 
     5                        - Three*s*C*(d1sF*s + Two*F))
     6                   - Six*DHsw*(-Three*d1sDHs*C 
     7                        + s*(Ten*EG + Five*d1sEG*s
     8                            - Three*d1sDHs*s*C*F))))
     9           / (Sixteen*DHs2*DHsw72)

      d1rterm3 = (-Two*d1rw*DHsw * (Four*DHsw2*B 
     1                   + Six*DHsw*C + Fifteen*E
     2                   + Three*s2*(Five*EG + Two*DHsw*C*F))
     3                + w * d1rDHsw * (r75*(EG*s2 + E) 
     4                   + Two*DHsw*(Two*DHsw*B + Nine*C 
     5                               + Nine*s2*C*F)))
     5           / (Sixteen*DHs*DHsw72)

      term4 = - w3 * (DHsw*C + Five*E + DHsw*s2*C*F + Five*s2*EG) /
     1               (Two*DHs2*DHsw52)

      d1sterm4 = (w3 * (Four*d1sDHs*DHsw * (DHsw*C + Five*E 
     1                       + s2 * (Five*EG + DHsw*C*F))
     2                  + DHs * (r25*d1sDHs*(EG*s2 + E)
     3                       - Two*DHsw2*s*C*(d1sF*s + Two*F)
     4                       + DHsw * (Three*d1sDHs*C + s*(-r20*EG
     5                             - Ten*d1sEG*s 
     6                             + Three*d1sDHs*s*C*F)))))
     7           / (Four*DHs3*DHsw72)
  
      d1rterm4 = (w2 * (-Six*d1rw*DHsw * (DHsw*C + Five*E 
     1                       + s2 * (Five*EG + DHsw*C*F))
     2                  + w * d1rDHsw * (r25*(EG*s2 + E) 
     3                       +Three*DHsw*C*(One + s2*F))))
     4           / (Four*DHs2*DHsw72)
     ...
     1                  + DHs * (-Two*DHsw*s * (Two*EG + d1sEG*s)
     2                       + Five*d1sDHs * (EG*s2 + E))))
     3           / (Two*DHs4*DHsw72)
...
c     Calculate exp(x)*f(x) depending on size of x
      ...
      if(HsbwA94 .lt. expfcutoff) then
        ...
        piexperf = pi*Exp(HsbwA94)*arch_Erfc(HsbwA9412)
        call EI(-HsbwA94,eiHsbwA94)

        expei    = Exp(HsbwA94)*eiHsbwA94
...
c     Calculate the derivatives (based on the orig. expression)
c     --> Is this ok? ==> seems to be ok...

      piexperfd1  = - (Three*srpi*sqrt(Hsbw/A))/(Two*Hsbw)
     1              + (Nine*piexperf)/(Four*A)
...
c       Fall back to original expression for the PBE hole
        ...
        if(s .gt. 0.0D0) then
          ...
          d1sfx = X * (d1sterm1 + d1sterm2)*stra1s
          d1rfx = X * d1rterm1
...
c TODO    This is checked to be true for term1
c         How about the other terms???
...
c       First, we calculate the polynomials for the first term
     ...
     1           - (r1215*ea5*d1rw*w4)/(r32*A32)
     2           + (r15309*ea7*d1rw*w6)/(r128*A52)

        np2 = -A + f94*ea2*w2 - r81*ea4*w4/(Sixteen*A)
     $        + r729*ea6*w6/(r64*A2) - r6561*ea8*w8/(r256*A3)

        d1rnp2 =   f12*(Nine*ea2*d1rw*w) 
     1           - (r81*ea4*d1rw*w3)/(Four*A)
     2           + (r2187*ea6*d1rw*w5)/(r32*A2)
     3           - (r6561*ea8*d1rw*w7)/(r32*A3)
...
c       The first term is
     ...
     1               d1rexpei*np2 + d1rnp1*piexperf)

c       The factors for the main polynomoal in w and their derivatives
     ...
     1            + d1rf3*w2 + Three*d1rw*f4*w2 
     1            + d1rf4*w3 + Four*d1rw*f5*w3 
     2            + d1rf5*w4 + Five*d1rw*f6*w4 
     3            + d1rf6*w5 + Six*d1rw*f7*w5 
     4            + d1rf7*w6 + Seven*d1rw*f8*w6 
     5            + d1rf8*w7 + Eight*d1rw*f9*w7 + d1rf9*w8
...
c       The final value for the enhancement factor and its
c       derivatives is:
