c######################################################################
c----------------------------------------------------------------------
      SUBROUTINE EXCHXPBE_derivs
     +( rho, squared_grad_rho,
     +  en_density_x, x_density_deriv, x_gradient_deriv )
C
C  IYZ - Subroutine based on:
c
C----------------------------------------------------------------------
C  xPBE EXCHANGE FOR A SPIN-UNPOLARIZED ELECTRONIC SYSTEM
c  XX's modification of PBE codes, Sep. 1, 2004
c----------------------------------------------------------------------
c----------------------------------------------------------------------
C
C  INPUT: 
...
c     {\bf 40},  3399  (1989) (E);
c [c] Xin Xu, William A. Goddard III, JCP, Vol. 121, No. 9, 4068 (2004).
c----------------------------------------------------------------------
c----------------------------------------------------------------------
c Formulas:
...
C     declare constants
     ...
     +          thrd8=8.d0/3.d0)
      ...
      parameter(um=0.23214d0,uk=0.91954d0,ul=um/uk)
...
c      parameter(gamma=0.03109069086965489503494086371273d0)
      parameter(gamma=0.02043355766025039751965231005215d0)
c      parameter(bet=0.06672455060314922d0,delt=bet/gamma)
      parameter(bet=0.089809d0,delt=bet/gamma)
