...
subroutine get_occupation_numbers(KS_eigenvalue, n_electrons, t_out, occ_numbers, chemical_potential)
...
!  SOURCE
  ...
  do i_spin = 1, n_spin, 1
     do i_state = 1, n_states, 1
        if (KS_eigenvalue(i_state, i_spin) .lt. lowest_eigenvalue) then
           lowest_eigenvalue = KS_eigenvalue(i_state, i_spin)
        end if
     end do
  end do
  ...
  do i_spin = 1, n_spin, 1
     do i_state = 1, n_states, 1
        if (KS_eigenvalue(i_state, i_spin) .gt. highest_eigenvalue) then
           highest_eigenvalue = KS_eigenvalue(i_state, i_spin)
        end if
     end do
  end do
  ...
  call check_norm(chemical_potential_l, KS_eigenvalue, n_electrons, occ_numbers, diff_l, i_counter)
  call check_norm(chemical_potential_r, KS_eigenvalue, n_electrons, occ_numbers, diff_r, i_counter)
  ...
  call zeroin(chemical_potential_l, chemical_potential_r, diff_l, diff_r, fermi_acc, occupation_acc, max_zeroin, &
  ...
  if (i_counter .gt. max_zeroin) then
     ...
     do i_spin = 1, n_spin, 1
        ...
        call localorb_info(info_str,use_unit,'(A)')
        do i_state = 1, n_states, 1
           write (info_str,*) i_state, KS_eigenvalue(i_state, i_spin), occ_numbers(i_state, i_spin)
           call localorb_info(info_str,use_unit,'(A)')
        end do
     end do
     stop
