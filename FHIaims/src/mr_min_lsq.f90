...
!     mr_min_lsq
!  SYNOPSIS
      subroutine  mr_min_lsq (n, x, y, npar, par)
...
! initialize the parameter to get started
      ...
      if(use_scgw .or. use_scgw0.or.use_dmft_gw)then
         fit_acc = 1.d-10
      else 
         fit_acc = 1.d-15
      endif

      call initialize_fitting_parameter  &
             (n, x, y, npar, par)   
...
!                 chisq,  " is achieved after ", i_cycle,  " iterations."
          ...
          elseif (i_cycle .gt. n_max_cycle) then
            ...
            write(use_unit,'(A,e16.3,A,I6,A)') "  Self-energy fitting with accuary  ", &
                 fit_acc,  " is NOT achieved after ", i_cycle,  " iterations."
...
!      enddo
!      write(use_unit,*)
      end subroutine mr_min_lsq
!****************
!****s* FHI-aims/mr_min_lsq
!  NAME
!     mr_min_lsq
!  SYNOPSIS
      subroutine  mr_min_lsq_re (n, x, y, npar, par)
...
!    Local variables
     ...
     real*8  :: dydp1, dydp2 
      ...
      real*8 :: coeff_matr(npar,npar) 
      real*8 :: adapted_coeff_matr(npar,npar) 
      real*8 :: c_vect(npar) 
      real*8 :: delta_par(npar) 
...
! initialize the parameter to get started
      ...
      call initialize_fitting_parameter_re  &
             (n, x, y, npar, par)   

      call evaluate_fitting_function_W  &
           (n, x, npar, par, yexp, dydp)   
...
!   to be solved in Gauss-Newton method (or Levenberg-Marquardt method).
           ...
           do i_par = 1, npar, 1
             ...
             c_vect(i_par) = c_vect(i_par) + dy*dydp1  
...
! on exit, delta_par(:) gives  the change of the parameters
            delta_par(:) = c_vect(:)
            call dgesv(npar,1,adapted_coeff_matr,npar,ipiv,delta_par,npar,info)

! update the the parameters
            do i_par = 1, npar 
              par_exp(i_par) = par(i_par)+ delta_par(i_par)
...
!                 chisq,  " is achieved after ", i_cycle,  " iterations."
          ...
          elseif (i_cycle .gt. n_max_cycle) then
            converg = .true.
            write(use_unit,'(A,e16.3,A,I6,A)') "  Self-energy fitting with accuracy  ", &
                 fit_acc,  " is NOT achieved after ", i_cycle,  " iterations."
            write(use_unit,'(A,e16.3)') " The accuracy achieved currently is ", &
                 abs(chisq-chisq_old)  
          endif 
