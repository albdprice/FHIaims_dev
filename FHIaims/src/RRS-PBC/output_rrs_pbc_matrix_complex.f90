!****s* FHI-aims/RRS-PBC/output_rrs_pbc_matrix_complex
!  NAME
!   output_rrs_pbc_matrix_complex
!  SYNOPSIS

    subroutine output_rrs_pbc_matrix_complex(n_basis_1,n_basis_2,l_matrix,p_matrix)
!  USES
    use dimensions
    use runtime_choices
    use localorb_io
...
!  SOURCE
    ...
    complex*16  :: p_matrix(l_matrix)
    complex*16  :: o_matrix(n_basis_1, n_basis_2)
    integer :: i, j, o, p
    integer :: i_center
    integer :: start_cell
    integer :: n
    ...
    do i=1,n_basis_1,1
        do j=1,i,1
            ...
            o_matrix(j,i)=conjg(o_matrix(i,j))
        enddo
    enddo
        ...
        do i=1, n, 1
            ...
            do o=1,n_basis_2,1
                write(rrs_pbc_info,'(4X,A,I5,A,5(F16.8))') '| ',&
                    o,'R',(real(o_matrix(j,o)), &
                    j=start_cell,start_cell+4)
                call localorb_info(rrs_pbc_info)
                !write(rrs_pbc_info,'(4X,A,I5,A,5(F16.8))') '| ',&
                !    o,'I',(aimag(o_matrix(j,o)), &
                !    j=start_cell,start_cell+4)
                !call localorb_info(rrs_pbc_info)
            enddo
    ...
    if (m .eq. 1) then
        ...
        do o=1,n_basis_2,1
            write(rrs_pbc_info,'(4X,A,I5,A,F16.8)') '| ',&
                o,'R',real(o_matrix(start_cell,o))
            call localorb_info(rrs_pbc_info)
            !write(rrs_pbc_info,'(4X,A,I5,A,F16.8)') '| ',&
            !    o,'I',aimag(o_matrix(start_cell,o))
            !call localorb_info(rrs_pbc_info)
        enddo
    elseif (m .eq. 2) then
