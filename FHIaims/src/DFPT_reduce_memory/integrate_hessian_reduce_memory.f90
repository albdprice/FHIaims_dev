...
subroutine integrate_hessian_reduce_memory &
...
!  ARGUMENTS
  ...
  real*8, target, dimension(n_full_points) ::v_hartree_gradient_std
...
! VB - renewed index infrastructure starts here
  ...
  integer j_coord_new,j_atom_new
  integer i_division
...
!  end if !-----shanghui always calcualet gradients-------------------------
  ...
  allocate(first_order_DM_rho( n_max_batch_size), STAT=info )
...
!--------------initialize--------------------------------
  !--------inside vars, can be initialize to 0-----------
  ionic_hessian(1:3, 1:n_atoms, 1:3, 1:n_atoms) = 0.0d0
  hellman_feynman_hessian(1:3,1:n_atoms,1:3,1:n_atoms)=0.0d0
  pulay_hessian(1:3,1:n_atoms,1:3,1:n_atoms)=0.0d0

  !-------outside vars, only initialize some part to 0----
  gradient_dipole(3*j_atom+j_coord-3,1:3) = 0.0d0
           ...
           if (i_atom .ne. j_atom) then
                 ...
                 do i_coord = 1, 3, 1 
                   ...
                   !ionic_hessian(i_coord, i_atom, i_coord, j_atom)
                    ...
                    ionic_hessian(i_coord, j_atom, i_coord, j_atom) =   &
                    ionic_hessian(i_coord, j_atom, i_coord, j_atom) -   &
                    species_z(species(i_atom)) * species_z(species(j_atom))   &
                    /(distance_squared ** 1.5d0)                        & 
                    +3.0d0*direction(i_coord)*direction(j_coord)        & 
                    ...
                    *species_z(species(i_atom)) * species_z(species(j_atom))   &
                    ...
                    ionic_hessian(i_coord, j_atom, j_coord, j_atom) =   &
                    ionic_hessian(i_coord, j_atom, j_coord, j_atom)    &
                    +3.0d0*direction(i_coord)*direction(j_coord)        & 
                    *species_z(species(i_atom)) * species_z(species(j_atom))   &
                    /(distance_squared ** 2.5d0)
     ...
     if (n_compute_c.gt.0) then
        ...
        do i_index = 1, batches_work(i_my_batch)%size, 1
           ...
           if (partition_tab(i_full_points).gt.0.d0) then
              ...
              v_hartree_gradient_std(i_full_points)
...
!-------------------------shanghui add for first_H--------------------------- 
             call  evaluate_first_order_rho_reduce_memory( &
                   n_points,n_occ_states,       &
                   KS_ev_compute,n_compute_c, i_basis, & 
                   wave,gradient_basis_wave_npoints, &
                   density_matrix,first_order_density_matrix,first_order_DM_rho,local_first_order_rho, &
                   j_atom, j_coord)
...
!-------------------------shanghui use local_first_order_rho to get gradient_diple_electron----------------
       ...
       if (partition_tab(i_full_points_first_order_rho).gt.0.d0) then
           ...
           i_point = i_point+1
             ...
             gradient_dipole(3*j_atom+j_coord-3,i_coord)=gradient_dipole(3*j_atom+j_coord-3,i_coord) &
            -partition_tab(i_full_points_first_order_rho)  &
            *local_first_order_rho(i_point)*coord_current(i_coord) 
...
!------------------------shanghui end use local_first_order_rho to get gradient_diple_electron----------------
        ...
        do i_spin = 1, n_spin, 1
          call  evaluate_hellman_feynman_hessian_fixed_reduce_memory(hellman_feynman_hessian,        &
                n_points,partition, dist_tab_backup, dir_tab_backup,       &
                local_rho(i_spin,1:n_points),local_first_order_rho,j_atom,j_coord)


          call  evaluate_pulay_hessian_fixed_reduce_memory(pulay_hessian, &
    ...
    if(.not. use_local_index) call sync_integrate_hellman_feynman_hessian( pulay_hessian )
    ...
    if (module_is_debugged("DFPT")) then 
      ...
      do i_coord = 1 , 3
         ...
         write(use_unit,'(100E20.12)')  & 
             (((hellman_feynman_hessian(i_coord,i_atom,j_coord_new,j_atom_new)+ &
               ionic_hessian(i_coord,i_atom,j_coord_new,j_atom_new)+ & 
               pulay_hessian(i_coord,i_atom,j_coord_new,j_atom_new))* &
                 hartree / (bohr*bohr) , j_coord_new=1,3) ,j_atom_new=1,n_atoms)
      enddo
      enddo 
      endif 
    endif 
...
!      end do ! end loop over batches
  ...
  !-------shanghui begin parallel------
  if(.not. use_local_index) call sync_vector( gradient_dipole(3*j_atom+j_coord-3,1:3),3 )
  !-------shanghui end parallel--------
...
end subroutine integrate_hessian_reduce_memory
!******
