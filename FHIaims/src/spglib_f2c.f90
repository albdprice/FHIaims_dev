   ...
   !RJM moved this type to the top of the module to
   !circumvent a compiler bug in gfortran 4.3 to 4.6 
...
!   Beware the map refers to positions starting at 0      
   ...
   function spg_get_ir_reciprocal_mesh_sym(grid_point, map, map_sym, mesh, &
      & is_shift, is_time_reversal, lattice, position, types, num_atom, symprec) bind(c)
      import c_int, c_double
!   Beware the map refers to positions starting at 0      
      integer(c_int), intent(out) :: grid_point(3, *), map(*), map_sym(*) ! size is product(mesh)
      integer(c_int), intent(in) :: mesh(3), is_shift(3)
      integer(c_int), intent(in), value :: is_time_reversal
      real(c_double), intent(in) :: lattice(3,3), position(3, *)
      integer(c_int), intent(in) :: types(*)
      integer(c_int), intent(in), value :: num_atom
      real(c_double), intent(in), value :: symprec   
      integer(c_int) :: spg_get_ir_reciprocal_mesh_sym ! the number of points in the reduced mesh
   end function spg_get_ir_reciprocal_mesh_sym
...
!   Beware the map refers to positions starting at 0     
   ...
   public :: SpglibDataset, spg_get_dataset, &
      ...
      & spg_get_ir_kpoints, spg_get_ir_reciprocal_mesh, spg_get_ir_reciprocal_mesh_sym, spg_get_stabilized_reciprocal_mesh, &
   ...
   function spg_get_dataset(lattice, position, types, num_atom, symprec) result(dset)
      ...
      ! Aims indexing of atoms starts with 1      
      dset % equivalent_atoms = 1
      dset % equivalent_atoms = dset % equivalent_atoms + equivalent_atoms
