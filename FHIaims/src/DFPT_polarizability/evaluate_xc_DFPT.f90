!****s* FHI-aims/evaluate_xc_DFPT
!  NAME
!  evaluate_xc_DFPT : this is for both LDA and GGA, while evaluate_xc_shanghui is for LDA.
!  SYNOPSIS
      subroutine evaluate_xc_DFPT &
      ( rho, rho_gradient, en_density_xc, &
        en_density_x, en_density_c, &
        local_xc_derivs, xc_gradient_deriv,dVxc_drho, &
...
!
     ...
     !------------(2) shanghui add for libxc---------------------
      use tools,           only: libxc_yes_no
      use xc_f90_types_m
      use xc_f90_lib_m 
     !------------(2) shanghui end add for libxc-----------------
...
!  ARGUMENTS
      ...
      real*8  dVxc_drho(3) ! = v2rho2, need to be remove 
      real*8  vrho(n_spin), vsigma(3), v2rho2(3)
      real*8  v2rhosigma(6), v2sigma2(6)
...
!     counters
     ...
     !-------------(1) shanghui add for DFT-repository-------
      integer ideriv, npt, i_coord
     ...
     !-------------(1) shanghui end add for DFT-repository test-------
     ...
     !-------------(2) shanghui add for libxc---------------------
      ...
      real*8  vx(n_spin),fx(3)  
      real*8  vc(n_spin),fc(3)
    
      real*8 sigma(3) 
      real*8 vsigma_x(3), v2rho2_x(3), & 
             v2rhosigma_x(6), v2sigma2_x(6)
      real*8 vsigma_c(3), v2rho2_c(3), & 
             v2rhosigma_c(6), v2sigma2_c(6)
      ...
      !call xcpot_pz_lda_DFPT(rho, en_density_xc, en_density_x, & 
      !                   en_density_c,  &
      !                   local_xc_derivs, dVxc_drho(1:n_spin) )
      !write(use_unit,*) 'aims:',rho,en_density_xc,local_xc_derivs,dVxc_drho
      !stop
...
!  begin work
       ...
       !----here decide if using libxc, this function is from Raul Laasner's MagneticResponse code. 
       ! libxc_yes_no() returns .true. if USE_LIBXC=yes or LIBXC_DIR are present in make.sys and
       !                        .false. otherwise (that includes any version of libxc, not just 3.0.0)
       using_libxc =  libxc_yes_no()

       !write(use_unit,*) 'test using_libxc:',using_libxc

      !---------shanghui begin initial ----
        ! here I test if this array tranfer right or not. In fact, I can direcly
        ! use dVxc_drho(:) = 0.0d0, however, by the following way, I could know
        ! if the variables have been passed in correctly or not. 
           sigma(1:3) = 0.0d0
           dVxc_drho(1:3) = 0.0d0
           vrho(1:n_spin) = 0.0d0 
           vsigma(1:3) = 0.0d0
           v2rho2(1:3) = 0.0d0 
           v2rhosigma(1:6) = 0.0d0 
           v2sigma2(1:6) = 0.0d0 
      !---------shanghui end initial ----
...
!           MR: Following the PBE evaluation for DFPT, but now with hybrid coeff
...
!              xc_gradient_deriv )
           ...
           if(n_spin.eq.1) then ! closed shell 
            ...
            if(.not.using_libxc) then 
             ...
             call rks_x_pbe                                         &
             (ideriv,npt,rhoa,sigma_aa,                            &
              zk_x,vrhoa_x,vsigma_aa_x,                             &
              v2rhoa2_x,v2rhoasigma_aa_x,v2sigma_aa2_x) 
             !-----correction. ref: LuTian--------
              vsigma_aa_x = vsigma_aa_x/4.0d0
              v2rhoasigma_aa_x = v2rhoasigma_aa_x/4.0d0
              v2rhoa2_x = v2rhoa2_x/2.0d0
              v2sigma_aa2_x = v2sigma_aa2_x/16.0d0            
             
             call rks_c_pbe                                         &
             (ideriv,npt,rhoa,sigma_aa,                            &
              zk_c,vrhoa_c,vsigma_aa_c,                             &
              v2rhoa2_c,v2rhoasigma_aa_c,v2sigma_aa2_c) 
             !-----correction. ref: LuTian--------
              vsigma_aa_c = vsigma_aa_c/4.0d0
              v2rhoasigma_aa_c = v2rhoasigma_aa_c/4.0d0
              v2rhoa2_c = v2rhoa2_c/2.0d0
              v2sigma_aa2_c = v2sigma_aa2_c/16.0d0            

             !------(0) energy density: exc----------------
             en_density_x(1) = (1-hybrid_coeff)*zk_x/rhoa
             en_density_c = zk_c/rhoa

             !------(1.1) d exc/d rho---------------------- 
             local_xc_derivs =  (1-hybrid_coeff)*vrhoa_x + vrhoa_c

             !------(1.2) d exc/d gradient_rho---------
             xc_gradient_deriv(1,1) = (1-hybrid_coeff)*vsigma_aa_x*rho_gradient(1,1) & 
                                    + vsigma_aa_c*rho_gradient(1,1) 
             xc_gradient_deriv(2,1) = (1-hybrid_coeff)*vsigma_aa_x*rho_gradient(2,1) & 
                                    + vsigma_aa_c*rho_gradient(2,1) 
             xc_gradient_deriv(3,1) = (1-hybrid_coeff)*vsigma_aa_x*rho_gradient(3,1) & 
                                    + vsigma_aa_c*rho_gradient(3,1) 

            vrho(1) = (1-hybrid_coeff)*vrhoa_x + vrhoa_c 
            vsigma(1) = (1-hybrid_coeff)*vsigma_aa_x + vsigma_aa_c          
            v2rho2(1) = (1-hybrid_coeff)*v2rhoa2_x + v2rhoa2_c 
            v2rhosigma(1) = (1-hybrid_coeff)*v2rhoasigma_aa_x + v2rhoasigma_aa_c
            v2sigma2(1) = (1-hybrid_coeff)*v2sigma_aa2_x + v2sigma_aa2_c 
            ...
            func_id=101
            call xc_f90_func_init(xc_func, xc_info, func_id, XC_UNPOLARIZED)
            call xc_f90_gga_vxc_fxc(xc_func, 1, rho(1), sigma(1),vx(1), &  
                 vsigma_x(1), v2rho2_x(1),v2rhosigma_x(1), v2sigma2_x(1))
            call xc_f90_func_end(xc_func)
            func_id=130
            call xc_f90_func_init(xc_func, xc_info, func_id, XC_UNPOLARIZED)
            call xc_f90_gga_vxc_fxc(xc_func, 1, rho(1), sigma(1),vc(1), &  
                 vsigma_c(1), v2rho2_c(1),v2rhosigma_c(1), v2sigma2_c(1))
            call xc_f90_func_end(xc_func)

            vrho(1) = (1-hybrid_coeff)*vx(1) + vc(1) 
            vsigma(1) = (1-hybrid_coeff)*vsigma_x(1) + vsigma_c(1)          
            v2rho2(1) = (1-hybrid_coeff)*v2rho2_x(1) + v2rho2_c(1) 
            v2rhosigma(1) = (1-hybrid_coeff)*v2rhosigma_x(1) + v2rhosigma_c(1)
            v2sigma2(1) = (1-hybrid_coeff)*v2sigma2_x(1) + v2sigma2_c(1) 
           ...
           if(.not.using_libxc) then 
            ...
            (ideriv,npt,rhoa,rhob,sigma_aa,sigma_bb,sigma_ab,       &
             ...
             !------(0) energy density: exc----------------
             en_density_x(1) = (1-hybrid_coeff)*zk_x/rhoa
             en_density_x(2) = (1-hybrid_coeff)*zk_x/rhob
             en_density_c = zk_c/rhoa

             !------(1.1) d exc/d rho---------------------- 
             local_xc_derivs(1) =  (1-hybrid_coeff)*vrhoa_x + vrhoa_c
             local_xc_derivs(2) =  (1-hybrid_coeff)*vrhob_x + vrhob_c

             !------(1.2) d exc/d gradient_rho---------
             do i_coord = 1, 3
             xc_gradient_deriv(i_coord,1) = & 
                               (1-hybrid_coeff)*vsigma_aa_x*rho_gradient(i_coord,1) & 
                             + vsigma_aa_c*rho_gradient(i_coord,1) &
                      + 0.5d0*((1-hybrid_coeff)*vsigma_ab_x*rho_gradient(i_coord,2) & 
                             + vsigma_ab_c*rho_gradient(i_coord,2))

             xc_gradient_deriv(i_coord,2) = & 
                               (1-hybrid_coeff)*vsigma_bb_x*rho_gradient(i_coord,2) & 
                             + vsigma_bb_c*rho_gradient(i_coord,2) &
                      + 0.5d0*((1-hybrid_coeff)*vsigma_ab_x*rho_gradient(i_coord,1) & 
                             + vsigma_ab_c*rho_gradient(i_coord,1))
             enddo


           !---------Here we follow libxc's defination------------------
           ! if nspin == 2 
           !rho(2)          = (u, d)
           !sigma(3)        = (uu, ud, dd)
           ...
           !vsigma(3)       = (uu, ud, dd)
            vrho(1) =(1-hybrid_coeff)* vrhoa_x + vrhoa_c 
            vrho(2) = (1-hybrid_coeff)*vrhob_x + vrhob_c 
            vsigma(1) = (1-hybrid_coeff)*vsigma_aa_x + vsigma_aa_c          
            vsigma(2) = (1-hybrid_coeff)*vsigma_ab_x + vsigma_ab_c          
            vsigma(3) = (1-hybrid_coeff)*vsigma_bb_x + vsigma_bb_c          
           ...
           !v2sigma2(6)     = (uu_uu, uu_ud, uu_dd, ud_ud, ud_dd, dd_dd)
            v2rho2(1) = (1-hybrid_coeff)*v2rhoa2_x + v2rhoa2_c 
            v2rho2(2) = (1-hybrid_coeff)*v2rhoab_x + v2rhoab_c
            v2rho2(3) = (1-hybrid_coeff)*v2rhob2_x + v2rhob2_c
            v2rhosigma(1) = (1-hybrid_coeff)*v2rhoasigma_aa_x + v2rhoasigma_aa_c
            v2rhosigma(2) = (1-hybrid_coeff)*v2rhoasigma_ab_x + v2rhoasigma_ab_c
            v2rhosigma(3) = (1-hybrid_coeff)*v2rhoasigma_bb_x + v2rhoasigma_bb_c
            v2rhosigma(4) = (1-hybrid_coeff)*v2rhobsigma_aa_x + v2rhobsigma_aa_c
            v2rhosigma(5) = (1-hybrid_coeff)*v2rhobsigma_ab_x + v2rhobsigma_ab_c
            v2rhosigma(6) = (1-hybrid_coeff)*v2rhobsigma_bb_x + v2rhobsigma_bb_c

            v2sigma2(1) = (1-hybrid_coeff)*v2sigma_aa2_x + v2sigma_aa2_c 
            v2sigma2(2) = (1-hybrid_coeff)*v2sigma_aaab_x + v2sigma_aaab_c
            v2sigma2(3) = (1-hybrid_coeff)*v2sigma_aabb_x + v2sigma_aabb_c
            v2sigma2(4) = (1-hybrid_coeff)*v2sigma_ab2_x + v2sigma_ab2_c
            v2sigma2(5) = (1-hybrid_coeff)*v2sigma_abbb_x + v2sigma_abbb_c
            v2sigma2(6) = (1-hybrid_coeff)*v2sigma_bb2_x + v2sigma_bb2_c
           !-------------shanghui end DFT_repository-------------------
            ...
            sigma(1) =  rho_gradient(1,1)*rho_gradient(1,1)+  &
                        rho_gradient(2,1)*rho_gradient(2,1)+  &
                        rho_gradient(3,1)*rho_gradient(3,1)
            sigma(2) =  rho_gradient(1,2)*rho_gradient(1,2)+  &
                        rho_gradient(2,2)*rho_gradient(2,2)+  &
                        rho_gradient(3,2)*rho_gradient(3,2)
            sigma(3) =  rho_gradient(1,1)*rho_gradient(1,2)+  &
                        rho_gradient(2,1)*rho_gradient(2,2)+  &
                        rho_gradient(3,1)*rho_gradient(3,2)
            ...
            call xc_f90_func_end(xc_func)
             ...
             local_xc_derivs(1:2) =  (1-hybrid_coeff)*vx(1:n_spin) + vc(1:n_spin)

             do i_coord = 1, 3
             xc_gradient_deriv(i_coord,1) = & 
                               (1-hybrid_coeff)*vsigma_x(1)*rho_gradient(i_coord,1) & 
                             + vsigma_c(1)*rho_gradient(i_coord,1) &
                      + 0.5d0*((1-hybrid_coeff)*vsigma_x(2)*rho_gradient(i_coord,2) & 
                             + vsigma_c(2)*rho_gradient(i_coord,2))

             xc_gradient_deriv(i_coord,2) = & 
                               (1-hybrid_coeff)*vsigma_x(3)*rho_gradient(i_coord,2) & 
                             + vsigma_c(3)*rho_gradient(i_coord,2) &
                      + 0.5d0*((1-hybrid_coeff)*vsigma_x(2)*rho_gradient(i_coord,1) & 
                             + vsigma_c(2)*rho_gradient(i_coord,1))
             enddo

             vrho(1:n_spin) = (1-hybrid_coeff)*vx(1:n_spin) + vc(1:n_spin) 
             vsigma(1:3) = (1-hybrid_coeff)*vsigma_x(1:3) + vsigma_c(1:3)          
             v2rho2(1:3) = (1-hybrid_coeff)*v2rho2_x(1:3) + v2rho2_c(1:3) 
             v2rhosigma(1:6) = (1-hybrid_coeff)*v2rhosigma_x(1:6) + v2rhosigma_c(1:6)
             v2sigma2(1:6) = (1-hybrid_coeff)*v2sigma2_x(1:6) + v2sigma2_c(1:6) 
...
!           Perdew-Zunger LDA
           ...
           if(n_spin.eq.1) then ! closed shell             
            ...
            if(.not.using_libxc) then 
              ...
              func_id=1
              call xc_f90_func_init(xc_func, xc_info, func_id, XC_UNPOLARIZED)
              call xc_f90_lda_vxc_fxc(xc_func, 1, rho(1),vx(1),fx(1))
              call xc_f90_func_end(xc_func)
              func_id=9
              call xc_f90_func_init(xc_func, xc_info, func_id, XC_UNPOLARIZED)
              call xc_f90_lda_vxc_fxc(xc_func, 1, rho(1),vc(1),fc(1))
              call xc_f90_func_end(xc_func)
             
              local_xc_derivs(1) = vx(1) + vc(1) 
              dVxc_drho(1) = fx(1) + fc(1)               
           ...
           !-------------shanghui end DFT_repository-------------------
            ...
            (ideriv,npt,rhoa,rhob,sigma_aa,sigma_bb,sigma_ab,       &
             ...
             !------(0) energy density: exc----------------
             en_density_x(1) = zk_x/rhoa
             en_density_x(2) = zk_x/rhob
             en_density_c    = zk_c/rhoa

             !------(1) d exc/d rho---------------------- 
             local_xc_derivs(1) =  vrhoa_x + vrhoa_c
             local_xc_derivs(2) =  vrhob_x + vrhob_c
          
             !------(2) d^2 exc/d rho^2 = fxc 
             ! http://www.tddft.org/programs/octopus/wiki/index.php/Libxc:manual
             dVxc_drho(1) = v2rhoa2_x + v2rhoa2_c
             dVxc_drho(2) = v2rhoab_x + v2rhoab_c 
             dVxc_drho(3) = v2rhob2_x + v2rhob2_c 
           !-------------shanghui end DFT_repository-------------------
              ...
              local_xc_derivs(1:n_spin) = vx(1:n_spin) + vc(1:n_spin) 
              dVxc_drho(1:3) = fx(1:3) + fc(1:3)               
...
!           Johan has already supplied that subroutine. Need to integrate it here.
           ...
           !--------begin compare with DFPT_repository--------------------
           ! rho(1)=0.1d0
           ! rho_gradient(1,1)=0.1d0 
           ! rho_gradient(2,1)=0.2d0
           ! rho_gradient(3,1)=0.3d0     
           ...
           !--------end compare with DFPT_repository--------------------
           ...
           if(n_spin.eq.1) then ! closed shell 
            ...
            if(.not.using_libxc) then 
             ...
             !------(1.1) d exc/d rho---------------------- 
             local_xc_derivs =  vrhoa_x + vrhoa_c

             !------(1.2) d exc/d gradient_rho---------
             xc_gradient_deriv(1,1) = vsigma_aa_x*rho_gradient(1,1) & 
                                    + vsigma_aa_c*rho_gradient(1,1) 
             xc_gradient_deriv(2,1) = vsigma_aa_x*rho_gradient(2,1) & 
                                    + vsigma_aa_c*rho_gradient(2,1) 
             xc_gradient_deriv(3,1) = vsigma_aa_x*rho_gradient(3,1) & 
                                    + vsigma_aa_c*rho_gradient(3,1) 

            vrho(1) = vrhoa_x + vrhoa_c 
            vsigma(1) = vsigma_aa_x + vsigma_aa_c          
            v2rho2(1) = v2rhoa2_x + v2rhoa2_c 
            v2rhosigma(1) = v2rhoasigma_aa_x + v2rhoasigma_aa_c
            v2sigma2(1) = v2sigma_aa2_x + v2sigma_aa2_c 
            ...
            vrho(1) = vx(1) + vc(1) 
            vsigma(1) = vsigma_x(1) + vsigma_c(1)          
            v2rho2(1) = v2rho2_x(1) + v2rho2_c(1) 
            v2rhosigma(1) = v2rhosigma_x(1) + v2rhosigma_c(1)
            v2sigma2(1) = v2sigma2_x(1) + v2sigma2_c(1) 
           ...
           if(.not.using_libxc) then 
            ...
            (ideriv,npt,rhoa,rhob,sigma_aa,sigma_bb,sigma_ab,       &
             ...
             do i_coord = 1, 3
             xc_gradient_deriv(i_coord,1) = & 
                               vsigma_aa_x*rho_gradient(i_coord,1) & 
                             + vsigma_aa_c*rho_gradient(i_coord,1) &
                      + 0.5d0*(vsigma_ab_x*rho_gradient(i_coord,2) & 
                             + vsigma_ab_c*rho_gradient(i_coord,2))

             xc_gradient_deriv(i_coord,2) = & 
                               vsigma_bb_x*rho_gradient(i_coord,2) & 
                             + vsigma_bb_c*rho_gradient(i_coord,2) &
                      + 0.5d0*(vsigma_ab_x*rho_gradient(i_coord,1) & 
                             + vsigma_ab_c*rho_gradient(i_coord,1))
             enddo

           !-----begin compare with aims---------------------
           !rho(1)=0.1d0
           !rho(2)=0.1d0
           !rho_gradient(1,1)=0.1d0 
           !rho_gradient(2,1)=0.2d0
           !rho_gradient(3,1)=0.3d0     
           !rho_gradient(1,2)=0.1d0 
           !rho_gradient(2,2)=0.2d0
           !rho_gradient(3,2)=0.3d0     
           !
           !call xc_partials_pbe &
           !( rho, rho_gradient, en_density_xc,  en_density_x, &
           !  en_density_c, local_xc_derivs, xc_gradient_deriv )
           ...
           !          local_xc_derivs,xc_gradient_deriv 
           !!aims-PBE
           !!  0.1000000000   0.1000000000  -0.5103836810  -0.5103836810  -0.0179282476  -0.5796076077  -0.5796076077
           !! -0.0031925558  -0.0063851116  -0.0095776675  -0.0031925558  -0.0063851116  -0.0095776675
           !!aims-PBE-DFT-repository
           !!  0.1000000000   0.1000000000  -0.5103836810  -0.5103836810  -0.0179282476  -0.5796076077  -0.5796076077
           !! -0.0031925558  -0.0063851116  -0.0095776675  -0.0031925558  -0.0063851116  -0.0095776675
           !! This test has been passed.
           !stop
           !-----end compare with aims---------------------
           ...
           !vsigma(3)       = (uu, ud, dd)
            vrho(1) = vrhoa_x + vrhoa_c 
            vrho(2) = vrhob_x + vrhob_c 
            vsigma(1) = vsigma_aa_x + vsigma_aa_c          
            vsigma(2) = vsigma_ab_x + vsigma_ab_c          
            vsigma(3) = vsigma_bb_x + vsigma_bb_c          
           ...
           !v2sigma2(6)     = (uu_uu, uu_ud, uu_dd, ud_ud, ud_dd, dd_dd)
            v2rho2(1) = v2rhoa2_x + v2rhoa2_c 
            v2rho2(2) = v2rhoab_x + v2rhoab_c
            v2rho2(3) = v2rhob2_x + v2rhob2_c
            v2rhosigma(1) = v2rhoasigma_aa_x + v2rhoasigma_aa_c
            v2rhosigma(2) = v2rhoasigma_ab_x + v2rhoasigma_ab_c
            v2rhosigma(3) = v2rhoasigma_bb_x + v2rhoasigma_bb_c
            v2rhosigma(4) = v2rhobsigma_aa_x + v2rhobsigma_aa_c
            v2rhosigma(5) = v2rhobsigma_ab_x + v2rhobsigma_ab_c
            v2rhosigma(6) = v2rhobsigma_bb_x + v2rhobsigma_bb_c

            v2sigma2(1) = v2sigma_aa2_x + v2sigma_aa2_c 
            v2sigma2(2) = v2sigma_aaab_x + v2sigma_aaab_c
            v2sigma2(3) = v2sigma_aabb_x + v2sigma_aabb_c
            v2sigma2(4) = v2sigma_ab2_x + v2sigma_ab2_c
            v2sigma2(5) = v2sigma_abbb_x + v2sigma_abbb_c
            v2sigma2(6) = v2sigma_bb2_x + v2sigma_bb2_c
           !-------------shanghui end DFT_repository-------------------
            ...
            call xc_f90_func_end(xc_func)
             ...
             local_xc_derivs(1:2) =  vx(1:n_spin) + vc(1:n_spin)

             do i_coord = 1, 3
             xc_gradient_deriv(i_coord,1) = & 
                               vsigma_x(1)*rho_gradient(i_coord,1) & 
                             + vsigma_c(1)*rho_gradient(i_coord,1) &
                      + 0.5d0*(vsigma_x(2)*rho_gradient(i_coord,2) & 
                             + vsigma_c(2)*rho_gradient(i_coord,2))

             xc_gradient_deriv(i_coord,2) = & 
                               vsigma_x(3)*rho_gradient(i_coord,2) & 
                             + vsigma_c(3)*rho_gradient(i_coord,2) &
                      + 0.5d0*(vsigma_x(2)*rho_gradient(i_coord,1) & 
                             + vsigma_c(2)*rho_gradient(i_coord,1))
             enddo

             vrho(1:n_spin) = vx(1:n_spin) + vc(1:n_spin) 
             vsigma(1:3) = vsigma_x(1:3) + vsigma_c(1:3)          
             v2rho2(1:3) = v2rho2_x(1:3) + v2rho2_c(1:3) 
             v2rhosigma(1:6) = v2rhosigma_x(1:6) + v2rhosigma_c(1:6)
             v2sigma2(1:6) = v2sigma2_x(1:6) + v2sigma2_c(1:6) 
...
!           Perdew-Wang 1991 LDA
            call xcpot_pw91_lda_DFPT(rho,en_density_xc,en_density_x, & 
                 en_density_c, & 
...
!             endif
      ...
      end subroutine evaluate_xc_DFPT
!---------------------------------------------------------------------
!******

 

  subroutine xcpot_pz_lda_DFPT &
  ...
  end subroutine xcpot_pz_lda_DFPT


  subroutine xcpot_pw91_lda_DFPT &
  ...
  end subroutine xcpot_pw91_lda_DFPT
