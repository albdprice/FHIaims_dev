!****s* FHI-aims/integrate_first_order_rho_polarizability
!  NAME
!   integrate_first_order_rho_polarizability
!  SYNOPSIS

subroutine integrate_first_order_rho_polarizability &
...
!  ARGUMENTS
  ...
  !shanghui------------------------------------------------------------------------
  real*8,  dimension(3, n_basis,n_basis), intent(in) :: first_order_density_matrix
  real*8,  dimension(3, n_full_points), intent(out) ::  first_order_rho 
  !shanghui------------------------------------------------------------------------
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
 ...
 !------------shanghui add for first_order_rho-----------------
  ...
  allocate(local_first_order_rho(3,n_max_batch_size), STAT=info ) 
...
!----------------------------shanghui------------------------------------------             
            ...
            !endif  
             call  evaluate_first_order_rho_polarizability( &
                   n_points,      &
       ...
       do i_index = 1, batches_work(i_my_batch)%size, 1
          ...
          if (partition_tab(i_full_points_DM_rho).gt.0.d0) then
              ...
              do i_coord=1,3
                 first_order_rho(i_coord,i_full_points_DM_rho)= & 
                 local_first_order_rho(i_coord,i_point) 
              enddo
...
end subroutine integrate_first_order_rho_polarizability
!******
