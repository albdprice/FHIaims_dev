!****s* FHI-aims/evaluate_mp2_selfenergy
!  NAME
!   evaluate_mp2_selfenergy
!  SYNOPSIS

      subroutine evaluate_mp2_selfenergy &
...
!     output
      complex*16   mp2_selfenergy(n_freq,n_KS_states,n_spin)
...
! OUTPUT
! o  mp2_self_energy -- complex array
...
!    determine the HOMO and LUMO level
      ...
      do i_spin = 1, n_spin
       do i_state = 1, n_states
         if(occ_numbers(i_state,i_spin,1).gt.1.d-3) then
...
!  endif myid
      ...
      call get_timestamps(time_mp2, clock_time_mp2 )
...
!                  if (i_state.eq.i_state_2 .or.i_virt.eq.i_virt2
!     +                .or. i_state.eq.i_virt
!     +                .or. i_state.eq.i_virt2) then
...
!                  if(i_state.ne.i_virt .and. i_state.ne.i_virt2) then
!                  if(i_state.ne.i_state_2) then
                  do i_freq = 1, n_freq
!                   if(myid.eq.0) then
...
!                   endif
                    ...
                    e_deno = (0.d0,1.d0)*omega(i_freq)  + &
                     ...
                     e_deno=  1.d0/e_deno
                     mp2_selfenergy (i_freq, i_state, n_spin) = &
                        mp2_selfenergy (i_freq, i_state, n_spin) + &
                        E_mp2_a*e_deno
...
!             end of loop over i_freq
                   enddo
                    e_deno = KS_eigenvalue(i_state,n_spin,1) + &
                             ...
                             KS_eigenvalue(i_virt,n_spin,1)- &
                             KS_eigenvalue(i_virt2,n_spin,1)
                    sigma(1) = sigma(1) + E_mp2_a/e_deno
!                  endif
...
!     +                 KS_eigenvalue(i_state,n_spin,1), real(sigma)
!                   endif
             enddo
              ...
              if(myid.eq.MOD(i_state_2,n_tasks)) then
               ...
               do i_occ = 1, n_homo(n_spin), 1
                do i_occ_2 = 1, n_homo(n_spin), 1
                  ...
                  E_mp2_a=aux_eri_2(i_occ, i_occ_2)

!                  if (i_occ.eq.i_state_2
!     +                .or. i_state.eq.i_virt
!     +               .or. i_state.eq.i_occ
!     +               .or. i_state.eq.i_state_2) then
!
!                     E_mp2_a= E_mp2_a* E_mp2_a
!                  else

                     E_mp2_b=aux_eri_2(i_occ_2,i_occ)
                     E_mp2_a= (2.d0*E_mp2_a-E_mp2_b)* E_mp2_a
!                  endif

!                  if(i_occ .ne. i_state .and. i_occ_2.ne.i_state) then
                  do i_freq = 1, n_freq
                    e_deno = (0.d0,1.d0)*omega(i_freq) + &
                             chemical_potential_spin(n_spin) + &
                             KS_eigenvalue(i_state_2,n_spin,1) - &
...
!                   if(i_occ .ne. i_state .and. i_occ_2.ne.i_state) then
                    e_deno = KS_eigenvalue(i_state,n_spin,1) + &
                             ...
                             KS_eigenvalue(i_occ,n_spin,1)- &
                             KS_eigenvalue(i_occ_2,n_spin,1)
                     sigma(2) = sigma(2) + E_mp2_a/e_deno
!                   endif
!                   if(i_occ .eq. i_state .or. i_occ_2.eq.i_state) then
!                    e_deno = KS_eigenvalue(i_state,n_spin,1) +
!     +                       1.d-6*(0.d0,1.d0)  +
!     +                       chemical_potential +
!     +                       KS_eigenvalue(i_state_2,n_spin,1) -
!     +                       KS_eigenvalue(i_occ,n_spin,1)-
!     +                       KS_eigenvalue(i_occ_2,n_spin,1)
!                     sigma(3) = sigma(3) + E_mp2_a/e_deno
!                     write(use_unit,'(4I4, 3f16.5)')
!     +                       i_state, i_occ, i_occ_2, i_state_2,
!     +                       real(e_deno), E_mp2_a,
!     +                       sigma(3)
!                   endif
...
!   end of loop over i_state_2
              enddo
!              write(use_unit,'(I6,5f12.5)') &
!                  i_state, KS_eigenvalue(i_state,n_spin,1)*hartree, &
!                  sigma(1) *hartree, sigma(2)*hartree, sigma(3)*hartree, &
!                  (sigma(1)+sigma(2)+sigma(3))*hartree
        ...
        do i_spin = 1, n_spin, 1
          do i_state = n_low_state, n_KS_states, 1
            ...
            do i_spin_2 = 1, n_spin, 1
                do i_state_2 = 1, n_homo(i_spin_2), 1
...
! MPI task distribution
               ...
               do i_virt = n_lumo(i_spin), n_states, 1
                do i_virt2 = n_lumo(i_spin_2), n_states, 1
...
!                        E_mp2_b=0.d0
                        ...
                        do i_freq = 1, n_freq, 1
                           e_deno = (0.d0,1.d0)*omega(i_freq)  + &
                              ...
                              KS_eigenvalue(i_state_2,i_spin_2,1) - &
                              KS_eigenvalue(i_virt,i_spin,1)- &
                              KS_eigenvalue(i_virt2,i_spin_2,1)
...
!                      write(use_unit,'(2X,3I4,4f16.6)') i_virt2, i_virt, i_spin_2,  E_mp2_a/e_deno, mp2_selfenergy(1,i_state,i_spin)
...
!   close j state2
              ...
              do i_state_2 = n_lumo(i_spin_2), n_states, 1
                 ...
                 call dgemm('T', 'N', n_homo(i_spin), &
                        ...
                        n_loc_prodbas, &
                        ovlp_3KS(:,i_state_2,1:n_homo(i_spin_2), &
                                 i_spin_2), &
                        n_loc_prodbas, 0.d0, &
                        aux_eri_2, n_homo_max &
                       )
                 ...
                 if(myid.eq.MOD(i_state_2,n_tasks)) then
                   ...
                   do i_occ = 1, n_homo(i_spin), 1
                     do i_occ_2 = 1, n_homo(i_spin_2), 1
                       ...
                       do i_freq = 1, n_freq
                         e_deno = (0.d0,1.d0)*omega(i_freq) + &
                               ...
                               KS_eigenvalue(i_occ_2,i_spin_2,1)
...
!  end of loop over i_spin2
         enddo
!         write(use_unit,*) mp2_selfenergy(1,i_state,i_spin), mp2_selfenergy(2,i_state,i_spin)
!   close i state
        enddo
       enddo
      ...
      if(use_mpi) then
        do i_spin = 1, n_spin, 1
          call sync_matrix_complex &
             (mp2_selfenergy(:,:,i_spin),n_freq,n_KS_states)

!          mp2_selfenergy(:,:,i_spin) = 0.5d0*
!     +       mp2_selfenergy(:,:,i_spin)
        enddo
      endif

      call get_times(time_mp2, clock_time_mp2)
      if(myid==0) then
        write(use_unit,*) '--------------------------------------------'
        write(use_unit,*)
        write(use_unit,'(10X,A,F12.3,A,F12.3,A)') &
            "| Total time MP2 selfenergy                      : ", &
             time_mp2, " s ",clock_time_mp2," s"
        write(use_unit,*)
      endif
...
!
!            if(n_spin.eq.2) then
!             write(filename,'(A,I0,A,I0,A)')"self_energy/Sigma.mp2.n_", &
!                        i_state,".s_",i_spin,".dat"
!            else
!            write(filename,'(A,I0,A)')"self_energy/Sigma.mp2.n_", &
!                        i_state,".dat"
!           endif
!
...
!                 omega(i_freq), mp2_selfenergy(i_freq,i_state,i_spin)
!            enddo
!
!            close(102)
!          enddo
...
!      endif
      ...
      end subroutine evaluate_mp2_selfenergy


!******
!---------------------------------------------------------------------------------------------------

      subroutine evaluate_mp2_selfenergy_2 &
       ( n_freq, n_low_state, n_KS_states, &
         omega, &
         ovlp_3KS, &
         chemical_potential_spin, &
...
!     Time counters
      ...
      real*8 time_mp2, clock_time_mp2
...
!    determine the HOMO and LUMO level
        ...
        do i_state = n_low_state, n_KS_states, 1
          ...
          do i_spin2 = 1, n_spin, 1
            ...
            do n_p1 = 0, np1_o3KS-1
              ...
              do i_state2 = 1, n_homo(i_spin2), 1
              ...
              do i_virt=n_lumo(i_spin),n_states,1
                ...
                do i_virt2=n_lumo(i_spin2),n_states,1
                  ...
                  if (i_spin.eq.i_spin2) then
                    ...
                    E_mp2_a= (2.d0/n_spin*E_mp2_a-E_mp2_b)* E_mp2_a
                  else
                    E_mp2_a=tmp_eri_full(i_virt_loc,i_virt2_loc,i_virt_own,i_state2_loc)
                    E_mp2_a=(E_mp2_a)*E_mp2_a
                  endif
            ...
            do n_p2 = 0, np2_o3KS-1
              ...
              do j = 1, max_homo_loc
                call dgemm('T', 'N', max_homo_loc, ndim1_o3KS-min_lumo_loc+1, n_basbas, 1.d0, &
                           tmp_o3KS, ubound(tmp_o3KS,1), &
                           ovlp_3KS(1,min_lumo_loc,j,i_spin2), ubound(ovlp_3KS,1), &
                           0.d0, tmp_prod, ubound(tmp_prod,1))
                
                ! Scatter the columns of the result into tmp_eri_full

                do i_state2 = n_lumo(i_spin2), n_states, 1
            ...
            do i_state2 = n_lumo(i_spin2), n_states, 1
              ...
              do i_occ = 1, n_homo(i_spin), 1
                ...
                do i_occ2 = 1, n_homo(i_spin2), 1
                  ...
                  if(i_spin .eq. i_spin2) then
                    E_mp2_a=tmp_eri_full(i_occ_loc,i_occ2_loc,i_occ_own,i_state2_loc)
                    E_mp2_b=tmp_eri_tran(i_occ2_loc,i_occ_loc,i_occ_own)
                    E_mp2_a= (2.d0/n_spin*E_mp2_a-E_mp2_b)*E_mp2_a
                  else
                    E_mp2_a=tmp_eri_full(i_occ_loc,i_occ2_loc,i_occ_own,i_state2_loc)
                    E_mp2_a= E_mp2_a*E_mp2_a
                  ...
                  do i_freq = 1, n_freq
                    e_deno = (0.d0,1.d0)*omega(i_freq) + &
                             ...
                             KS_eigenvalue(i_state2,i_spin2,1) - &
                             KS_eigenvalue(i_occ,i_spin,1)- &
                             KS_eigenvalue(i_occ2,i_spin2,1)
      ...
      end subroutine evaluate_mp2_selfenergy_2
