!****s*  FHI-aims/integrate_split_xc_matrix
!  NAME
!    integrate_split_xc_matrix
!  SYNOPSIS
      subroutine integrate_split_xc_matrix &
      ( partition_tab, &
        rho, rho_gradient, &
        kinetic_density, &
        basis_l_max, &
...
!   only referenced and allocated if gradient correction requested
      ...
      real*8 x_times_psi(n_basis, n_max_batch_size,n_spin)
      real*8 c_times_psi(n_basis, n_max_batch_size,n_spin)
...
!      real*8 :: local_xc_gradient_deriv(3,n_spin)
      real*8 :: local_x_density_deriv(n_spin)
      real*8 :: local_c_density_deriv(n_spin)
      real*8 :: local_x_gradient_deriv(3,n_spin)
      real*8 :: local_c_gradient_deriv(3,n_spin)
      real*8 :: local_x_tau_deriv(n_spin)
      real*8 :: local_c_tau_deriv(n_spin)
...
!     evaluate V_x*psi and V_c*psi in this particular point
                     ...
                     call evaluate_xc_split &
                      ( rho(1:n_spin,i_full_points), &
                        ...
                        local_x_density_deriv(1), &
                        local_c_density_deriv(1), &
                        local_x_gradient_deriv(1,1), &
                        local_c_gradient_deriv(1,1), &
                        local_x_tau_deriv(1), &
                        local_c_tau_deriv(1) &
                      )

                      do i_spin = 1, n_spin
                         x_times_psi (1:n_compute,i_point,i_spin)= &
                           local_x_density_deriv (i_spin) * &
                           wave(1:n_compute,i_point)

                         c_times_psi (1:n_compute,i_point,i_spin)= &
                           local_c_density_deriv (i_spin) * &
                           wave(1:n_compute,i_point)
                      enddo
                      ...
                      do i_spin = 1, n_spin, 1
                        do i_coord = 1, 3, 1
                          do i_compute = 1, n_compute, 1
                           ...
                           x_times_psi(i_compute, i_point, i_spin) = &
                            x_times_psi(i_compute, i_point, i_spin) + &
                            4.0d0*local_x_gradient_deriv(i_coord, i_spin) * &
                            gradient_basis_wave(i_compute,i_coord, &
                                                i_point)

                           c_times_psi(i_compute, i_point, i_spin) = &
                            c_times_psi(i_compute, i_point, i_spin) + &
                            4.0d0*local_c_gradient_deriv(i_coord, i_spin) * &
                            gradient_basis_wave(i_compute,i_coord, &
                                                i_point)
...
!     end integration loop over batches
      ...
      do i_spin = 1, n_spin
          call sync_matrix &
             (x_matr(:,:,i_spin),n_basis,n_basis)
          call sync_matrix &
             (c_matr(:,:,i_spin),n_basis,n_basis)
      enddo
!test
!      i_index = 0
...
!
!         write(use_unit, '(4X,3I5,2f16.10)') &
!                     i_spin, i_basis_1, i_basis_2,  &
!                     x_matr(i_basis_2,i_basis_1, 1), &
!                     c_matr(i_basis_2,i_basis_1, 1) 
!
!         enddo
!         write(use_unit, *)
!        enddo
...
!test
      ...
      return
      end subroutine integrate_split_xc_matrix
