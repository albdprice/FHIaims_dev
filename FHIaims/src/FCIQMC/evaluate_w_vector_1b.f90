!****s* FHI-aims/fciqmc/evaluate_w_vector_1b
!  NAME
!    evaluate_w_vector_1b
!  SYNOPSIS 

      subroutine evaluate_w_vector_1b()

!  PURPOSE
!
!  The '''evaluate_w_vector_1a''' evaluate the contributions of single-excitation
!  in the beta spin space for w_vect
!
!
!  AUTHOR
...
!  SOURCE
  ...
  use fciqmc_module
    ...
    do i_task = 0, n_tasks-1, 1
      ...
      i_ci        = memory_table(1,1,2,myid+1)-1
      i_ci_local  = memory_table(1,2,2,myid+1)-1
      ...
      do i_state = index_start(1), n_elec(2), 1
        do a_state = index_start(2), n_states, 1
          ...
          if (i_ci .gt. memory_table(2,1,2,myid+1)) goto 666
          ...
          do j_state = i_start_ci, n_elec(2), 1
            do b_state = n_elec(2)+1, n_states, 1
              ...
              if (j_task .eq. i_task) then
                  w_3 = w_3 + (&
                      ddot(n_basbas,&
                      ovlp_3ks(:,j_state,i_state,tmp_spin(2)),1,&
                      ovlp_3ks(:,a_state,b_state,tmp_spin(2)),1)&
                      - &
                      ddot(n_basbas,&
          ...
          ! j in O_{\alpha}, b in V_{\alpha}, c in V_{\beta}
          do j_state = i_start_ci, n_elec(1), 1 
            do b_state = n_elec(1)+1, n_states, 1
          ...
          w_5 = 0.0d0
          ! there are three terms in w_5
          ! the first one
          !! 2) j, k>j (j,k in O_{\beta}),  b<a (b in V_{\beta})
          do j_state = i_start_ci, n_elec(2), 1 
            do k_state = j_state+1, n_elec(2), 1
              do b_state = n_elec(2)+1, a_state-1, 1
                substitude(1,2) = j_state
          ...
          ! the third one
          ! j in O_{\beta}, k in O_{\alpha}, b in V_{\alpha})
          do j_state = i_start_ci, n_elec(2), 1 
...
end subroutine evaluate_w_vector_1b
