!****s* FHI-aims/fciqmc/evaluate_w_vector_2a
!  NAME
!    evaluate_w_vector_2a
!  SYNOPSIS 

      subroutine evaluate_w_vector_2a()

!  PURPOSE
!
!  Calculate w_vector on-the-fly for a given c_vect
!  '''evaluate_w_vector_2a''' generates w_vect of double excitations in the alpha spin space
!
!  AUTHOR
...
!  SOURCE
  ...
  use fciqmc_module
    ...
    do i_task = 0, n_tasks-1, 1
      ...
      i_ci        = memory_table(1,1,4,myid+1)-1
      i_ci_local  = memory_table(1,2,4,myid+1)-1
        ...
        do j_state = index_start(3), n_elec(1), 1 
          do a_state = index_start(2), n_states, 1
            do b_state = index_start(4), n_states, 1
              ...
              if (i_ci .gt. memory_table(2,1,4,myid+1)) goto 666
                ...
                do d_state = c_state+1, n_states, 1
                  ...
                  if (j_task .eq. i_task) then
                      w_1 = w_1 + (&
                          ...
                          ovlp_3ks(:,b_state,d_state,tmp_spin(1)),1)&
                          - &
                          ddot(n_basbas,&
                          ovlp_3ks(:,a_state,d_state,tmp_spin(1)),1,&
                          ovlp_3ks(:,b_state,c_state,tmp_spin(1)),1)&
                          )* c_vect_tmp(j_ci)
                  endif
              ...
              do k_state = i_start_ci, n_elec(1), 1
                do l_state = k_state+1, n_elec(1), 1
                  substitude(1,1) = k_state
                  substitude(2,1) = l_state
                  occupy(1,1)     = a_state
                  occupy(2,1)     = b_state
                  ...
                  if (j_task .eq. i_task) then
                      w_2 = w_2 + (&
                          ...
                          ovlp_3ks(:,l_state,j_state,tmp_spin(1)),1)&
                          - &
                          ddot(n_basbas,&
                          ovlp_3ks(:,k_state,j_state,tmp_spin(1)),1,&
                          ovlp_3ks(:,l_state,i_state,tmp_spin(1)),1)&
                          )* c_vect_tmp(j_ci)
                  endif
                  ...
                  if ((k_state.ne.i_state).and.(c_state.ne.a_state)) then
                    ...
                    if (j_task .eq. i_task) then
                        w_3 = w_3 + (&
                            ...
                            ovlp_3ks(:,b_state,j_state,tmp_spin(1)),1)&
                  ...
                  if ((k_state.ne.i_state).and.(c_state.ne.b_state)) then
                    ...
                    if (j_task .eq. i_task) then
                        w_3 = w_3 + (&
                            ...
                            ddot(n_basbas,&
                            ovlp_3ks(:,k_state,c_state,tmp_spin(1)),1,&
                            ovlp_3ks(:,a_state,j_state,tmp_spin(1)),1)&
                            )* c_vect_tmp(j_ci) * state_sign
                    endif
                  end if
                enddo
                  ...
                  if ((k_state.ne.j_state).and.(c_state.ne.a_state)) then
                    ...
                    if (j_task .eq. i_task) then
                        w_3 = w_3 + (&
                            ...
                            ddot(n_basbas,&
                            ovlp_3ks(:,k_state,c_state,tmp_spin(1)),1,&
                            ovlp_3ks(:,b_state,i_state,tmp_spin(1)),1)&
                            )* c_vect_tmp(j_ci) * state_sign
                    endif
                  end if
                enddo
              ...
              if (flag_single_excitation) then
                  ...
                  enddo
                  ! the second one goes over V_{\alpha}
                  do c_state = n_elec(1)+1, n_states, 1
                    substitude(1,1) = j_state
                    occupy(1,1)     = c_state
                  ...
                  enddo
                  ! the second one goes over O_{\alpha}
                  do k_state = i_start_ci, n_elec(1), 1
                    substitude(1,1) = k_state
                    occupy(1,1)     = b_state
...
end subroutine evaluate_w_vector_2a
