!****s* FHI-aims/evaluate_partition_tab_2atoms_p0
!  NAME
!   evaluate_partition_tab_2atoms_p0
!  SYNOPSIS

      subroutine evaluate_partition_tab_2atoms_p0 &
      ( n_atoms_list, atoms_list, dist_tab, i_r, &
...
!     counters
      ...
      character(*), parameter :: func = 'evaluate_partition_tab_2atoms_p0'
...
!     initialize numerator / denominator of partition function
      ...
      aux_dens = &
        val_spline &
        ( i_r(1), partition_rho_spl(1,1,species(i_atom)), &
         ...
         select case(partition_type_temp)
          ...
          case(1)
             wt_dens = &
                  aux_dens / (dist_tab(1)**2.0d0)
          case(2)
             wt_dens = &
                  aux_dens / abs(dist_tab(1))
          case(3)
             wt_dens = &
                  aux_dens
          case(4)
             wt_dens = &
               1.0/ ( 1+ &
               exp( (dist_tab(1)- hartree_partition_parameters(1))/ &
                    hartree_partition_parameters(2) ) )
          case(6)
             wt_dens = &
                  aux_dens / (dist_tab(1)**2.0d0)
          case default
             call aims_stop('Invalid value for partion_type_temp', func)
...
!    loop over the first atom
      do i_atom_1 = 2, n_atoms_list, 1

!        i_atom_2 = mod(atoms_list(i_atom_1),n_atoms)
!        if(i_atom_2.eq.0) then
!          i_atom_2=n_atoms
!        endif
        i_atom_2 = i_atom_1 - 1
...
!        if (i_atom_1 .ne. i_atom ) then
           aux_dens =   val_spline &
            ( i_r(i_atom_1), partition_rho_spl(1,1,species(i_atom_2)), &
           ...
           select case(partition_type_temp)
            ...
            case(6)
               aux_partition_norm = &
                   aux_dens / (dist_tab(i_atom_1)**2.0d0)
            case default
               call aims_stop('Invalid value for partion_type_temp', &
                              func)
               !stop   ! Satisfy compiler
           end select
...
!         this grid point is too far away 
!         - we should not be integrating here!
             partition_tab(i_atom_2) = 0.d0

         end if
!         write(use_unit,*) dist_tab(:)
!         write(use_unit,*) dist_tab(i_atom)**2,radial_weight,angular_weight
!         write(use_unit,*) wt_dens, aux_partition_norm, partition_norm,wt_dens/partition_norm
!         write(use_unit,*)i_atom_2, partition_tab(i_atom_2)


!     end loop over other atoms, i_atom_1
      enddo

      end subroutine evaluate_partition_tab_2atoms_p0
