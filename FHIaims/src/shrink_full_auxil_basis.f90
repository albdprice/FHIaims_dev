...
!  SYNOPSIS
      ...
      subroutine shrink_full_auxil_basis &
        ( )
...
!  extract the information about the radial function from the original basis
      do i_species = 1, n_species
        ...
        do i_basis = 1, n_basis
          ...
          i_l    = basis_l (i_basis)
          i_m    = basis_m (i_basis)
          i_n    = basisfn_n (basis_fn(i_basis))
...
!   only count the first atom belonging to this particular species
       ...
       if(.not.flag_species_incl) then
         i_function = i_function + n_basis_fn_species(i_species)
       endif
      ...
      enddo
        ...
        do i_species = 1, n_species, 1
          ...
          if( max_l_prodbas(i_species).gt.2*l_shell_max(i_species) ) &
                        then
              max_l_prodbas(i_species)= 2*l_shell_max(i_species)
          endif
...
!    product basis distributed over different threads
       ...
       end subroutine shrink_full_auxil_basis
!******
