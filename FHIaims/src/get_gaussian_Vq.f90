!----------------------------------------------------------------------------
!****s* FHI-aims/get_gaussian_Vq
!  NAME
!    get_gaussian_Vq
!  SYNOPSIS

subroutine get_gaussian_Vq(qvec, recip_lattice_vector, chi, n_Dvec, Dvec, &
&                          gamma, max_L1, max_L2, max_n_LM, Vq)
  ...
  !
  !    The Coulomb singularity for qvec=0 is fixed by the value chi, which can
  !    be obtained by extrapolation of get_singularity_lifting_chi() results
  !    for different Gamma to Gamma->0 [Gamma not related to gamma in this
  !    routine].
  !
  !  USES
  ...
  complex*16, intent(OUT) :: Vq(max_n_LM, max_n_LM, n_Dvec)
  ...
  character(*), parameter :: func = 'get_gaussian_Vq'

  if(n_Dvec.eq.0) return
  max_L = max_L1 + max_L2
  n_LM1 = (max_L1 + 1)**2
  n_LM2 = (max_L2 + 1)**2
  n_LM = (max_L + 1)**2
  ...
  allocate(lattice_sum(n_LM, n_Dvec, 0:n_add_ksq), stat=info)
  ...
  do i_add_ksq = 0, n_add_ksq
     ...
     call gaussian_lattice_sum(recip_lattice_vector, -qvec, &
     &                         power, exponent, this_max_L, n_LM, &
           ...
           do M1 = -L1, L1
              ...
              do M2 = -L2, L2
                 ...
                 do i_MM = 1, 2
                    ...
                    if (MM >= -LL) then
                       i_LLMM = LL**2 + LL + MM + 1
                       Vq(i_LM1, i_LM2, :) = Vq(i_LM1, i_LM2, :) &
                       &       + triple_Yr(M1, M2, i_MM) &
                       &         * cfac * lattice_sum(i_LLMM, :, i_add_ksq)
...
end subroutine get_gaussian_Vq
!******
