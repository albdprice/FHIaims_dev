!****s* FHI-aims/integrate_first_order_S
!  NAME
!   integrate_first_order_rho
!  SYNOPSIS

subroutine integrate_first_order_rho &
     (partition_tab_std, basis_l_max, KS_ev_compute, first_order_density_matrix, &
      max_occ_number,first_order_rho,first_order_rho_moving_grid)
...
!  ARGUMENTS
  ...
  real*8,  dimension(3, n_atoms, n_full_points), intent(out) ::  first_order_rho 
  real*8,  dimension(3, n_atoms, n_full_points), intent(out) :: &  
                                                     first_order_rho_moving_grid 
  !shanghui------------------------------------------------------------------------
...
!
!  OUTPUT
!  o first_order_DM_rho -- first_order density matrix 's contribution to first-order rho.  
!
!  AUTHOR
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
  ...
  !----------shanghui add here for evaluate_first_order_rho----
   integer n_occ_states
    !write(use_unit,*) 'integrade_DM:',max_occ_number(1)
     n_occ_states= max_occ_number(1)  
   ! shanghui need to change this in the future for    nspin=2.
...
!----------------------------shanghui------------------------------------------             
            !if(i_my_batch.eq.398) then
            !write(use_unit,*) 'before:',wave(1,53)
            !endif  
           ...
           !if(i_my_batch.eq.398) then 
           !write(use_unit,*) 'after:', wave(1,53)
           ! write(use_unit,*) local_first_order_DM_rho(1,1,53), & 
           !            local_first_order_rho(1,1,53)
           !endif 
       i_point = 0
       ! loop over one batch of integration points
       do i_index = 1, batches_work(i_my_batch)%size, 1
          ...
          if (partition_tab(i_full_points_DM_rho).gt.0.d0) then
                 ...
                 do i_atom=1,n_atoms
                    do i_coord=1,3
                 first_order_rho(i_coord,i_atom,i_full_points_DM_rho)= & 
                 !local_first_order_DM_rho(i_coord,i_atom,i_point)
                 local_first_order_rho(i_coord,i_atom,i_point) 
  
               first_order_rho_moving_grid(i_coord,i_atom,i_full_points_DM_rho)= & 
                 local_first_order_rho_moving_grid(i_coord,i_atom,i_point) 
                    enddo
                 enddo
         endif  
       ...
       enddo
!----------------------------shanghui-------------------------------------------
        ! first_order_DM_rho is now complete.
        !
     else
       ...
       i_full_points_DM_rho + batches_work(i_my_batch)%size) =0.0d0
        ...
        first_order_rho_moving_grid(:,:,i_full_points_DM_rho+1:& 
       i_full_points_DM_rho + batches_work(i_my_batch)%size) =0.0d0
...
end subroutine integrate_first_order_rho
!******
