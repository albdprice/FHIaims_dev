!****s* FHI-aims/evaluate_H_psi_p2
!  NAME
!   evaluate_H_psi_p2
!  SYNOPSIS

subroutine evaluate_H_psi_p2 &
     ( n_compute, n_compute_atoms, n_compute_fns, &
     l_ylm_max, ylm_tab, one_over_dist_tab, &
     radial_wave, H_times_psi, &
     local_potential_parts,  &
     kinetic_wave, zora_operator, &
...
!   o zora_operator --  2. light_speed_sq /( 2 * light_speed_sq -   potential ) for ZORA relativity
...
!
  ...
  if( (flag_rel.eq.REL_none).or.(flag_rel.eq.REL_atomic_zora) &
       .or.(flag_rel.eq.REL_own) .or.(flag_rel.eq.REL_4c_dks) ) then  
  ...
  do i_compute_fn = 1, n_compute_fns, 1
        ...
        call mul_vec_2 ( &
             H_times_psi(wave_index(i_compute_fn)), l_count(i_compute_fn)+1, &
...
end subroutine evaluate_H_psi_p2
!******
!---------------------------------------------------------------------
!****s* FHI-aims/mul_vec_2
!  NAME
!   mul_vec_2
!  SYNOPSIS

subroutine mul_vec_2 ( wave, n_mul, ylm, factor )
...
!  INPUTS
!    o n_mul -- dimension of the vectors
