      subroutine self_consistent_DMFT &
       ( )
...
!----------------------------------------------------------
      ...
      if (.not.allocated(self_energy_freq_old)) then
         allocate(self_energy_freq_old(n_basis,n_basis,nomega))
      endif
...
!       endif
        ...
        write(info_str,'(A)') "------------------------------------------------------------"
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(10X,A,1X,I4)') "Self-Consistent DMFT embedding starts ..."
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(A)') ''
        call localorb_info ( info_str, use_unit,'(A)', OL_norm )
        ...
        write(info_str,'(A)') ''
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(10X,A,1X,I4)') "Initialization of the frequency grid ..."
        write(info_str,'(A)') ''
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(A)') ''
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
...
!       endif
       ...
       if (.not. allocated(full_ovlp_matrix_sqrt))then
        ...
        write(info_str,'(A)') ''
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(10X,A,1X,I4)') "evaluating the PBC and&
         & Cluster quantities..."
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(A)') ''
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
       ...
       threshold_green = 1.d-4
        ...
        &--------------------------"
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(10X,A,1X,I4)') "  Self-Consistent main &
        &loop initialization ..."
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        call date_and_time(cdate, ctime)
      ...
      call get_on_site_gf_freq(full_hamiltonian,&
                                 full_ovlp_matrix,&
                                 inv_on_site_gf,&
                                 on_site_gf,&
                                 on_site_xc_matr,&
      ...
      if (.not.allocated(self_energy_freq)) then
       ...
       inner_loop_reiterations = 0
        ...
        write(info_str,'(10X,A,1X,I4)') "  Self-Consistent &
         &Self-Consistent DMFT inner-loop starts ..."
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        call date_and_time(cdate, ctime)
        write(info_str,'(2X,A,A,A,A)') "Date     :  ",&
        cdate, ", Time     :  ", ctime
...
!         if(.false.)then
        ...
        &------------------------"
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(10X,A,1X,I4)') "Calculating the &
         &embedded Green function iteration", inner_loop_reiterations
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        call date_and_time(cdate, ctime)
        write(info_str,'(2X,A,A,A,A)') "Date     :  "&
        , cdate, ", Time     :  ", ctime
...
!         call get_density_matr(embed_gf, &
!                               embed_dens_matr, &
!                               inner_loop_reiterations, &
!                               number_reiterations,&
!                               full_ovlp_matrix_sqrt,&
!                               inv_full_ovlp_matrix_sqrt)
        ...
        write(info_str,'(A)') ''
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(10X,A,1X,I4)') "Local GW Self-Energy &
         &Caluclation starts ..."
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        call date_and_time(cdate, ctime)
        write(info_str,'(2X,A,A,A,A)') "Date     :  ",&
        ...
        if(.true.) then 
!        if(.false.) then 
!        if( error_GF_on_site .lt. 1.d-2) then 
!         if (inner_loop_reiterations.eq.0)then
              self_energy_freq_old(:,:,:) = self_energy_freq(:,:,:)
              call get_GW_self_energy (embed_gf, embed_gf_time, &
                                       self_energy_freq,&
                                       !inv_full_ovlp_matrix_sqrt, &
                                       k_summed_overlap_matr )
        endif
...
!         if (inner_loop_reiterations.ge.1)&
!             call transform_G (embed_gf , n_basis, &
!               n_basis, embed_gf_time(:,:,:,1))

        call evaluate_hartree_and_exchange_embed(embed_gf_time(:,:,0,1), &
                                          loc_self_enrg_new, &
                                          number_reiterations, &
                                          inner_loop_reiterations, &
                                          hartree_pot_LDA,&
     ...
     if(.false.)then
!     if(.true.)then
!     if(number_reiterations.ne.0)then
...
!       endif
        ...
        write(info_str,'(10X,A,1X,I4)') "  Self-Consistent DMFT &
         &Embedding inner-loop converged!"
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        call date_and_time(cdate, ctime)
        write(info_str,'(2X,A,A,A,A)') "Date     :  "& 
        , cdate, ", Time     :  ", ctime
...
!stop
        enddo ! while(.not. inner_loop_converged), 
              !end of the self consistent inner-loop
...
!       endif
        ...
        new_chemical_potential = chemical_potential
         ...
         call get_NEW_on_site_gf_freq(full_hamiltonian, &
                                     full_ovlp_matrix, &
                                     !inv_full_ovlp_matrix, &
                                     new_on_site_gf,&
                                     on_site_xc_matr, &
        ...
        &-----------------------------"
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        write(info_str,'(10X,A,1X,I4)') "  Galitskii-Migdal &
         &Total Energy Calculation iteration", number_reiterations
        call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
        call date_and_time(cdate, ctime)
        write(info_str,'(2X,A,A,A,A)') "Date     :  "&
        , cdate, ", Time     :  ", ctime
        ...
        call get_timestamps ( time_sc_loop, clock_time_sc_loop )
           ...
           call  get_total_energy_dmft(new_on_site_gf,&
                                  loc_self_enrg_new,&
                                  inner_loop_reiterations,&
                                  free_cluster_ham,&
                                  hartree_pot_LDA,&
      ...
      new_on_site_gf(:,:,:) = DCMPLX(dmft_alpha)*new_on_site_gf(:,:,:) +&
                       DCMPLX(1-dmft_alpha)*on_site_gf(:,:,:)
...
!     endif
      ...
      end subroutine self_consistent_DMFT
