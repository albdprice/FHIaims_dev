...
!  SYNOPSIS
!
      subroutine invert_overlap_matrix_complex (auxmat)
!                  (auxmat, power, name)
...
!  ARGUMENTS
      complex*16 auxmat(n_basis, n_basis)
      !complex*16, intent(INOUT) :: auxmat(n_basbas,n_loc_prodbas)
      real*8 :: power
!      character*(*) :: name
!
!  INPUTS
...
!!$         if (power < 0.d0) then
            threshold =  1.d-5
!!$         else
         call power_genmat_lapack_complex(n_basis, auxmat, power, &
...
!       end subroutine power_auxmat_lapack
        end subroutine invert_overlap_matrix_complex
!******
