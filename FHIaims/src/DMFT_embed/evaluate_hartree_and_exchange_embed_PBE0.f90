      ...
      subroutine evaluate_hartree_and_exchange_embed_PBE0(density_matrix, &
                                                     loc_self_enrg,&
...
hartree_pot_HF(:,:) = 0.d0
      ...
      if(.not. allocated( aux_ovlp_3fn ))then
         allocate (aux_ovlp_3fn (n_basis,n_basis))!local
      endif
        ...
        do j_basis =1, n_basis, 1
          if(basis_nghr(i_basis,j_basis).gt.0)then
            aux_ovlp_3fn (i_basis, j_basis) = &
             ovlp_3fn(basis_nghr(i_basis,j_basis),i_basbas)
          else
   ...
   do i_basis = 1, n_basis, 1
       do j_basis = 1, n_basis, 1
         ...
         loc_self_enrg(i_basis,j_basis) =loc_self_enrg(i_basis,j_basis) - &!+ &
                                              (hybrid_alpha)*exchange_self_energy(i_basis,j_basis)+ &
                                              on_site_PBE_xc_matr(i_basis,j_basis)  
       enddo
   enddo
...
!          close(65)
      ...
      end subroutine evaluate_hartree_and_exchange_embed_PBE0
!---------------------------------------------------------------------
!******
