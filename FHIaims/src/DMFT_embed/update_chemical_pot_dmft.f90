  subroutine update_chemical_pot_dmft(embed_gf,& 
...
!   real*8, dimension(n_basis,n_basis), intent(in) :: k_summed_overlap_matr
   ...
   complex*16 , dimension (n_basis,n_basis,nomega,n_k_points) ::&
            ...
            do j_matrix_size = 1, n_basis, 1
              do i_matrix_size = 1, n_basis, 1
                  ...
                  (real(embed_gf(i_matrix_size,j_matrix_size, i_freq))*&
                   cos((omega(i_freq))*(5.1023E-4))&! for grid with 200 points
                  -aimag(embed_gf(i_matrix_size,j_matrix_size, i_freq))*&
                   sin((omega(i_freq))*(5.1023E-4)))! for grid with 200 points
...
call  update_chemical_pot(inv_full_ovlp_matrix_sqrt, &
                          full_ovlp_matrix_sqrt, &
                          full_ovlp_matrix,&
                          free_cluster_ham,&
                          loc_self_enrg,&
...
!-----------------------------------------------------------------------------------------
  ...
  end subroutine update_chemical_pot_dmft 
