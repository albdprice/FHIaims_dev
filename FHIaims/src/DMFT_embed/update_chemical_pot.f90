...
subroutine update_chemical_pot(inv_full_ovlp_matrix_sqrt,&
...
!  ARGUMENTS
  ...
  logical :: t_out
   ...
   real*8, dimension(n_basis,n_basis), intent(in) :: on_site_xc_matr_no_ovlp
   real*8, dimension(n_basis,n_basis), intent(in) :: loc_self_enrg
   real*8, dimension(n_basis,n_basis), intent(in) ::&
   on_site_PBE_xc_matr_no_ovlp
...
!write(use_unit,*) 'n_electrons', n_electrons
  ...
  call check_norm_dmft_p2(inv_full_ovlp_matrix_sqrt,&
                          full_ovlp_matrix_sqrt, &
                          full_ovlp_matrix, & 
                          free_cluster_ham, &
                          loc_self_enrg,&
  ...
  call check_norm_dmft_p2(inv_full_ovlp_matrix_sqrt,&
     ...
     if (i_counter .gt. max_zeroin) then
        ...
        write (info_str,'(1X,A)') "* Too many iterations needed &
                                     &to find good interval around E_F."
        call localorb_info(info_str,use_unit,'(A)')
        write (info_str,'(1X,A)') &
  ...
  call zeroin_dmft(inv_full_ovlp_matrix_sqrt, &
                   full_ovlp_matrix_sqrt, &
                   full_ovlp_matrix, &
                   free_cluster_ham, &
                   loc_self_enrg,&
  ...
  if (i_counter .gt. max_zeroin) then
     ...
     call localorb_info(info_str,use_unit,'(A)')
     write (info_str,'(1X,A)') "* Too many iterations needed &
                      &to find correct occupation of levels."
     call localorb_info(info_str,use_unit,'(A)')
...
!  end if
  ...
  if (t_out) then
     ...
     if (occupation_thr.gt.0.d0) then
        write (info_str, '(2X, A, E10.4)') "| Error in electron count &
                      &after thresholding : ", diff_electrons_thr
        call localorb_info(info_str,use_unit,'(A)',OL_norm)
     end if
  end if
...
end subroutine update_chemical_pot 
