  subroutine get_total_energy_dmft_PBE0(on_site_gf,&
                                   loc_self_enrg,&
...
!   complex*16, dimension(n_basis,n_basis,n_spin,n_k_points) :: full_k_xc_matr
   complex*16, dimension(n_basis,n_basis,n_spin,n_k_points) :: full_k_PBE_xc_matr
...
!----------------------------------------------------------------------------------
     ...
     enddo
       ...
       total_enrg_PBE_x_dmft(:,:) = total_enrg_PBE_x_dmft(:,:)*(1./n_k_points)
       total_enrg_PBE_x(:,:) = total_enrg_PBE_x(:,:)*(1./n_k_points)
       total_enrg_PBE_c_dmft(:,:) = total_enrg_PBE_c_dmft(:,:)*(1./n_k_points)
       total_enrg_PBE_c_lda(:,:) = total_enrg_PBE_c_lda(:,:)*(1./n_k_points)
       kin_enrg_dmft_matr(:,:) = kin_enrg_dmft_matr(:,:)*(1./n_k_points)
       kin_enrg_surround_matr(:,:) = kin_enrg_surround_matr(:,:)*(1./n_k_points)
...
!-----------------------------------------------------------------------------------------
  ...
  do i_a = 1, n_basis,1
    ...
    total_X_enrg_dmft = total_X_enrg_dmft - &
                        total_enrg_fock_dmft(i_a,i_a)
    total_hartree_enrg_dmft = total_hartree_enrg_dmft +&
                        total_enrg_hartree_dmft(i_a,i_a)
    total_hartree_enrg_LDA = total_hartree_enrg_LDA + &
                        E_tot_hartree_lda(i_a,i_a)
    total_XC_enrg_LDA = total_XC_enrg_LDA + &
                        E_tot_xc_non_loc(i_a,i_a)
    total_X_enrg_surround = total_X_enrg_surround + & 
                        total_enrg_PBE_x_dmft(i_a,i_a)
    total_X_enrg_surround_LDA = total_X_enrg_surround_LDA + &
                        total_enrg_PBE_x(i_a,i_a)
    total_C_enrg_surround = total_C_enrg_surround + &
                        total_enrg_PBE_c_lda(i_a,i_a)
    total_C_enrg_dmft = total_C_enrg_dmft + &
                        total_enrg_PBE_c_dmft(i_a,i_a)
    kin_enrg_dmft = kin_enrg_dmft + &
                        kin_enrg_dmft_matr(i_a,i_a) 
    kin_enrg_surround = kin_enrg_surround + &
                        kin_enrg_surround_matr(i_a,i_a) 
...
total_enrg_dmft =  total_energy - 0.25*en_xc&
         ...
         call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
         write (info_str,'(2X,A,1X,F20.8,A,1X,F20.8,A)') &
              "| EXX energy correction from DMFT          :"&
               , total_X_enrg_dmft, " Ha", &
         ...
         call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
         write (info_str,'(2X,A,1X,F20.8,A,1X,F20.8,A)') &
              "| C energy from environement               :"&
               , 0.5*total_C_enrg_surround, " Ha", &
              total_C_enrg_surround*0.5 * hartree, " eV"
         call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
         write (info_str,'(2X,A,1X,F20.8,A,1X,F20.8,A)') &
              "| C energy from DMFT                       :"&
               , 0.5*total_C_enrg_DMFT, " Ha", &
        ...
        if (myid.eq.0) then
         ...
         write (info_str,'(2X,A,1X,F20.8,A,1X,F20.8,A)') &
              "| embedding-XX energy Contribution         :"&
               , 0.25*(hybrid_alpha*total_X_enrg_dmft+(1.d0-&
                                 hybrid_alpha)*1*x_energy(1)&
                                 +1*c_energy-en_xc), " Ha", &
                 0.25*(hybrid_alpha*total_X_enrg_dmft+(1.d0-&
                                 hybrid_alpha)*1*x_energy(1)&
                       +1*c_energy-en_xc) * hartree, " eV"
         call localorb_info ( info_str, use_unit,'(A)', OL_norm  )
...
!-----------------------------------------------------------------------------------------
      ...
      if (allocated(total_enrg_PBE_x))then
          deallocate(total_enrg_PBE_x)
       endif
      if (allocated(total_enrg_PBE_x_dmft))then
          deallocate(total_enrg_PBE_x_dmft)
       endif
      if (allocated(total_enrg_PBE_c_dmft))then
          deallocate(total_enrg_PBE_c_dmft)
       endif
      if (allocated(total_enrg_PBE_c_lda))then
          deallocate(total_enrg_PBE_c_lda)
       endif
      if (allocated(free_ham_dmft))then
          deallocate(free_ham_dmft)
       endif
      if (allocated(free_ham))then
          deallocate(free_ham)
       endif
      if (allocated(kin_enrg_dmft_matr))then
          deallocate(kin_enrg_dmft_matr)
       endif
      if (allocated(kin_enrg_surround_matr))then
          deallocate(kin_enrg_surround_matr)
       endif


  end subroutine get_total_energy_dmft_PBE0 
