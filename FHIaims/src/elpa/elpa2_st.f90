...
! Version 1.1.2, 2011-02-21
  ...
  public :: solve_evp_real_2stage_2013
  public :: solve_evp_complex_2stage_2013
...
subroutine solve_evp_real_2stage_2013(na, nev, a, lda, ev, q, ldq, nblk, mpi_comm_rows, mpi_comm_cols, mpi_comm_all)
...
end subroutine solve_evp_real_2stage_2013

!-------------------------------------------------------------------------------

!-------------------------------------------------------------------------------

subroutine solve_evp_complex_2stage_2013(na, nev, a, lda, ev, q, ldq, nblk, mpi_comm_rows, mpi_comm_cols, mpi_comm_all)
...
!-------------------------------------------------------------------------------
   ...
   ttt0 = MPI_Wtime()
   call solve_tridi_2013(na, nev, ev, e, q_real, ubound(q_real,1), nblk, mpi_comm_rows, mpi_comm_cols)
   ttt1 = MPI_Wtime()
   if(my_prow==0 .and. my_pcol==0 .and. elpa_print_times)  &
      print 1,'Time solve_tridi                   :',ttt1-ttt0
   time_evp_solve = ttt1-ttt0
...
end subroutine solve_evp_complex_2stage_2013
...
!AJL/2014
! For some reason I am getting negative numbers returned from this 
! subroutine in trig_band_real in blocks_limits, which is then breaking the 
! waitall statements at the end of the subroutine. What is going on?
   ...
   if(nblocks_total < n_pes) then
      ...
      !enddo
      ! AJL/2014
      ! Making this for loop more explicit seems to prevent the 
      ! "negative" numbers I've been getting in the block_limits
      ! This error occurred both on the CRAY XT30 (pgf90) 
      ! and Intel (ifort) clusters.
      ! I must admit I have no idea why this is, 
      ! and why it is consistent across machines.
      do n=1,nblocks_total
         block_limits(n) = n
      enddo
      do n=nblocks_total+1, n_pes
         block_limits(n) = nblocks_total
      enddo
      ! AJL/2014
   else
...
!AJL/DEBUG
!   do n=1,n_pes
!      write(use_unit,*) n, block_limits(n)
!   enddo
!AJL/DEBUG

end subroutine

end module ELPA2_2013
