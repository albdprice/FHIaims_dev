...
subroutine density_matrix_mixing(i_spin, number_of_loops, density_matrix)
  ...
  use synchronize_mpi_basic, only: sync_matrix
  implicit none
  ...
  if (number_of_loops <= 1) then
     ...
     ! than using zero.
     prev_denmat(:, i_spin) = 0.d0
     call glob2loc(prev_denmat(:, i_spin), density_matrix, 1.d0)
  end if

  ! delta_denmat := density_matrix - prev_denmat
  delta_denmat = - prev_denmat(:, i_spin)
  ...
  density_matrix = 0.d0
  prev_denmat(:, i_spin) = prev_denmat(:, i_spin) + denmat_diff
  call loc2glob(density_matrix, prev_denmat(:, i_spin), 1.d0)
...
end subroutine density_matrix_mixing
!******
