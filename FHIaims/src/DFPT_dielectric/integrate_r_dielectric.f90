!****s* FHI-aims/integrate_r_dielectric
!  NAME
!   integrate_r_dielectric
!  SYNOPSIS

subroutine integrate_r_dielectric &
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
  ...
  if(use_batch_permutation > 0) then
    write(info_str,'(2X,A)') "Integrating r matrix: batch-based integration with load balancing"
  else
    write(info_str,'(2X,A)') "Integrating r matrix: batch-based integration."
  endif
     ...
     if (n_compute_c.gt.0) then
        ...
        do i_index = 1, batches_work(i_my_batch)%size, 1
           ...
           if (partition_tab(i_full_points).gt.0.d0) then
              ...
              coords_npoints(:,i_point) = coord_current(:)  
  ...
  if(.not. use_local_index) call sync_integrate_hamiltonian(r_sparse) 
...
end subroutine integrate_r_dielectric
!******
