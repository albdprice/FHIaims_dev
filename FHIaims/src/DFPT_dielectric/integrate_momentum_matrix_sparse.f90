!****s* FHI-aims/integrate_momentum_matrix_sparse
!  NAME
!   integrate_momentum_matrix
!  SYNOPSIS

subroutine integrate_momentum_matrix_sparse &
     (partition_tab_std, basis_l_max, j_coord, momentum_matrix_sparse)
...
!  shanghui 2015.11.27 : change to momentum matrix 
!
!  USES
...
!  ARGUMENTS
  ...
  real*8, dimension(n_hamiltonian_matrix_size_no_symmetry),intent(inout) :: &
...
!
!  OUTPUT
!  o momentum_matrix_sparse -- momentum_matrix in sparse atomic basis
!
!  AUTHOR
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
  ...
  if(use_batch_permutation > 0) then
    write(info_str,'(2X,A)') "Integrating momentum_matrix_sparse matrix: batch-based integration with load balancing"
  else
    write(info_str,'(2X,A)') "Integrating momentum_matrix_sparse matrix: batch-based integration."
  endif
  ...
  !-----------------------------------------------------------------------------
  ! initialize
  momentum_matrix_sparse(1:n_hamiltonian_matrix_size_no_symmetry)= 0.0d0
...
!----------------shanghui begin momentum_matrix_sparse------------------------------------------             
       call evaluate_momentum_matrix_sparse & 
           ( n_points, partition(1:n_points), &
             n_compute_c, i_basis,   & 
             wave(:,1:n_points), gradient_basis_wave_npoints(:,:,1:n_points), & 
             j_coord,  &  
             momentum_matrix_sparse)    
!---------------shanghui end momentum_matrix_sparse------------------------------------------             
     else
  ...
  if(.not. use_local_index) call sync_vector(momentum_matrix_sparse,n_hamiltonian_matrix_size_no_symmetry )  
...
end subroutine integrate_momentum_matrix_sparse
!******
