!****s* FHI-aims/integrate_Born_effective_charges_Pulay
!  NAME
!   integrate_Born_effective_charges_Pulay
!  SYNOPSIS

subroutine integrate_Born_effective_charges_Pulay &
...
!  PURPOSE
!  get Born effective charges, the Pulay part.
...
!  ARGUMENTS
  ...
  real*8, dimension(n_hamiltonian_matrix_size),intent(in):: zero_order_density_matrix_sparse
  real*8, dimension(n_hamiltonian_matrix_size),intent(in):: first_order_density_matrix_sparse
  real*8, dimension(n_hamiltonian_matrix_size),intent(in):: first_order_energy_density_matrix_sparse
 
  real*8, dimension(3, n_atoms) :: Born_effective_charges_Pulay
...
!
!  OUTPUT
!  o Born_effective_charges_Pulay
!
!  AUTHOR
...
!-------For pulay_term--------------------------------------
    real*8, dimension(:,:),  allocatable :: zero_order_density_matrix_con
    real*8, dimension(:,:),  allocatable :: first_order_density_matrix_con
    real*8, dimension(:,:),  allocatable :: first_order_energy_density_matrix_con
!-------End for pulay_term----------------------------------
...
! VB - renewed index infrastructure starts here
  ...
  if(use_batch_permutation > 0) then
    write(info_str,'(2X,A)') "Integrating Born_effective_charges_Pulay matrix: batch-based integration with load balancing"
  else
    write(info_str,'(2X,A)') "Integrating Born_effective_charges_Pulay matrix: batch-based integration."
  endif
...
!-------For pulay_term-------------------------------------
  if(.not. allocated(zero_order_density_matrix_con))then
  allocate(zero_order_density_matrix_con(n_max_compute_dens,n_max_compute_dens),stat=info)
  call check_allocation(info, 'zero_order_density_matrix_con            ')
  end if
...
!-------End for pulay_term--------------------------------------
  ...
  ! initialize
   Born_effective_charges_Pulay = 0.0d0
  ...
  do i_my_batch = 1, n_my_batches_work, 1
    ...
    !-----------shanghui begin test prune_matrix here-------------------------
     call prune_density_matrix_sparse_dielectric(zero_order_density_matrix_sparse, &
                     zero_order_density_matrix_con, &
                     n_compute_c, i_basis)
     ...
     call prune_density_matrix_sparse_dielectric(first_order_energy_density_matrix_sparse, &
...
!----------shanghui-gga: always calculate gradient---------------------
              ...
              if ((flag_rel.eq.REL_atomic_zora)) then    !!! atomic zora (evaluate grad[T_z*PHI])
        ...
        do i_spin = 1, n_spin, 1
            ...
            call evaluate_Born_effective_charges_Pulay( & 
                 partition,  &
    ...
    if(.not. use_local_index) call sync_vector(Born_effective_charges_Pulay,3*n_atoms)
...
!  if(.not. use_local_index) call sync_integrate_hamiltonian( hamiltonian )
  ...
  if (allocated( zero_order_density_matrix_con   )) deallocate( zero_order_density_matrix_con   )
...
end subroutine integrate_Born_effective_charges_Pulay
!******
