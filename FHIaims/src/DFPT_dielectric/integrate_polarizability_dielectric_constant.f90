...
!****s* FHI-aims/integrate_polarizability_dielectric_constant
!  NAME
!   integrate_polarizability_dielectric_constant
!  SYNOPSIS

subroutine integrate_polarizability_dielectric_constant &
     (partition_tab_std, first_order_rho_std, dielectric_constant,j_coord)
...
!-------------end mpi---------------------------
        ...
        do i_index = 1, batches_work(i_batch)%size, 1
          ...
          if (partition_tab(i_full_points).gt.0.d0) then
           ...
           do i_coord = 1, 3
             ...
             *first_order_rho(i_full_points)*coord_current(i_coord)
  ...
  if(.not. use_local_index) call sync_vector( dP_dE, 9)
  !-------shanghui end parallel------


   write(info_str,'(A)') '----polarizability: dP_dE (Bohr^3) at every cycle:--->'
   call localorb_info(info_str, use_unit,'(A)', OL_norm)
...
end subroutine integrate_polarizability_dielectric_constant
!******
!------------------------------------------------------------------------------------------------------------
