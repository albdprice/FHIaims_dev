!****s* FHI-aims/integrate_Omega_part_1_sparse
!  NAME
!   integrate_Omega_part_1_sparse
!  SYNOPSIS

subroutine integrate_Omega_part_1_sparse &
     (partition_tab_std, basis_l_max, j_coord, Omega_part_1_sparse)

!  PURPOSE
!   Omega_MO =  
!        <2> = Omega_part_1 + Omega_MO_part_2
...
!
!  OUTPUT
!  o Omega_part_1_sparse -- Omega_part_1 in sparse atomic basis
!
!  AUTHOR
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
  ...
  if(use_batch_permutation > 0) then
    write(info_str,'(2X,A)') "Integrating Omega_part_1_sparse matrix: batch-based integration with load balancing"
  else
    write(info_str,'(2X,A)') "Integrating Omega_part_1_sparse matrix: batch-based integration."
  endif
  ...
  !-----------------------------------------------------------------------------
  ! initialize
  Omega_part_1_sparse(1:n_hamiltonian_matrix_size_no_symmetry)= 0.0d0
     ...
     if (n_compute_c.gt.0) then
        ...
        do i_index = 1, batches_work(i_my_batch)%size, 1
           ...
           if (partition_tab(i_full_points).gt.0.d0) then
              ...
              if(n_periodic > 0)then
                 call map_to_center_cell(coords_npoints(1:3,i_point) )
              end if
...
!----------------shanghui begin Omega_part_1_sparse------------------------------------------             
       call evaluate_Omega_part_1_sparse & 
           ( n_points, partition(1:n_points), &
             ...
             j_coord,  &  
             Omega_part_1_sparse)    
!---------------shanghui end Omega_part_1_sparse------------------------------------------             
     else
  ...
  if(.not. use_local_index) call sync_vector(Omega_part_1_sparse,n_hamiltonian_matrix_size_no_symmetry )  
...
end subroutine integrate_Omega_part_1_sparse
!******
