...
subroutine integrate_Born_effective_charges_MP &
     (multipole_moments, &
     partition_tab_std, & 
     rho_std, rho_multipole,  &
...
!  USES
  ...
  use pbc_lists
  use load_balancing
  ! first_ordre_rho_multipole from hartree_potential_storage conflicts with the local variable here,
...
!  ARGUMENTS
  ...
  real*8, dimension(n_full_points), intent(in)    :: rho_multipole 
  real*8, dimension(3,n_atoms,n_full_points), intent(in)  :: v_hartree_gradient_multipole 
  real*8, dimension(n_full_points), intent(in)    :: first_order_rho
...
!  SOURCE
  ...
  ! work arrays
  real*8, allocatable :: first_order_rho_multipole(:)
  ...
  allocate(first_order_rho_multipole(n_full_points_work),stat=info)
  call check_allocation(info, 'first_order_rho_multipole')
  ...
  first_order_rho_multipole    = 0.d0
          ...
          if (partition_tab(i_full_points).gt.0.d0) then
            ...
            if (dist_tab_sq.lt.multipole_radius_sq(current_spl_atom) ) then
              ...
              if(i_iter == 1) then
                ...
                first_order_rho_multipole(i_full_points) = first_order_rho_multipole(i_full_points) + rho_multipole_aux
            ...
            if (i_iter == 2) then
                ...
                if (.not.empty(center_to_atom(current_center))) then
                  ...
                  do i_coord = 1, 3, 1
                    ...
                    multipole_forces(i_coord,center_to_atom(current_center)) &
                           + (rho(i_full_points) -  rho_multipole(i_full_points)) &
       ...
       do i_batch = 1, n_my_batches_work
        do i_index = 1, batches_work(i_batch)%size, 1
          ...
          if (partition_tab(i_full_points).gt.0.d0) then
               do i_coord = 1, 3, 1
               do i_atom = 1, n_atoms
                 multipole_forces(i_coord,i_atom) = &
                 multipole_forces(i_coord,i_atom) &
                        + (first_order_rho(i_full_points) - & 
                           first_order_rho_multipole(i_full_points)) & 
                        * v_hartree_gradient_multipole(i_coord,i_atom,i_full_points) & 
                        * partition_tab(i_full_points)
               enddo
               end do
  ...
  if(allocated(first_order_rho_multipole))   deallocate(first_order_rho_multipole)
...
end subroutine integrate_Born_effective_charges_MP
!******
!------------------------------------------------------------------------------------------------------------
