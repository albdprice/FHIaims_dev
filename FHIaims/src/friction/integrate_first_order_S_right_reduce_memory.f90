...
!  NAME
!   integrate_first_order_S
!  SYNOPSIS

subroutine integrate_first_order_S_right_reduce_memory &
     (partition_tab_std, basis_l_max, first_order_S,j_atom,j_coord)
...
!  ARGUMENTS
  ...
  integer ::  basis_l_max (n_species), j_atom, j_coord
  !shanghui------------------------------------------------------------------------
  real*8  first_order_S(n_basis,n_basis)
  !shanghui------------------------------------------------------------------------
...
!
!  OUTPUT
!  o first_order_S -- first_order overlap matrix
!
!  AUTHOR
...
!  integer ld_hamiltonian  ! leading dimension of hamiltonian in calling routine
  ...
  if(use_batch_permutation > 0) then
    write(info_str,'(2X,A)') "Integrating overlap matrix: batch-based integration with load balancing"
  else
    write(info_str,'(2X,A)') "Integrating overlap matrix: batch-based integration."
  endif
  ...
  !--------------initialize----------------------
  first_order_S(1:n_basis,1:n_basis)=0.0d0
...
!             if(i_my_batch.eq.1) then
!               do i=1,n_compute_c
!                 if(i_point.eq.2) then
!                write(use_unit,*) 'gradient_basis_wave_npoints:', i, gradient_basis_wave_npoints(i,2,i_point)   !gradient_basis_wave(i,2)
!                 endif
!                enddo
!             endif
        ...
        ! Now add all contributions to the full first_order_S
         !if(i_my_batch.eq.69) then 
         !write(use_unit,*) n_points, n_compute_c
         !write(use_unit,*) wave(1,1:n_points)
         !write(use_unit,*) wave(2,1:n_points)
         !write(use_unit,*) 
         !endif
!----------------------------shanghui------------------------------------------             
           call evaluate_first_order_S_right_reduce_memory(first_order_S, partition(1), &
                            n_points, n_compute_c,i_basis,   & 
                            wave, gradient_basis_wave_npoints,j_atom,j_coord)    
           !!! write(use_unit,*) i_my_batch,first_order_S(1,1,2,1) 
          !   if(i_my_batch.eq.1) then
          !   
          !    do i_point=1,2
          !      do i=1,n_compute_c 
          !    !   if(wave(i,i_point).gt.100.or. & 
          !    !gradient_basis_wave_npoints(i,2,i_point).gt.100) then 
          !    write(use_unit,*) i_point, wave(i,i_point), & 
          !    gradient_basis_wave_npoints(i,2,i_point) 
          !    !   stop
          !    !   endif
          !      enddo
          !    enddo
...
!----------------------------shanghui-------------------------------------------
  ...
  if(.not. use_local_index) call sync_first_order_matrix( first_order_S )  
...
end subroutine integrate_first_order_S_right_reduce_memory
!******
