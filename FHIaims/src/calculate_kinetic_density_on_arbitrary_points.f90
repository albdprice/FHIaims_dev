subroutine calculate_kinetic_density_cube_on_arbitrary_points &
     (n_points, points, &!rho, rho_gradient, &
!     hartree_partition_tab,  partition_tab, basis_l_max, &
!     KS_eigenvector, KS_eigenvector_complex, occ_numbers, &
     density_matrix, density_matrix_sparse, &
     kinetic_density_cube)

!  PURPOSE
!  Calculates the density matrix on a supplied array of points
!  This routine is copied and adapted from the file meta_gga_postproc.f90
...
!  ARGUMENTS
  ...
  real*8, dimension(n_points,3), intent(in) :: points
  ...
  real*8, dimension(2,n_points) :: kinetic_density_cube

!  real*8,  dimension(n_spin, n_points)     :: rho
!  real*8,  dimension(3, n_spin, n_points)  :: rho_gradient
...
! o points -- coordinates of the points
...
!
! OUTPUTS
! o kinetic_density_cube -- contains the kinetic density on the coordinates supplied in points
!  AUTHOR
...
!  SOURCE
  ...
  real*8,  dimension(n_points)             ::  partition_tab !Dummy array, not actually used
  real*8,  dimension(n_points)             ::  hartree_partition_tab !Dummy array, not actually used
...
!  integer :: n_points
  ...
  real*8, dimension(:), allocatable :: temp_kinetic_density_cube
...
!  call localorb_info( &
!       "  Calculating kinetic energy density for cube files ", 6,'(2X,A)', OL_norm )

!safeguard
  if (n_points.lt.n_points) stop
  hartree_partition_tab=1.0
  partition_tab=1.0
...
! continue allocating stuff
     ...
     if(.not. allocated(dist_tab_sq))  then
        allocate(dist_tab_sq(n_centers_integrals, n_points),stat=info)
     ...
     if(.not. allocated(dir_tab)) then
        allocate(dir_tab(3, n_centers_integrals, n_points),stat=info)
     ...
     if(.not. allocated(work))then
        allocate(work(n_max_compute_dens, n_points),stat=info)
        call check_allocation(info, 'work                          ')
     end if
     if(.not. allocated(grad_work))then
        allocate(grad_work(n_max_compute_dens, n_points),stat=info)
        call check_allocation(info, 'grad_work                     ')
     end if
     if(.not. allocated(temp_kinetic_density_cube))then
        allocate(temp_kinetic_density_cube(n_points),stat=info)
        call check_allocation(info, 'temp_kinetic_density_cube          ')
     end if
     ...
     if(.not. allocated(wave))then
        allocate(wave(n_max_compute_dens, n_points),stat=info)
...
!   allocate local arrays needed for gradients
        if(.not. allocated (gradient_basis_wave))then
           allocate (gradient_basis_wave(n_max_compute_dens, 3, n_points),stat=info)
...
! first initialize variables
     ...
     kinetic_density_cube(i_spin,:) = 0.d0
     i_basis_fns_inv = 0
...
! go over the batches
!    do i_my_batch = 1, n_my_batches, 1 !only a single batch - this one

           temp_kinetic_density_cube(:)=0.d0
...
!                i_point = i_point+1
                 ...
                 coord_current(:) = points(i_point,:)  !batches(i_my_batch) % points(i_index) % coords(:)
                 ...
                 if (max(partition_tab(i_full_points_2),&
                   hartree_partition_tab(i_full_points_2)).gt.0.d0) then
                    ...
                    ( n_max_compute_atoms, n_max_compute_fns_dens, &
                       dist_tab_sq(1,i_point2), dist_tab, dir_tab(1,1,i_point2), &
                    ...
                    ! of current integration point, viewed from all atoms
                       ...
                       call tab_trigonom_p0 &
                            ( n_compute_atoms, dir_tab(1,1,i_point2),  &
                            trigonom_tab(1,1) &
                            )
                       ...
                       call evaluate_wave_gradient_p2  &
                          ( n_compute_c, n_compute_atoms, n_compute_fns, &
                            one_over_dist_tab, dir_tab(1,1,i_point2), trigonom_tab(1,1),  &
                            l_ylm_max, ylm_tab,  &
                            dylm_dtheta_tab,  &
                            scaled_dylm_dphi_tab,  &
                            radial_wave,  &
                            radial_wave_deriv,  &
                            gradient_basis_wave (1:n_compute_c,1:3,i_point2),  &
...
! In this case, really finally evaluate the kinetic density
                    call evaluate_kinetic_density_denmat  &
                         ...
                         density_matrix_con,  temp_kinetic_density_cube,  &
                         n_max_compute_dens)
...
!Since we have only a single batch, this is not needed
!              i_point = 0
!              do i_index = 1, batches(i_my_batch)%size, 1
!
...
!                    i_point = i_point + 1                    
!
!                    kinetic_density_cube(i_spin, i_full_points_3) =   &
!                            temp_kinetic_density_cube(i_point)
!
!                 end if
!               end do
...
!  if (allocated( density_matrix       )) deallocate( density_matrix       )
  if (allocated( temp_kinetic_density_cube )) deallocate( temp_kinetic_density_cube )

  return

end subroutine calculate_kinetic_density_cube_on_arbitrary_points
