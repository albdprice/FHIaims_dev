...
subroutine integrate_first_order_rho_p0 &
     (partition_tab_std, basis_l_max, density_matrix, first_order_density_matrix, &
      first_order_rho)
     ...
     if (n_compute_c.gt.0) then
        ...
        do i_index = 1, batches_work(i_my_batch)%size, 1
           ...
           if (partition_tab(i_full_points).gt.0.d0) then
             ...
             !------shanghui test first_order_rho) 
             !if(i_full_points.eq.1020) then 
             ! write(use_unit,*) i_my_batch,i_point
             ! stop
             !endif
...
end subroutine integrate_first_order_rho_p0
!******
