!****s* FHI-aims/integrate_pulay_hessian_p0
!  NAME
!   integrate_pulay_hessian_p0
!  SYNOPSIS

subroutine integrate_pulay_hessian_p0 &
...
!  ARGUMENTS
  ...
  real*8, target, dimension(3, n_atoms, n_full_points) :: first_order_potential_std
  ...
  real*8, dimension(n_Cbasis,n_Cbasis), intent(in) :: density_matrix
  real*8, dimension(n_Cbasis,n_Cbasis), intent(in) :: energy_density_matrix
  real*8, dimension(3, n_atoms, n_Cbasis,n_Cbasis),intent(in):: first_order_density_matrix
  real*8, dimension(3, n_atoms, n_Cbasis,n_Cbasis),intent(in):: first_order_energy_density_matrix
...
!  real*8 dir_tab(3,n_centers_integrals, n_max_batch_size)
  ...
  real*8, dimension( :, :, :), allocatable :: local_first_order_DM_rho
...
! VB - renewed index infrastructure starts here
  ...
  allocate( local_first_order_potential(3,n_atoms,n_max_batch_size),stat=info)
...
!  end if !-----shanghui always calcualet gradients-------------------------
  ...
  allocate(local_first_order_DM_rho(3,n_atoms, n_max_batch_size), STAT=info )
  call check_allocation(info, 'local_first_order_DM_rho                          ')
...
!-------For pulay_hessian--------------------------------------
    ...
    allocate(first_order_S(3,n_atoms,n_Cbasis,n_Cbasis),stat=info)
    call check_allocation(info, 'first_order_S            ')
    allocate(second_order_S(3,n_atoms,3,n_atoms,n_Cbasis,n_Cbasis),stat=info)
    call check_allocation(info, 'second_order_S            ')

    allocate(first_order_H_pulay(3,n_atoms,n_Cbasis,n_Cbasis),stat=info)
    call check_allocation(info, 'first_order_H_pulay            ')
    allocate(second_order_H_pulay(3,n_atoms,3,n_atoms,n_Cbasis,n_Cbasis),stat=info)
...
!-------End for pulay_hessian--------------------------------------
  ...
  first_order_H_pulay(1:3,1:n_atoms,1:n_Cbasis,1:n_Cbasis) = 0.0d0 
  first_order_S(1:3,1:n_atoms,1:n_Cbasis,1:n_Cbasis) = 0.0d0
  second_order_H_pulay(1:3,1:n_atoms,1:3,1:n_atoms,1:n_Cbasis,1:n_Cbasis) = 0.0d0
  second_order_S(1:3,1:n_atoms,1:3,1:n_atoms,1:n_Cbasis,1:n_Cbasis) = 0.0d0
     ...
     if (n_compute_c.gt.0) then
        ...
        do i_index = 1, batches_work(i_my_batch)%size, 1
           ...
           if (partition_tab(i_full_points).gt.0.d0) then
              ...
              do i_coord=1,3,1             
              local_first_order_potential(i_coord,1:n_atoms,i_point)=      &
              first_order_potential_std(i_coord,1:n_atoms,i_full_points)
...
!-------------------------shanghui add for first_H--------------------------- 
             ...
             call  evaluate_first_order_rho_p0( &
                    n_points, n_compute_c,i_basis,& 
                    wave,gradient_basis_wave_npoints, &
                    density_matrix,first_order_density_matrix, & 
                   local_first_order_DM_rho, local_first_order_rho)
...
!-------------------------shanghui end add-----------------------------------
        ...
        do i_spin = 1, n_spin, 1
           ...
           ! H_pulay(1)_{uv}:  
            call evaluate_first_order_H_pulay_p0(first_order_H_pulay, &
                 n_points,partition, & 
                 n_compute_c, i_basis,  &
                 H_times_psi(1,1,i_spin), gradient_basis_wave_npoints)

           !H_pulay(2)_{uv} :
            call evaluate_second_order_H_pulay_p0(second_order_H_pulay,  &
                 n_points, partition,   &
                 n_compute_c, i_basis, index_hessian, &
                 wave, gradient_basis_wave_npoints,H_times_psi(1,1,i_spin), & 
                 H_times_gradient_psi(1,1,1,i_spin), hessian_basis_wave,  &  
                 local_first_order_rho,local_first_order_potential, & 
                 local_dVxc_drho(i_spin,1:n_points) )


           !S(1)_{uv}
            call evaluate_first_order_S_p0(first_order_S, & 
                 n_points, partition, &
                 n_compute_c,i_basis,   &
                 wave, gradient_basis_wave_npoints)

           !S(2)_{uv} 
            call evaluate_second_order_S_p0(second_order_S, & 
                 n_points,partition, &
                 n_compute_c, i_basis, index_hessian, &
...
!      write(use_unit,*) 'shanghui test pulay_force:'
!      write(use_unit,*) ' x 1:', pulay_force(1,1)
!      write(use_unit,*) ' x 2:', pulay_force(1,2)
!      write(use_unit,*) ''
!----------------------end Pulay force test---------------------------------
...
!      write(use_unit,*) ''
!      write(use_unit,*) 'shanghui test Pulay_hessian:(a.u.)'
!      write(use_unit,*) ' x 1 x 1', pulay_hessian(1,1,1,1)
!      write(use_unit,*) ' x 1 x 2', pulay_hessian(1,1,1,2)
!      write(use_unit,*) ' x 2 x 1', pulay_hessian(1,2,1,1)
!      write(use_unit,*) ' x 2 x 2', pulay_hessian(1,2,1,2)
!      write(use_unit,*) ' y 1 y 1', pulay_hessian(2,1,2,1)
...
!  if(.not. use_local_index) call sync_integrate_hamiltonian( hamiltonian )
  ...
  if(allocated( local_first_order_DM_rho  )) deallocate( local_first_order_DM_rho      )
...
end subroutine integrate_pulay_hessian_p0
!******
