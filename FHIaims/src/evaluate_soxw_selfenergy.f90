!****s* FHI-aims/evaluate_soxw_selfenergy
!  NAME
!   evaluate_soxw_selfenergy
!  SYNOPSIS

      subroutine evaluate_soxw_selfenergy &
...
!     output
      complex*16   soxw_selfenergy(n_freq,n_KS_states,n_spin)
...
!  local variables
      ...
      real*8, dimension(:,:), allocatable :: ovlpw
...
!    determine the HOMO and LUMO level
      ...
      if(myid.eq.0) then
        write(use_unit,'(A)') 
        write(use_unit,'(A)') "---------------------------------------------"
        write(use_unit,'(2A)') " | Second-order exchange self-energy in terms of static W", &
                   " calculation starts ..."
        write(use_unit,'(A)') "---------------------------------------------"
        if (n_spin.gt.1) then
...
!       write(use_unit,*) &
!              " * Only one occupied state: SOXW correction is zero! "
!      return
!     endif
...
!      endif
      ...
      if (.not.allocated(dielec_func)) then
          allocate(dielec_func(n_basbas,n_loc_prodbas))
      endif
      if (.not.allocated(ovlpw)) then
          allocate(ovlpw(n_loc_prodbas,n_states))
      endif
...
!  calculate the static dielectric function
      do i_freq = 1, n_freq, 1
         call evaluate_polarisability_freq_0 &
             ( n_low_state, n_homo, n_lumo, n_KS_states, &
               occ_numbers, omega(i_freq), &
               KS_eigenvalue, ovlp_3KS(:,:,1:n_KS_states,:), polar_freq &
             )

         call screened_coulomb_interaction(polar_freq, dielec_func)

         call power_auxmat_lapack(dielec_func, 5.d-1, '')

         do i_spin = 1, n_spin, 1
           bare_v = 0.d0
           do i_state = n_homo(i_spin), n_homo(i_spin), 1
                ovlpw(:,:)=ovlp_3KS(:,:,i_state,i_spin)
                call multi_ovlp3KS_sqrtw(dielec_func,ovlpw)
!                ovlp_3KS(:,:,i_state,i_spin)=ovlpw(:,:)
              do i_prodbas_1 = 1, n_basbas, 1
                bare_v(1) = bare_v(1) + &
                      ovlpw(i_prodbas_1, n_homo(1)) * ovlpw(i_prodbas_1, n_homo(1))
                bare_v(2) = bare_v(2) + &
                      ovlpw(i_prodbas_1, n_lumo_min) * ovlpw(i_prodbas_1, n_lumo_min)
              enddo
           enddo
          write(use_unit,'(3f16.8)') omega(i_freq), bare_v(1)*hartree, bare_v(2)*hartree
         enddo
...
!        i_basbas = map_prodbas(i_prodbas_1, myid+1)
!        if(i_basbas .gt. 0) then
!          dielec_func(i_basbas,i_prodbas_1) = 1.d0
!        endif
!      enddo
      do i_state = n_low_state, n_KS_states, 1
         bare_v = 0.d0
         do i_prodbas_1 = 1, n_basbas, 1
           bare_v = bare_v + ovlp_3KS(i_prodbas_1, n_lumo_min, i_state,1) &
            * ovlp_3KS(i_prodbas_1, n_lumo_min, i_state,1)
         enddo
         write(use_unit,'(I4,f16.8)') i_state, bare_v(1)
      enddo

      soxw_selfenergy = (0.d0,0.d0)
      do i_spin = 1, n_spin, 1

! multiply the square root of the dielectric function to ovlp_3KS
          do i_state = n_low_state, n_KS_states, 1
              ovlpw(:,:)=ovlp_3KS(:,:,i_state,i_spin)
              call multi_ovlp3KS_sqrtw(dielec_func,ovlpw)
              ovlp_3KS(:,:,i_state,i_spin)=ovlpw(:,:)
          ...
          do i_state = n_low_state, n_KS_states, 1
            ...
            do i_state_2 = 1, n_homo(i_spin), 1
              ...
              call dgemm('T', 'N', n_unoccupied, n_unoccupied, &
                     n_loc_prodbas, 1.0d0, &
                     ovlp_3KS(:,n_lumo_min:n_states,i_state,i_spin), &
                     n_loc_prodbas, &
                     ovlp_3KS(:,n_lumo_min:n_states,i_state_2,i_spin), &
                     n_loc_prodbas,0.d0, &
              ...
              call sync_matrix &
               ...
               if(i_state .eq. i_state_2) then
                 write(use_unit,'(2f18.8)') aux_eri(n_lumo_min,n_lumo_min), aux_eri(n_lumo_min+1,n_lumo_min+1)
               endif
...
!   close a virtual state
             ...
             do i_state_2 = n_lumo(i_spin), n_states, 1
              ...
              call dgemm('T', 'N', n_homo(i_spin), &
                      n_homo(i_spin), &
                      n_loc_prodbas, 1.0d0, &
                      ovlp_3KS(:,i_state,1:n_homo(i_spin),i_spin), &
                      n_loc_prodbas, &
                      ovlp_3KS(:,i_state_2,1:n_homo(i_spin),i_spin), &
                      n_loc_prodbas,0.d0, &
...
! End of loop over i_spin
      ...
      if(use_mpi) then
        do i_spin = 1, n_spin, 1
          call sync_matrix_complex &
             (soxw_selfenergy(:,:,i_spin),n_freq,n_KS_states)
      ...
      end subroutine evaluate_soxw_selfenergy


!******
!---------------------------------------------------------------------------------------------------

      subroutine evaluate_soxw_selfenergy_2 &
       ( n_freq, n_low_state, n_KS_states, &
         omega, &
         ovlp_3KS, &
         chemical_potential_spin, &
...
!    determine the HOMO and LUMO level
      ...
      if(myid.eq.0) then
        write(use_unit,'(A)') 
        write(use_unit,'(A)') "---------------------------------------------"
        write(use_unit,'(2A)') " | Second-order exchange self-energy in terms of ", &
                     "static W ation starts ..."
        write(use_unit,'(A)') "---------------------------------------------"
        if (n_spin.gt.1) then
          write(use_unit,'(A)') "  | Note : Spin-polarized system "
        else
...
!      if (n_spin.eq.1.and.n_homo_max.eq.1) then
!       write(use_unit,*) &
!               " * Only one occupied state: 2nd-order exchange in terms of W correction is zero! "
!       return
!      endif
...
!      endif
      ...
      end subroutine evaluate_soxw_selfenergy_2
