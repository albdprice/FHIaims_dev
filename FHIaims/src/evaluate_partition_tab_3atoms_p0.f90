!****s* FHI-aims/evaluate_partition_tab_3atoms_p0
!  NAME
!   evaluate_partition_tab_3atoms_p0
!  SYNOPSIS

      subroutine evaluate_partition_tab_3atoms_p0 &
      ( i_atom, dist_tab, i_r, &
...
!  PURPOSE
!     Subroutine evaluate_partition_tab_3atoms
!     creates partition_tab for the present integration point among
!     3 atoms. These three atoms can be either identical or different.
!     In particular, the atom i is given, and partition_tab_3atoms
!     return partition tab for all possible different combination
!     of 2 atoms.
...
!  ARGUMENTS
      ...
      real*8 partition_tab(n_atoms_list_max*(n_atoms_list_max+1)/2)
...
!  OUTPUT
!  o  partition_tab -- the 3-center partition functions
!
!  AUTHOR
...
!     loop over the second atom
        do i_atom_2_ind = 1, i_atom_1_ind, 1
           i_atom_2 = atoms_list(i_atom_2_ind)
         partition_norm = partition_norm_start

          i_index = i_index +1
          if (i_atom_2.ne.i_atom.and.i_atom_2.ne.i_atom_1) then

           aux_dens =   val_spline &
            ( i_r(i_atom_2_ind), partition_rho_spl(1,1,species_center(i_atom_2)), &
             n_grid(species_center(i_atom_2)) )

           select case(partition_type_temp)

            case(1)
               aux_partition_norm_2 = &
                   aux_dens / (dist_tab(i_atom_2_ind)**2.0d0) + par_offset
            case(2)
               aux_partition_norm_2 = &
                   aux_dens / abs(dist_tab(i_atom_2_ind))
            case(3)
               aux_partition_norm_2 = &
                    aux_dens
            case(4)
               aux_partition_norm_2 = &
                  1.0/ ( 1+ exp( (dist_tab(i_atom_2_ind)- &
                        hartree_partition_parameters(1))/ &
                        hartree_partition_parameters(2) ) )
            case(6)
               aux_partition_norm_2 = &
           ...
           if (i_atom_1 .eq. i_atom) then
            ...
            partition_norm = partition_norm_start+aux_partition_norm_2
           else
          ...
          elseif (i_atom_1.ne.i_atom ) then

            partition_norm = partition_norm_start + aux_partition_norm

!     end of if (i_atom_2.ne.i_atom .and. i_atom_2.ne.i_atom_1)
          endif
...
!      end loop over other atoms, i_atom_2_ind
        enddo

!     end loop over other atoms, i_atom_1_ind
      enddo

    end subroutine evaluate_partition_tab_3atoms_p0
