!****s* FHI-aims/check_norm
!  NAME
!    check_norm
!  SYNOPSIS

subroutine check_norm(chemical_potential, KS_eigenvalue, n_electrons, occ_numbers, diff_electrons, i_counter)
...
!  INPUTS
!   o chemical_potential -- chemical potentila
!   o KS_eigenvalue -- Kohn-Sham eigenvalue
...
!  end do
  ...
  case (0)
     ...
     do i_spin = 1, n_spin, 1
        do i_state = 1, n_states, 1
            occ_numbers(i_state, i_spin) = spin_degeneracy * 0.5d0 * & 
           (1 - arch_erf((KS_eigenvalue(i_state, i_spin) - chemical_potential) * one_over_ow))
           temp_n_electrons = temp_n_electrons + occ_numbers(i_state, i_spin)
        end do
     end do
  ...
  case (1)
     ...
     do i_spin = 1, n_spin, 1
        do i_state = 1, n_states, 1
           exp_arg = (KS_eigenvalue(i_state, i_spin) - chemical_potential) * one_over_ow
           if (exp_arg .lt. max_exponent) then
              occ_numbers(i_state, i_spin) = spin_degeneracy / (1 + exp(exp_arg))
              temp_n_electrons = temp_n_electrons + occ_numbers(i_state, i_spin)
           else
              occ_numbers(i_state, i_spin) = 0.d0
           end if
        end do
     end do
  ...
  case (2)
     ...
     do i_spin = 1, n_spin, 1
        do i_state = 1, n_states, 1
           hermite_arg  = (KS_eigenvalue(i_state, i_spin) - chemical_potential) * one_over_ow 
           gauss_weight = exp(- hermite_arg * hermite_arg)
           !     zero order contribution
           occ_numbers(i_state, i_spin) = 0.5 * (1 - arch_erf(hermite_arg))
...
!              write(use_unit,*) A, H_even, H_odd, hermite_arg
              ...
              !     H_odd =  H_1 = 2 * x
              H_odd  = 2 * hermite_arg
              occ_numbers(i_state, i_spin) = occ_numbers(i_state, i_spin) + A * H_odd * gauss_weight
           end if
