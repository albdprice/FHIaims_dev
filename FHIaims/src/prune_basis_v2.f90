!****s* FHI-aims/prune_basis_v2
!  NAME
!    prune_basis_v2
!  SYNOPSIS

subroutine prune_basis_v2 &
     ( dist_tab_sq,  n_compute, i_basis &
     )
...
!
!     The only difference to prune_basis_v1() is that the squares of distances
!     are used.
!     
!  USES
...
!       this is the first point of the batch - simply list all non-zero wave functions
     ...
     do i_basis_1 = 1, n_basis, 1
        ...
        if (dist_tab_sq(basis_atom(i_basis_1)) .le. &
             outer_radius_sq(basis_fn(i_basis_1)) ) then
...
!     basis function already in list
        ...
        else if (dist_tab_sq(basis_atom(i_basis_1)) .le. &
             outer_radius_sq(basis_fn(i_basis_1)) ) then
