...
SUBROUTINE interpolate_interval( xl, xh, yl, yh, d2ydx2l, d2ydx2h, x, y, dydx, secyx )
! ^LML
...
real(dp),         intent(out):: y       ! function value at point x
real(dp),optional,intent(out):: dydx    ! function derivative at point x
real(dp),optional,intent(out):: secyx   ! second function derivative at point x
! ^LML
...
if (present(secyx)) then
  secyx = A*d2ydx2l + B*d2ydx2h
end if
! ^LML
...
SUBROUTINE evaluate_spline_dx( dx, yi, d2ydx2, n, x, y, dydx, secyx )
! ^LML
...
if (present(secyx)) then
call interpolate_interval( xl, xh, yi(kl), yi(kh), &
                           d2ydx2(kl), d2ydx2(kh), x, y, dydx, secyx )
else
call interpolate_interval( xl, xh, yi(kl), yi(kh), &
                           d2ydx2(kl), d2ydx2(kh), x, y, dydx ) 
end if
