...
C 7) RPHIATM with ITYPE = 0 is strictly equivalent to VNA_SUB
      ...
      subroutine all_phi( is, it, r, nphi, phi, grphi, Ggrphi )
C ^LML
      integer,   intent(in) :: is     ! Species index
      ...
      real(dp), optional, intent(out) :: Ggrphi(:,:,:)
C ^LML
...
C    value of nphi
      ...
      integer i, ig, jg, jlm, l, lmax, m, maxlm, n
C ^LML
      double precision  rmod, phir, dphidr
      double precision d2phidr2
C ^LML
      real(dp) rly(max_ilm), grly(3,max_ilm)
      real(dp) ggrly(3,3,max_ilm)
C ^LML
...
!     Check size of output arrays
     ...
     .    call die('all_phi: incorrect first dimension of grphi')
      ...
      if (present(Ggrphi)) then
        if (size(Ggrphi,1).ne.3)
     .    call die('all_phi: incorrect first dimension of Ggrphi')
        if (size(Ggrphi,2).ne.3)
     .    call die('all_phi: incorrect second dimension of Ggrphi')
        n = min( size(phi), size(Ggrphi,3) )
      else
        n = size(phi)
      end if
! ^LML
...
!     Initialize orbital values
      ...
      if (present(Ggrphi)) Ggrphi(:,:,1:nphi) = 0._dp
! ^LML
...
!     Find spherical harmonics
      ...
      if (present(Ggrphi)) then
        call rlylm(lmax,r,rly,grly,ggrly)
      else
...
!       Find radial part
        ...
        if (present(Ggrphi)) then
          call rad_get(func,rmod,phir,dphidr,d2phidr2)
        else
          call rad_get(func,rmod,phir,dphidr)
        end if
! ^LML
...
!       Multiply radial and angular parts
     ...
     .                 phir * grly(:,jlm)
        ...
        if (present(Ggrphi)) then
          do ig = 1,3
            do jg = 1,3
              Ggrphi(ig,jg,i) = phir * ggrly(ig,jg,jlm) +
     .                   rly(jlm) * d2phidr2 * r(ig) * r(jg) / rmod**2 +
     .                   grly(ig,jlm) * dphidr * r(ig) / rmod +
     .                   grly(jg,jlm) * dphidr * r(jg) / rmod
            end do
          end do
        end if
! ^LML
