...
!
      ...
      subroutine rlylm( LMAX, R, RLY, GRLY, GGRLY )
! ^LML
...
!      real(dp), intent(out) :: grly(3,0:)   !!! Not accepted...
      ...
      real(dp), optional, intent(out) :: ggrly(1:,1:,0:)
! ^LML
...
C REAL*8 GGRLY(3,,3LMAX*LMAX) : Gradient of the GRLY's at point R.
C ^LML
C *********** UNITS ***************************************************
...
C ^LML
         ...
         IF (PRESENT(GGRLY)) THEN
           GGRLY(3,2,5) = (-C(5))*THREE
           GGRLY(2,3,5) = GGRLY(3,2,5)
         END IF
C ^LML
...
C ^LML
         ...
         IF (PRESENT(GGRLY)) THEN
           GGRLY(3,1,7) = (-C(7))*THREE
           GGRLY(1,3,7) = GGRLY(3,1,7)
         END IF
C ^LML
...
C ^LML
!            ZP(L,M)=( (L+M)*P(L-1,M) - L*RZ*P(L,M) ) / RXY**2
            Z2P(L,M)=( 2*RZ*ZP(L,M) + (L*(L+1) - M**2/RXY**2)*P(L,M) )
     .               / RXY**2
! Lebedev 'Special Functions and Their Applications; Spherical Harmonics: Theory' Chapter 7, Prentice Hall, 1965
! Poulakris 'Legendre Polynomials' Chapter 21, CRC Press, 1999
! Dunster 'Legendre and Related Functions' Chapter 14, http://dlmf.nist.gov/14.10
C ^LML
   50   CONTINUE
   60 CONTINUE
        ...
        DO 80 L=M,LMAX
          DO 75 MS = -1,1,2
            IF (MS.EQ.-1) THEN
              ...
              IF (PRESENT(GGRLY)) THEN
                GGY(1,1)=Z2P(L,M) * RX**2 * RZ**2 * SINM
     .                   - ZP(L,M) * RZ * SINM
     .                   + ZP(L,M) * RX * RZ * M * COSM * SINPHI/RXY
     .                   + ZP(L,M) * RX * RZ * M * COSM * SINPHI/RXY
     .                   - P(L,M) * M**2 * SINM * SINPHI**2/RXY**2
     .                   + P(L,M) * M * COSM * RY * RX/RXY**4
     .                   + P(L,M) * M * COSM * SINPHI * RX/RXY**3
!                GGY(1,2)=
!                GGY(1,3)=
!                GGY(2,1)=
                GGY(2,2)=Z2P(L,M) * RY**2 * RZ**2 * SINM
     .                   - ZP(L,M) * RZ * SINM
     .                   - ZP(L,M) * RY * RZ * M * SINM * COSPHI/RXY
     .                   - ZP(L,M) * RY * RZ * M * COSM * COSPHI/RXY
     .                   - P(L,M) * M**2 * SINM * COSPHI**2/RXY**2
     .                   - P(L,M) * M * COSM * RX * RY/RXY**4
     .                   - P(L,M) * M * COSM * COSPHI * RY/RXY**3
!                GGY(2,3)=
!                GGY(3,1)=
!                GGY(3,2)=
                GGY(3,3)=Z2P(L,M) * RXY**4 * SINM
              END IF
C ^LML
            ELSE
              ILM=L*L+L+M
              ...
              IF (PRESENT(GGRLY)) THEN
                GGY(1,1)=Z2P(L,M) * RX**2 * RZ**2 * COSM
     .                   - ZP(L,M) * RZ * COSM
     .                   - ZP(L,M) * RX * RZ * M * SINM * SINPHI/RXY
     .                   - ZP(L,M) * RX * RZ * M * SINM * SINPHI/RXY
     .                   - P(L,M) * M**2 * COSM * SINPHI**2/RXY**2
     .                   - P(L,M) * M * SINM * RY * RX/RXY**4
     .                   - P(L,M) * M * SINM * SINPHI * RX/RXY**3
!                GGY(1,2)=
!                GGY(1,3)=
!                GGY(2,1)=
                GGY(2,2)=Z2P(L,M) * RY**2 * RZ**2 * COSM
     .                   - ZP(L,M) * RZ * COSM
     .                   + ZP(L,M) * RY * RZ * M * SINM * COSPHI/RXY
     .                   + ZP(L,M) * RY * RZ * M * SINM * COSPHI/RXY
     .                   - P(L,M) * M**2 * COSM * COSPHI**2/RXY**2
     .                   + P(L,M) * M * SINM * RX * RY/RXY**4
     .                   + P(L,M) * M * SINM * COSPHI * RY/RXY**3
!                GGY(2,3)=
!                GGY(3,1)=
!                GGY(3,2)=
                GGY(3,3)=Z2P(L,M) * RXY**4 * COSM
              END IF
C ^LML
            ENDIF
            ...
            IF (PRESENT(GGRLY)) THEN
              GGY(1,1)= GGY(1,1)*C(ILM)/RSIZE
!              GGY(1,2)= GGY(1,2)*C(ILM)/RSIZE
!              GGY(1,3)= GGY(1,3)*C(ILM)/RSIZE
!              GGY(2,1)= GGY(2,1)*C(ILM)/RSIZE
              GGY(2,2)= GGY(2,2)*C(ILM)/RSIZE
!              GGY(2,3)= GGY(2,3)*C(ILM)/RSIZE
!              GGY(3,1)= GGY(3,1)*C(ILM)/RSIZE
!              GGY(3,2)= GGY(3,2)*C(ILM)/RSIZE
              GGY(3,3)= GGY(3,3)*C(ILM)/RSIZE
            END IF
C ^LML
            RLY(ILM)=RL(L)*YY
            ...
            IF (PRESENT(GGRLY)) THEN
              GGRLY(1,1,ILM)=L*RL(L-1)*YY + RX**2*L*(L-1)*RL(L-2)*YY
     .                       + 2*RX*L*RL(L-1)*GY(1) + RL(L)*GGY(1,1)
!              GGRLY(1,2,ILM)=
!              GGRLY(1,3,ILM)=
!              GGRLY(2,1,ILM)=
              GGRLY(2,2,ILM)=L*RL(L-1)*YY + RY**2*L*(L-1)*RL(L-2)*YY
     .                       + 2*RY*L*RL(L-1)*GY(2) + RL(L)*GGY(2,2)
!              GGRLY(2,3,ILM)=
!              GGRLY(3,1,ILM)=
!              GGRLY(3,2,ILM)=
              GGRLY(3,3,ILM)=L*RL(L-1)*YY + RZ**2*L*(L-1)*RL(L-2)*YY
     .                       + 2*RZ*L*RL(L-1)*GY(3) + RL(L)*GGY(3,3)
            END IF
C ^LML
   75     CONTINUE
   80   CONTINUE
...
C Makes a radial times spherical-harmonic expansion of a function.
      ...
      integer, intent(in)          :: lmax
      interface
         subroutine rlylm_f(lmax,rvec,yy,grady,ggrady)
C ^LML
         ...
         real(dp), optional, intent(out) :: ggrady(1:,1:,0:)
C ^LML
