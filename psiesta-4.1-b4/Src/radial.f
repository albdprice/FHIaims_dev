...
!      allocate(func%f(n),func%d2(n))
      ...
      subroutine rad_get(func,r,fr,dfdr,d2fdr2)
! ^LML
      type(rad_func), intent(in) :: func
      real(dp), intent(in)         :: r
      real(dp), intent(out)        :: fr
      real(dp), intent(out)        :: dfdr
      real(dp), optional, intent(out) :: d2fdr2
! ^LML

      if (func%n .eq. 0) then
          fr = 0._dp
          dfdr = 0._dp
        if (present(d2fdr2)) then
          d2fdr2 = 0._dp
        end if
! ^LML
      else
        if (present(d2fdr2)) then
          call splint(func%delta,func%f,func%d2,func%n,r,fr,dfdr,d2fdr2)
        else
