...
! *******************************************************************
! MODULE m_minvec
!   Just wraps subroutine minvec
! *******************************************************************
! SUBROUTINE minvec(B0,BMIN,C)
!   FINDS THE LATTICE BASIS OF MINIMUM LENGTH, I.E. SUCH TAHT ANY 
!   OTHER BASIS (NOT EQUIVALENT BY SYMMETRY) HAS ONE VECTOR LONGER.
!   WRITTEN BY J.MORENO AND J.SOLER. AUGUST 1989 AND OCTOBER 1997.
! --------- INPUT ---------------------------------------------------
! REAL*8 B0(3,3)   : Cell vectors B0(xyj,vector)
! --------- OUTPUT --------------------------------------------------
! REAL*8 BMIN(3,3) : Minimum cell vectors B0(xyj,vector)
! REAL*8 C(3,3)    : Transformation matrix: BMIN=MATMUL(B0,C)
! *******************************************************************
  ...
  SUBROUTINE minvec(B0,BMIN,C)
    ...
    iter_loop: DO ITER=1,NITER
      ...
      DO I1=0,1
        DO I2=-1,1
          i3_loop: DO I3=-1,1
            IF (I1.EQ.0.AND.I2.NE.1) cycle i3_loop
            IF (I2.EQ.0.AND.I3.EQ.0) cycle i3_loop
            BNEW(1)=B(1,1)*I1+B(1,2)*I2+B(1,3)*I3
            BNEW(2)=B(2,1)*I1+B(2,2)*I2+B(2,3)*I3
            BNEW(3)=B(3,1)*I1+B(3,2)*I2+B(3,3)*I3
            BNEW2=SUM(BNEW**2)
            DO I=3,1,-1
              IF (BNEW2+EPS.GE.B2(1,I)) cycle i3_loop
