...
! use mpi_instr, only: MPI_AllGather
! use mpi_instr, only: MPI_Send
! use mpi_instr, only: MPI_Recv
! use mpi_instr, only: MPI_COMM_WORLD
...
! Used MPI procedures and types
  ...
  ! MPI variables
#ifdef MPI
  use gridxc_config, only: comm => gridxc_comm
#endif
  use gridxc_config, only: myNode => gridxc_myNode 
  use gridxc_config, only: totNodes => gridxc_totNodes
...
! Gather the boxes of all nodes
  call MPI_AllGather( box(1,1) , 6, MPI_Integer, &
                      boxes(1,1,0), 6, MPI_Integer, COMM, MPIerror )
#else
...
! Gather initial transfer sequence of all nodes
  call MPI_AllGather( myTrsf, maxTrsf, MPI_Integer, &
                      inTrsf(1,1), maxTrsf, MPI_Integer, COMM, MPIerror )
...
!      end do ! iPart
      ...
      if (trsfSize>0) &
        call MPI_Recv( trsfBuff(1:trsfSize), trsfSize, MPI_grid_real, &
                       srcNode, 0, COMM, MPIstatus, MPIerror )
