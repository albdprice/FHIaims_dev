...
!!            call pop_elstack(fx%element_stack,dummy)
            ...
            else if (have_pcdata_handler) then
               if (fx%debug) print *, &
                    "... Warning: CDATA section chunk processed as pcdata"
               call pcdata_chunk_handler(str(fx%pcdata))
            endif

         else if (fx%context == COMMENT_TAG) then
            ! It might overflow for long comments
      ...
      else if (signal == CHUNK_OF_CDATA_SECTION) then

         if (fx%debug) print *, "We found a chunk of CDATA"
         if (is_empty(fx%element_stack)) then
            if (fx%debug) print *, "... Warning: CDATA outside element context"
               ! Just a warning
               call build_error_info(error_info, &
                  "CDATA outside of element context", &
                  line(fb),column(fb),fx%element_stack,WARNING_CODE)
               if (have_error_handler) then
                  call error_handler(error_info)
               else
            ...
            if (have_cdata_section_handler) then
               call cdata_section_handler(str(fx%pcdata))
            ! Note that this will replace entities!
            else if (have_pcdata_handler) then
               if (fx%debug) print *, &
                    "... Warning: CDATA section chunk processed as pcdata"
               call pcdata_chunk_handler(str(fx%pcdata))
