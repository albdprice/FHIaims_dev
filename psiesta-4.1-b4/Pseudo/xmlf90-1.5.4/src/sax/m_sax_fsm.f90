...
integer, parameter, public   ::  CHUNK_OF_CDATA_SECTION  = 1300
...
select case(fx%state)
 ...
 case (IN_CDATA_SECTION)
      if (c == "]") then
         ...
         ! Note that we do not split in whitespace (there might not be)
         call add_to_buffer(c,fx%buffer)
         if (buffer_nearly_full(fx%buffer)) then
               signal = CHUNK_OF_CDATA_SECTION
               if (fx%debug) fx%action = ("Resetting almost full CDATA buffer")
               fx%pcdata = fx%buffer
               call reset_buffer(fx%buffer)
         else
